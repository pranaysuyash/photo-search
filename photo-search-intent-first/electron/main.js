...
/**
 * Main entry point for Photo Search Electron application
 */
...
function httpPing(url, timeoutMs = 1000) {
...
// Check if the API server is already running on a given port
async function checkAPIRunning(port = 8000) {
  const endpoints = [
...
function loadUI(target) {
  ...
  mainWindow.webContents.on('did-fail-load', (_event, errorCode, errorDescription, validatedURL) => {
    ...
    try {
      ...
      log.error('Critical: fallback UI load also failed:', e?.message);
    }
    dialog.showErrorBox('Boot Failure', `Failed to load UI (${errorCode}): ${errorDescription}`);
  });
  ...
  mainWindow.webContents.on('will-navigate', (e, url) => {
    ...
    if (!url.startsWith('http://127.0.0.1:') && !url.startsWith('file://')) {
      e.preventDefault();
      log.warn('[Nav] Blocked external navigation to:', url);
    }
  });
  ...
  mainWindow.webContents.setWindowOpenHandler(({ url }) => {
    try {
      ...
      const u = new URL(url);
      // Allow opening external resources in default browser
...
const pythonServiceSupervisor = new PythonServiceSupervisor({
  ...
  pythonPath: path.join(path.resolve(__dirname, '..'), '.venv', 'bin', 'python'),
  logLevel: 'info',
...
app.whenReady().then(async () => {
  ...
  try {
    ...
    log.error('[Services] Failed to start Python service supervisor:', error);
  }
...
app.whenReady().then(() => {
  ...
  // Get service supervisor status
  ipcMain.handle('python-service:get-status', async () => {
    return pythonServiceSupervisor.getStatus();
  });

  // Get service supervisor configuration
  ipcMain.handle('python-service:get-config', async () => {
    return pythonServiceSupervisor.getConfig();
  });
  ...
  ipcMain.handle('python-service:restart', async () => {
    try {
      await pythonServiceSupervisor.restart();
      return { success: true, status: pythonServiceSupervisor.getStatus() };
    } catch (error) {
      return { success: false, error: error.message, status: pythonServiceSupervisor.getStatus() };
    }
  });

  // Start Python service
  ipcMain.handle('python-service:start', async () => {
    try {
      const success = await pythonServiceSupervisor.start();
      return { success, status: pythonServiceSupervisor.getStatus() };
    } catch (error) {
      return { success: false, error: error.message, status: pythonServiceSupervisor.getStatus() };
    }
  });
  ...
  // Get API token from Python service
  ipcMain.handle('python-service:get-api-token', async () => {
    return pythonServiceSupervisor.getApiToken() || '';
  });

  // Get API configuration from Python service
  ipcMain.handle('python-service:get-api-config', async () => {
