...
const PHOTO_CACHE = 'photovault-photos-v1';
const METADATA_CACHE = 'photovault-metadata-v1';
  ...
  event.waitUntil(
    caches.keys().then((cacheNames) => {
      return Promise.all(
        cacheNames
          .filter((cacheName) => {
            return cacheName.startsWith('photovault-') && 
...
self.addEventListener('fetch', (event) => {
  ...
  if (url.pathname.startsWith('/api/thumb/') || url.pathname.startsWith('/api/media/')) {
    event.respondWith(
      photoCacheStrategy(request)
    );
    return;
  }
