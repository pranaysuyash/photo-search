"""
OCR (Optical Character Recognition) routes - text extraction from images.

Handles building OCR indexes, retrieving text snippets from images,
and checking OCR processing status.
"""
...
def api_build_ocr(
    ...
    """Build OCR text index for a photo directory."""
    ...
    # Log analytics event
    try:
        _write_event(store.index_dir, { 'type': 'ocr_build', 'updated': updated, 'langs': languages_value or [] })
    except Exception:
...
def api_ocr_snippets(
    ...
    """Get OCR text snippets for specific image paths."""
    dir_value = _require(_from_body(body, dir, "dir"), "dir")
