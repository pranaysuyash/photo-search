"""
Models-related endpoints for API v1.
"""
...
@models_router.get("/status", response_model=SuccessResponse)
def model_status_v1(
    force: int = Query(0),
    ...
    if cached is not None and now - ts < _CACHE_TTL:
        return SuccessResponse(ok=True, data=cached)  # type: ignore[return-value]
    payload = _build_status_payload()
    _cache["payload"] = payload
    _cache["ts"] = now
    return SuccessResponse(ok=True, data=payload)