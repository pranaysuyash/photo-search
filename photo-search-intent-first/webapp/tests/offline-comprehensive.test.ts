...
import { waitForAppReady, dismissOverlays, safeClick } from "./utils/test-helpers";
...
test.describe("Comprehensive Offline Functionality", () => {
  test.beforeEach(async ({ page, context }) => {
    ...
    await page.waitForFunction(() => {
      ...
      console.log("Service worker not available, continuing with offline tests");
    });
  });
  ...
  test.describe("Offline Detection and Indicators", () => {
    ...
    test("should hide offline indicator when connection is restored", async ({ page, context }) => {
      // First go offline
      await context.setOffline(true);
      await page.waitForTimeout(2000);
    ...
    test("should update network status in real-time", async ({ page, context }) => {
  ...
  test.describe("Model Status Monitoring", () => {
    test("should display model status indicator in offline mode", async ({ page, context }) => {
      // Model status indicator should be visible
      const modelStatus = page.locator('[data-testid="model-status"], .model-status, [class*="model-status"]');
      await expect(modelStatus).toBeVisible();

      // Should show offline mode badge
      const offlineBadge = modelStatus.locator('[class*="offline"], .offline-badge');
    ...
    test("should handle model status updates when going offline", async ({ page, context }) => {
      const modelStatus = page.locator('[data-testid="model-status"], .model-status, [class*="model-status"]');
    ...
    test("should display model capabilities correctly", async ({ page, context }) => {
      const modelStatus = page.locator('[data-testid="model-status"], .model-status, [class*="model-status"]');

      // Should show capabilities section
      await expect(modelStatus).toContainText(/capabilities|gpu|cuda|mps|clip/i);

      // Should show model count if available
      const modelInfo = await modelStatus.locator('[class*="model"], [data-testid*="model"]').textContent();
      if (modelInfo) {
  ...
  test.describe("Action Queuing and Sync", () => {
    test.beforeEach(async ({ page }) => {
      ...
      await page.addInitScript(() => {
        ...
        localStorage.setItem('testCollections', JSON.stringify([
    ...
    test("should queue actions when offline", async ({ page, context }) => {
      ...
      const collectionButton = page.locator('[data-testid="save-to-collection"], button:has-text("Save"), button:has-text("Collection")').first();
      ...
      if (await collectionButton.isVisible()) {
        ...
        const queueIndicator = page.locator('[data-testid="queue-indicator"], [class*="queue"], [class*="pending"]');
    ...
    test("should sync queued actions when coming back online", async ({ page, context }) => {
      ...
      const syncIndicator = page.locator('[data-testid="sync-indicator"], [class*="sync"], [class*="syncing"]');
      await expect(syncIndicator).toBeVisible({ timeout: 5000 });

      // Should eventually complete
      await page.waitForFunction(() => {
        const syncIndicator = document.querySelector('[data-testid="sync-indicator"], [class*="sync"], [class*="syncing"]');
        return !syncIndicator || syncIndicator.getAttribute('class')?.includes('completed');
      }, { timeout: 10000 });
    });
    ...
    test("should handle sync errors gracefully", async ({ page, context }) => {
      ...
      const errorIndicator = page.locator('[data-testid="sync-error"], [class*="error"], [class*="failed"]');
      await expect(errorIndicator).toBeVisible({ timeout: 5000 });
    });
  });

  test.describe("Connectivity History Logging", () => {
    test("should log connectivity events in diagnostics", async ({ page, context }) => {
      // Open diagnostics drawer
      const diagnosticsButton = page.locator('[data-testid="diagnostics-button"], button:has-text("Diagnostics")');
      if (await diagnosticsButton.isVisible()) {
        ...
        const connectivityHistory = page.locator('[data-testid="connectivity-history"], [class*="connectivity-history"]');
        await expect(connectivityHistory).toBeVisible();

        // Should have event log
        const eventLog = connectivityHistory.locator('[data-testid="event-log"], [class*="event"]');
        await expect(eventLog).toBeVisible();
      }
    });

    test("should record offline/online transitions", async ({ page, context }) => {
      // Go offline
      await context.setOffline(true);
      await page.waitForTimeout(1000);
      ...
      if (await diagnosticsButton.isVisible()) {
        ...
        const events = page.locator('[data-testid="connectivity-event"], [class*="connectivity-event"]');
    ...
    test("should display connectivity statistics", async ({ page, context }) => {
      ...
      if (await diagnosticsButton.isVisible()) {
        ...
        const stats = page.locator('[data-testid="connectivity-stats"], [class*="stats"]');
    ...
    test("should allow exporting connectivity history", async ({ page, context }) => {
      // Open diagnostics
      const diagnosticsButton = page.locator('[data-testid="diagnostics-button"], button:has-text("Diagnostics")');
      if (await diagnosticsButton.isVisible()) {
        await safeClick(page, diagnosticsButton);
        ...
        const exportButton = page.locator('[data-testid="export-history"], button:has-text("Export")');
        ...
        if (await exportButton.isVisible()) {
          ...
          expect(download.suggestedFilename()).toMatch(/connectivity.*history/i);
        }
      }
    });
  });
  ...
  test.describe("Diagnostics Panel Features", () => {
    test("should show system diagnostics in offline mode", async ({ page, context }) => {
      // Go offline
      await context.setOffline(true);
      await page.waitForTimeout(1000);
      ...
      if (await diagnosticsButton.isVisible()) {
        ...
        const systemInfo = page.locator('[data-testid="system-info"], [class*="environment"], [class*="system"]');
    ...
    test("should maintain diagnostics data across connectivity changes", async ({ page, context }) => {
      // Go offline
      await context.setOffline(true);
      await page.waitForTimeout(1000);
      ...
      if (await diagnosticsButton.isVisible()) {
        ...
        const initialContent = await page.locator('[data-testid="diagnostics-content"]').textContent();
        ...
        const updatedContent = await page.locator('[data-testid="diagnostics-content"]').textContent();
  ...
  test.describe("PWA Service Worker Behavior", () => {
    ...
    test("should cache app shell for offline access", async ({ page, context }) => {
      ...
      const appLoaded = await page.isVisible('#root, [data-testid="app"], main');
    ...
    test("should handle network failures gracefully", async ({ page, context }) => {
      ...
      const apiResponse = await page.request.get('http://localhost:8000/api/health').catch(() => null);

      // Should handle gracefully without crashing
      expect(page.url()).not.toContain('about:blank');

      // App should remain functional
      const appContent = await page.isVisible('#root, [data-testid="app"], main');
  ...
  test.describe("Error Handling and Recovery", () => {
    test("should handle network interruptions gracefully", async ({ page, context }) => {
      ...
      if (await searchInput.isVisible()) {
        ...
        const errorMessage = page.locator('[data-testid="error-message"], [class*="error"], [class*="retry"]');
    ...
    test("should preserve user data during connectivity changes", async ({ page, context }) => {
  ...
  test.describe("Performance Under Offline Conditions", () => {
    test("should maintain acceptable performance offline", async ({ page, context }) => {
      // Go offline
      await context.setOffline(true);
      await page.waitForTimeout(1000);
      ...
      if (await settingsButton.isVisible()) {
        ...
        await page.waitForTimeout(500);
        const modal = page.locator('[data-testid="settings-modal"], [class*="modal"]').first();
    ...
    test("should handle large offline action queues efficiently", async ({ page, context }) => {
      // Go offline
      await context.setOffline(true);
      await page.waitForTimeout(1000);
      ...
      const appResponsive = await page.isVisible('#root, [data-testid="app"], main');
      ...
      const syncIndicator = page.locator('[data-testid="sync-indicator"], [class*="sync"]');
      const syncStarted = await syncIndicator.isVisible();

      if (syncStarted) {
        // Wait for sync to complete or timeout
        await page.waitForFunction(() => {
          const indicator = document.querySelector('[data-testid="sync-indicator"], [class*="sync"]');
          return !indicator || indicator.getAttribute('class')?.includes('completed');
        }, { timeout: 15000 });
      }
    });
  });
  ...
  test.describe("Cross-Browser Offline Compatibility", () => {
    test("should work consistently across different browsers", async ({ page, context }) => {
      // Go offline
      await context.setOffline(true);
      await page.waitForTimeout(1000);
