  ...
  test.describe("Search Error States", () => {
    test("handles empty and invalid search queries", async ({ page }) => {
      ...
      for (const testCase of testCases) {
        console.log(
          `Testing ${testCase.description}: "${testCase.query.substring(
            0,
            20
          )}..."`
        );
    ...
    test("handles no results scenarios helpfully", async ({ page }) => {
      ...
      // Should show helpful search tips instead of just "no results"
      ...
      // Updated expectations: new UX shows helpful suggestions instead of explicit "no results" text
      expect(noResultsState.hasHelpfulSuggestions).toBe(true);
      expect(noResultsState.hasSearchUI).toBe(true);
      // Note: hasNoResultsText may be false in new UX that prioritizes helpful suggestions
