...
async function primeApiMocks(page: Page) {
  ...
  await page.route("**/analytics*", (route) => fulfillJson(route, { events: [] }));
  await page.route("**/favorites?*", (route) => fulfillJson(route, { favorites: [] }));
  await page.route("**/saved", (route) => fulfillJson(route, { saved: [] }));
  await page.route("**/tags?*", (route) => fulfillJson(route, { tags: [] }));
  await page.route("**/library?*", (route) => fulfillJson(route, { items: [] }));
  await page.route("**/analytics/log", (route) => fulfillJson(route, { ok: true }));
...
async function performMockSearch(page: Page) {
  ...
  await expect(
    page.waitForResponse((response) =>
      response.url().includes("/search") && response.request().method() === "POST"
    )
  ).resolves;
