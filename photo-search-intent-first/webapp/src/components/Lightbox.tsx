...
export function Lightbox({
	...
	// Track edited path if needed by parent in the future (currently not used here)
	...
	function onWheel(e: ReactWheelEvent) {
		e.preventDefault();
		// Normalize wheel across devices; deltaY is typically inverted for zoom intents
		const raw = e.deltaY;
		const step = Math.max(0.05, Math.min(0.3, Math.abs(raw) / 300));
	...
	useEffect(() => {
		...
		async function load() {
			...
			try {
				...
				/* noop: best-effort metadata fetch */
		...
		<div className="fixed inset-0">
			...
			<div
				...
				onKeyDown={(e) => {
					...
					else if (e.key.toLowerCase() === "r") resetZoom();
				}}
			>
					...
					<div className="flex gap-2">
						...
						<button
							...
							className={`rounded px-2 py-1 ${
								showInfo ? "bg-blue-600 text-white" : "bg-white/20"
							}`}
						>
							Info
						</button>
						<button
							...
							className={`rounded px-2 py-1 ${
								showQuality ? "bg-green-600 text-white" : "bg-white/20"
							}`}
						>
							Quality
						</button>
						<button
							...
							className={`rounded px-2 py-1 ${
								showFacePanel ? "bg-purple-600 text-white" : "bg-white/20"
							}`}
						>
							Faces
						</button>
				...
				<div className="mt-3 flex flex-wrap gap-2 items-center">
					...
					<button
						type="button"
						aria-label="Reveal file in system file manager"
						onClick={onReveal}
