...
vi.mock('../api', () => ({
  ...
  apiResolveSmart: vi.fn(async () => ({ search_id: 's1', results: [{ path: '/a.jpg', score: 1 }] })),
  apiDeleteSmart: vi.fn(async () => ({ ok: true, deleted: 'Trip' })),
}))

describe('SmartCollections', () => {
  const baseProps = {
    dir: '/d', engine: 'local', topK: 24,
    smart: {},
    setSmart: vi.fn(), setResults: vi.fn(), setSearchId: vi.fn(), setNote: vi.fn(),
    query: 'beach', favOnly: false, tagFilter: 'sun,sea', useCaps: false, useOcr: false, hasText: false,
    camera: '', isoMin: '', isoMax: '', fMin: '', fMax: '', place: '', persons: [],
  }
  ...
  it('opens and deletes a smart collection', async () => {
    ...
    const props = { ...baseProps, smart: { Trip: { query: 'q' } }, setResults, setSearchId, setSmart, setNote }
    render(<SmartCollections {...props} />)
