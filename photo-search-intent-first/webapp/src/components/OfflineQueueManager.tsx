import { X, Trash2, RefreshCw, Play, Pause, AlertCircle } from "lucide-react";
...
export function OfflineQueueManager() {
  ...
  const removeSelected = useCallback(() => {
    ...
    offlineService.removeActionsFromQueue(idsToRemove)
      .then(() => {
        // Update local state
        const newQueue = queueItems.filter(item => !selectedItems.has(item.id));
  ...
  const getItemDescription = useCallback((item: QueueItem) => {
    switch (item.type) {
      case "search":
        return `Search: ${item.payload.query}`;
      case "collection":
        return `Update collection: ${item.payload.name}`;
      case "delete_collection":
        return `Delete collection: ${item.payload.name}`;
      case "tag":
        return `Tag ${item.payload.paths.length} items`;
      case "set_tags":
        return `Set tags on ${item.payload.path}`;
      case "delete":
        return `Delete ${item.payload.paths.length} items`;
      case "favorite":
        return `Favorite: ${item.payload.path}`;
      case "saved_search":
        return `Save search: ${item.payload.name}`;
      case "delete_saved_search":
        return `Delete saved search: ${item.payload.name}`;
      case "batch_add_to_collection":
        return `Add ${item.payload.paths.length} items to ${item.payload.collectionName}`;
      case "index":
        return `Build index for ${item.payload.dir}`;
      case "build_metadata":
        return `Build metadata for ${item.payload.dir}`;
      case "build_ocr":
        return `Build OCR for ${item.payload.dir}`;
      case "build_faces":
        return `Build faces for ${item.payload.dir}`;
      case "smart_collection":
        return `Update smart collection: ${item.payload.name}`;
      case "delete_smart_collection":
        return `Delete smart collection: ${item.payload.name}`;
      default:
          ...
          >
              ...
              {queueItems.length === 0 ? (
                <div className="text-center py-12">
                  ...
                  <p className="mt-1 text-gray-500 dark:text-gray-400">
                    Your offline actions will appear here when you're disconnected
                  </p>
                </div>
              ) : (
            ...
            <div className="p-4 border-t dark:border-gray-700 flex justify-between items-center">
              <div className="text-sm text-gray-500 dark:text-gray-400">
                {selectedItems.size > 0 ? (
                  <span>{selectedItems.size} of {queueItems.length} selected</span>
                ) : (
