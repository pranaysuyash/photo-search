        ...
        <div className="bg-white rounded-lg p-4 w-full max-w-md" role="dialog" aria-modal="true">
          ...
          <form
            onSubmit={(e) => {
              ...
              const stripGps = (form.elements.namedItem('strip_gps') as HTMLInputElement)?.checked || false;
              const keepCopy = (form.elements.namedItem('keep_copy') as HTMLInputElement)?.checked || false;
              ...
              (async () => {
                try {
                  ...
                  let resizeLong: number | undefined = undefined
                  let quality: number | undefined = undefined
                  if (preset === 'custom') {
                    const cl = parseInt((form.elements.namedItem('custom_long') as HTMLInputElement)?.value || '')
                    const cq = parseInt((form.elements.namedItem('custom_quality') as HTMLInputElement)?.value || '')
                    resizeLong = isNaN(cl) ? undefined : cl
                    quality = isNaN(cq) ? undefined : cq
                  }
          ...
          >
            <div className="grid gap-3">
              <div>
                <label className="block text-sm text-gray-600 mb-1" htmlFor="dest-input">Destination</label>
                <input id="dest-input" name="dest" className="w-full border rounded px-2 py-1" placeholder="/absolute/path" />
              </div>
              <fieldset>
                ...
                <div className="mt-2 grid grid-cols-2 gap-2 text-sm">
                  <label className="flex items-center gap-2">
                    ...
                    <input type="number" name="custom_long" className="border rounded px-2 py-1 w-28" placeholder="1600" />
                  </label>
                  <label className="flex items-center gap-2">
                    <span className="w-28">Quality (1–100)</span>
                    <input type="number" name="custom_quality" min={1} max={100} className="border rounded px-2 py-1 w-28" placeholder="85" />
                  </label>
                </div>
              </fieldset>
              <div>
                ...
                <label className="flex items-center gap-2 text-sm"><input type="checkbox" name="strip_gps" /> Strip GPS only</label>
                <label className="flex items-center gap-2 text-sm"><input type="checkbox" name="keep_copy" /> Keep copyright only</label>
              </div>
              <div>
            ...
            <div className="mt-4 flex justify-end gap-2">
              <button type="button" className="px-3 py-1 rounded border" onClick={onClose} aria-label="Cancel export">Cancel</button>
              <button type="submit" className="px-3 py-1 rounded bg-blue-600 text-white" aria-label="Export selected photos">Export</button>
            </div>
          </form>
        </div>
      </FocusTrap>
