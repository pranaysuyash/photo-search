...
export function OfflineStatusIndicator() {
  ...
  const getStatusColor = (): string => {
    if (!status.isOnline) return "bg-red-100 text-red-800 dark:bg-red-900/20 dark:text-red-400";
    if (status.queueStats.failedActions > 0) return "bg-yellow-100 text-yellow-800 dark:bg-yellow-900/20 dark:text-yellow-400";
    if (status.queueStats.totalActions > 0) return "bg-blue-100 text-blue-800 dark:bg-blue-900/20 dark:text-blue-400";
  ...
  const getStatusText = (): string => {
    ...
    if (status.queueStats.failedActions > 0) return `${status.queueStats.failedActions} failed`;
    if (status.queueStats.totalActions > 0) return `${status.queueStats.totalActions} queued`;
    return "Online";
  };
  ...
  return (
    <div className="relative">
      ...
      <div className={`flex items-center gap-2 px-3 py-2 rounded-lg text-sm font-medium ${getStatusColor()} cursor-pointer transition-colors`}
          ...
          <div className="p-4">
            ...
            <div className="space-y-3">
              ...
              <div className="flex items-center justify-between">
                ...
                <span className={`text-sm font-medium ${getNetworkQualityColor(status.networkQuality)}`}>
                  {getNetworkQualityText(status.networkQuality)}
                </span>
              </div>
