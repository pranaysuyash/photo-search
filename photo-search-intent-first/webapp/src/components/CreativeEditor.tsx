...
export function CreativeEditor({
	...
	const applyFilter = useCallback((filter: CreativeFilter) => {
	...
	const updateAdjustment = useCallback((key: keyof typeof adjustments, value: number) => {
	...
	const applyEdits = async () => {
		...
		try {
			...
			if (edits.length > 0) {
				const result = await analysisService.current.applyCreativeEdits(imagePath, edits);
			...
			toast({
				...
				description: "An error occurred while applying your edits. Please try again.",
				variant: "destructive"
			});
		} finally {
	...
	const getFilterStyle = () => {
		...
		if (adjustments.brightness) cssFilters.push(`brightness(${1 + adjustments.brightness})`);
		if (adjustments.contrast) cssFilters.push(`contrast(${1 + adjustments.contrast})`);
		if (adjustments.saturation) cssFilters.push(`saturate(${1 + adjustments.saturation})`);
		if (adjustments.hue) cssFilters.push(`hue-rotate(${adjustments.hue * 360}deg)`);
		if (adjustments.sharpness) cssFilters.push(`contrast(${1 + adjustments.sharpness * 0.5})`);
		if (adjustments.warmth) cssFilters.push(`sepia(${Math.abs(adjustments.warmth * 0.3)})`);
		if (adjustments.vignette) cssFilters.push(`radial-gradient(circle, transparent 50%, rgba(0,0,0,${adjustments.vignette}) 100%)`);
		...
		<div className="creative-editor">
			...
			<div className="editor-header">
				...
				<div className="editor-actions">
					<Button
						onClick={applyEdits}
						disabled={isProcessing || (!selectedFilter && Object.values(adjustments).every(v => v === 0) && rotation === 0)}
					>
						{isProcessing ? (
							<>
			...
			<div className="editor-content">
				...
				<div className="image-preview">
					<div className="image-container" style={{ transform: `scale(${zoom}) rotate(${rotation}deg)` }}>
						<img
							ref={imageRef}
							...
							<CardContent className="space-y-2">
								<div className="quality-metrics">
									<div className="metric">
										<span>Quality</span>
										<Progress value={analysis.quality.overall * 100} className="h-2" />
									</div>
									<div className="metric">
										<span>Sharpness</span>
										<Progress value={analysis.quality.sharpness * 100} className="h-2" />
									</div>
									<div className="metric">
										<span>Composition</span>
										<Progress value={analysis.composition.overallScore * 100} className="h-2" />
									</div>
								</div>
								...
								<CardContent className="space-y-4">
									{Object.entries(adjustments).map(([key, value]) => (
										<div key={key} className="adjustment-control">
											...
											<Slider
												value={[value]}
												onValueChange={([newValue]) => updateAdjustment(key as keyof typeof adjustments, newValue)}
												min={-1}
												max={1}
												step={0.01}
												className="w-full"
											...
											<div className="section">
												...
												<div className="metrics">
													<div className="metric">
														...
														<Progress value={analysis.composition.ruleOfThirds * 100} />
													</div>
													<div className="metric">
														<span>Balance</span>
														<Progress value={analysis.composition.balance * 100} />
													</div>
													<div className="metric">
														<span>Depth</span>
														<Progress value={analysis.composition.depth * 100} />
													</div>
												</div>
											</div>
