...
import { 
  Sparkles, Tag, Calendar, MapPin, User, 
  Camera, FileText, Eye, Palette, Image,
  TrendingUp, Heart, Clock
} from 'lucide-react'

export interface MatchReason {
  type: 'caption' | 'tag' | 'location' | 'person' | 'date' | 'color' | 
        'object' | 'scene' | 'emotion' | 'activity' | 'similarity' | 'metadata'
...
export function SearchExplainability({ 
  ...
  // Sort by confidence
  const sortedReasons = [...reasons].sort((a, b) => b.confidence - a.confidence)
    ...
    <div className="p-3 bg-blue-50 dark:bg-blue-900/20 rounded-lg">
      ...
      <div className="space-y-1">
        {topReasons.map((reason, idx) => (
          ...
          >
            <div className={`mt-0.5 p-1 rounded ${getConfidenceColor(reason.confidence)}`}>
              {getReasonIcon(reason.type)}
            </div>
            <div className="flex-1">
              ...
              <span className="ml-1 text-gray-800 dark:text-gray-200">
                {reason.highlight ? (
                  <HighlightedText text={reason.detail} highlight={reason.highlight} />
                ) : (
                  reason.detail
                )}
              </span>
              {reason.confidence < 0.5 && (
                <span className="ml-1 text-xs text-gray-400">(possible match)</span>
              )}
            </div>
          </div>
        ))}
    ...
    <>
      {parts.map((part, idx) => 
        part.toLowerCase() === highlight.toLowerCase() ? (
          <mark key={idx} className="bg-yellow-200 dark:bg-yellow-800 px-0.5 rounded">
            {part}
          </mark>
        ) : (
...
export class MatchAnalyzer {
  analyzeMatch(photo: any, query: string): MatchReason[] {
    ...
    if (photo.caption) {
      ...
      const matchedWords = queryWords.filter(word => captionLower.includes(word))
    ...
    // Check location
    if (photo.location && queryWords.some(word => photo.location.toLowerCase().includes(word))) {
      reasons.push({
        type: 'location',
        confidence: 0.85,
    ...
    if (queryWords.some(word => ['today', 'yesterday', 'week', 'month', 'year'].includes(word))) {
      if (photo.date) {
        reasons.push({
          type: 'date',
          confidence: 0.8,
    ...
    if (photo.scene && queryWords.some(word => photo.scene.toLowerCase().includes(word))) {
      reasons.push({
        type: 'scene',
        confidence: 0.7,
        detail: photo.scene
    ...
    const emotions = ['happy', 'sad', 'smiling', 'laughing', 'crying', 'angry', 'surprised']
    const activities = ['running', 'walking', 'playing', 'dancing', 'eating', 'sleeping', 'working']
    
    const matchedEmotions = queryWords.filter(word => emotions.includes(word))
    ...
    const matchedActivities = queryWords.filter(word => activities.includes(word))
