...
import { Camera, Aperture, Zap, Calendar, MapPin, Star } from 'lucide-react';
...
export function QuickFilters({
  ...
  const hasActiveFilters = camera || isoMin || isoMax || fMin || fMax || dateFrom || dateTo;
  ...
  const categories = [
          ...
          <div className="flex flex-wrap gap-1">
            <button
              ...
              className={`px-2 py-1 text-xs rounded border ${
                !camera ? 'bg-blue-600 text-white border-blue-600' : 'bg-white border-gray-300 hover:bg-gray-50'
              }`}
            >
              Any Camera
            </button>
    ...
    },
    {
      ...
      content: (
        <div className="space-y-2">
          <div className="text-xs text-gray-600 mb-1">Quick select f-stop range:</div>
          <div className="flex flex-wrap gap-1">
            <button
              type="button"
              onClick={() => {
              ...
              className={`px-2 py-1 text-xs rounded border ${
                !fMin && !fMax ? 'bg-blue-600 text-white border-blue-600' : 'bg-white border-gray-300 hover:bg-gray-50'
              }`}
            >
              Any
            </button>
    ...
    },
    {
      ...
      content: (
        <div className="space-y-2">
          <div className="text-xs text-gray-600 mb-1">Quick select ISO range:</div>
          <div className="flex flex-wrap gap-1">
            <button
              type="button"
              onClick={() => {
              ...
              className={`px-2 py-1 text-xs rounded border ${
                !isoMin && !isoMax ? 'bg-blue-600 text-white border-blue-600' : 'bg-white border-gray-300 hover:bg-gray-50'
              }`}
            >
              Any
            </button>
            <button
              ...
              className={`px-2 py-1 text-xs rounded border ${
                !isoMin && isoMax === '800' ? 'bg-blue-600 text-white border-blue-600' : 'bg-white border-gray-300 hover:bg-gray-50'
              }`}
            >
              Low (≤800)
            </button>
            <button
              ...
              className={`px-2 py-1 text-xs rounded border ${
                isoMin === '800' && !isoMax ? 'bg-blue-600 text-white border-blue-600' : 'bg-white border-gray-300 hover:bg-gray-50'
              }`}
            >
              High (≥800)
            </button>
          ...
          {yearRange && (
            <div className="flex flex-wrap gap-1 mb-2">
              <button
                ...
                className={`px-2 py-1 text-xs rounded border ${
                  !dateFrom && !dateTo ? 'bg-blue-600 text-white border-blue-600' : 'bg-white border-gray-300 hover:bg-gray-50'
                }`}
              >
                Any Year
              </button>
              {Array.from({ length: Math.min(6, yearRange[0] - yearRange[1] + 1) }, (_, i) => yearRange[0] - i).map(year => (
                <button
                  key={year}
                  type="button"
                  onClick={() => {
    ...
    <div className="bg-white border rounded-lg">
      <div className="flex items-center justify-between p-3 border-b">
        <div className="flex items-center gap-2">
          ...
          {hasActiveFilters && (
            <span className="text-xs bg-blue-100 text-blue-800 px-2 py-0.5 rounded">
              {[camera, isoMin, isoMax, fMin, fMax, dateFrom, dateTo].filter(Boolean).length} active
            </span>
          )}
        </div>
        <div className="flex items-center gap-2">
