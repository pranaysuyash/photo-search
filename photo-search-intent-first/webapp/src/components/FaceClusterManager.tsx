...
import { apiFacesClusters, apiGetFacePhotos, apiMergeFaceClusters, apiSplitFaceCluster, thumbFaceUrl } from '../api'
...
export function FaceClusterManager({ currentDir, provider }: FaceClusterManagerProps) {
  ...
  const splitCluster = async () => {
    ...
    try {
      ...
      setSelectedPhotos(new Set())
      await loadClusters()
      alert(`Successfully split ${photoPaths.length} photos into a new cluster`)
    } catch (err) {
                ...
                >
                  <div className="flex items-center gap-3">
                    ...
                    <div className="flex -space-x-2">
                      {cluster.examples.slice(0, 3).map(([path, embIndex], idx) => (
                        <img
                          key={idx}
                          src={thumbFaceUrl(currentDir, provider, path, embIndex, 48)}
                          alt="Face example"
          ...
          {selectedCluster ? (
            <div className="space-y-4">
              <div className="bg-gray-50 p-4 rounded">
                <div className="font-medium">{selectedCluster.name || `Cluster ${selectedCluster.id}`}</div>
                <div className="text-sm text-gray-600">
                  {selectedCluster.size} total faces • {clusterPhotos.length} photos
                </div>
