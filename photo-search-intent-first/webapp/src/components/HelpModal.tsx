...
interface HelpModalProps {
  isOpen: boolean;
  onClose: () => void;
  initialSection?: 'getting-started' | 'features' | 'shortcuts' | 'troubleshooting';
}
...
export function HelpModal({ isOpen, onClose, initialSection = 'getting-started' }: HelpModalProps) {
  ...
  const sections = [
        ...
        <div className="space-y-6">
          <div>
            ...
            <p className="text-gray-600 dark:text-gray-300 text-sm">
              Tip: Press <span className="font-mono">?</span> anytime to open this help.
            </p>
          </div>
    ...
    },
    {
      ...
      content: (
        <div className="py-6">
          <h3 className="text-lg font-medium text-gray-900 dark:text-white mb-2">Most Useful</h3>
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-2">
            {shortcuts.slice(0, 12).map((s, i) => (
              <div key={i} className="flex items-center justify-between py-2 px-3 rounded-lg bg-gray-50 dark:bg-gray-800">
                <span className="text-sm text-gray-800 dark:text-gray-200">{s.description}</span>
                <kbd className="px-2 py-1 text-xs font-mono bg-white dark:bg-gray-700 text-gray-700 dark:text-gray-300 rounded border border-gray-300 dark:border-gray-600">{s.keys}</kbd>
              </div>
            ))}
          </div>
