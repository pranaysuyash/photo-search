...
export interface SimilaritySearchOptions {
	threshold: number;
	maxResults: number;
	searchMode: 'visual' | 'color' | 'composition' | 'style' | 'objects' | 'mixed';
...
export function VisualSimilaritySearch({
	...
	const [selectedImage, setSelectedImage] = useState<string>(queryImagePath || '');
	...
	const [searchResults, setSearchResults] = useState<SimilarityResult | null>(null);
	const [imageAnalysis, setImageAnalysis] = useState<VisualAnalysisResult | null>(null);
	...
	const getSimilarityScoreBadge = (score: number) => {
		if (score >= 0.9) return { variant: 'default' as const, label: 'Very Similar' };
		if (score >= 0.8) return { variant: 'secondary' as const, label: 'Similar' };
		if (score >= 0.7) return { variant: 'outline' as const, label: 'Somewhat Similar' };
						...
						{selectedImage ? (
							<div className="query-image-container">
										...
										<div className="image-analysis-summary">
											<div className="analysis-grid">
												<div className="analysis-item">
													...
													<Progress value={imageAnalysis.quality.overall * 100} className="h-2" />
												</div>
												<div className="analysis-item">
													<span className="text-xs">Composition</span>
													<Progress value={imageAnalysis.composition.overallScore * 100} className="h-2" />
												</div>
											</div>
											{imageAnalysis.colors.length > 0 && (
												<div className="color-palette">
													{imageAnalysis.colors.slice(0, 6).map((color, index) => (
														<div
															key={index}
								...
								<div className="query-info">
									<h4 className="font-medium">{selectedImage.split('/').pop()}</h4>
									{imageAnalysis && (
					...
					<CardContent className="space-y-4">
						<div className="search-mode">
							...
							<Select
								...
								onValueChange={(value: any) => setSearchOptions(prev => ({
									...prev,
									searchMode: value
								}))}
							>
							...
							<div className="advanced-options space-y-3">
								...
								{searchOptions.searchMode === 'mixed' && (
									<div className="weight-controls space-y-2">
										<label className="text-sm font-medium">Analysis Weights</label>
										{[
											{ key: 'colorWeight', label: 'Color', value: searchOptions.colorWeight },
											{ key: 'compositionWeight', label: 'Composition', value: searchOptions.compositionWeight },
											{ key: 'styleWeight', label: 'Style', value: searchOptions.styleWeight },
											{ key: 'objectWeight', label: 'Objects', value: searchOptions.objectWeight }
										].map(({ key, label, value }) => (
											<div key={key} className="weight-control">
												...
												<Slider
													value={[value]}
													onValueChange={([newValue]) => setSearchOptions(prev => ({
														...prev,
														[key]: newValue
													}))}
													min={0}
								...
								<div className="similar-images-grid">
									{searchResults.similarPhotos.map((result, index) => {
										...
										return (
											<div
												key={index}
												className={`similar-image-item ${selectedResults.has(result.path) ? 'selected' : ''}`}
											...
											>
												...
												<div className="image-info">
													<div className="similarity-score">
														<span className={`score ${getSimilarityScoreColor(result.score)}`}>
