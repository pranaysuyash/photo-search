  ...
  useEffect(() => {
    ...
    const updateStats = () => {
      ...
      const newStats: PerformanceStats = {
        ...
        networkRequests: (performance as any).getEntriesByType?.('navigation')?.length || 0,
      };
    ...
    <div className="fixed bottom-4 left-4 bg-black/90 text-white p-3 rounded-lg text-xs font-mono z-[9999]">
      ...
      <div className="space-y-1">
        <div>
          ...
          <span className={stats.memoryUsage > 100 ? 'text-red-400' : 'text-green-400'}>
        ...
        <div>
          ...
          <span className="text-green-400">
            {stats.imageCache.size} items ({(stats.imageCache.totalSize / 1024 / 1024).toFixed(1)}MB)
          </span>
        </div>
        
        <div>
          <span className="text-blue-400">Cache Hit Rate:</span>{' '}
          <span className={stats.imageCache.hitRate > 0.8 ? 'text-green-400' : 'text-yellow-400'}>
