...
import { Search, FolderOpen, Settings, BookOpen, Zap, Layers, Filter } from 'lucide-react';
...
import { useEnhancedSearchContext, useWorkspaceStore } from '../hooks/useStores';
...
export default function MultiFolderSearchDemo() {
  ...
  // Get query analysis
  const queryAnalysis = state.query ? AdvancedQueryParser.parse(state.query) : null;
      ...
      <div className="max-w-7xl mx-auto space-y-6">
        ...
        <div className="text-center space-y-2">
          ...
          <p className="text-gray-600">
            Enhanced search capabilities across multiple photo folders with advanced query parsing
          </p>
        </div>
              ...
              <Card>
                <CardContent className="p-6">
                  <div className="space-y-3">
                    <div className="flex items-center justify-between">
                      <span className="text-sm font-medium">Searching folders...</span>
                      <span className="text-sm text-gray-600">{Math.round(state.searchProgress)}%</span>
                    </div>
            ...
            {queryAnalysis ? (
              <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
                        ...
                        {queryAnalysis.tokens.map((token, index) => (
                          <span
                            ...
                            className={`px-2 py-1 rounded text-xs ${
                              token.type === 'operator' ? 'bg-blue-100 text-blue-800' :
                              token.type === 'exclude' ? 'bg-red-100 text-red-800' :
                              token.type === 'keyword' ? 'bg-gray-100 text-gray-800' :
                ...
                <Card>
                  ...
                  <CardContent className="space-y-4">
                    {Object.entries(queryAnalysis.context).map(([key, value]) => (
                      value && (
                        <div key={key} className="flex items-center justify-between">
                          <span className="capitalize font-medium">{key.replace('Context', '')}</span>
              ...
              <Card>
                <CardContent className="p-12 text-center">
                  ...
                  <h3 className="text-lg font-medium mb-2">No Query to Analyze</h3>
                  <p className="text-gray-500">Enter a search query to see detailed analysis</p>
                </CardContent>
              </Card>
            )}
          </TabsContent>
                  ...
                  <CardContent className="p-4">
                    <div className="space-y-3">
                      ...
                      <div className="text-sm text-gray-600 space-y-1">
                        <div><strong>Query:</strong> {config.query}</div>
                        <div><strong>Scope:</strong> {config.scope.type}</div>
                        <div><strong>Folders:</strong> {config.scope.selectedFolders.length}</div>
                        <div><strong>Last Used:</strong> {new Date(config.lastUsed).toLocaleDateString()}</div>
                      </div>
                    </div>
                  </CardContent>
                </Card>
            ...
            {savedConfigs.length === 0 && (
              <Card>
                <CardContent className="p-12 text-center">
                  ...
                  <p className="text-gray-500">Save your search configurations for quick access</p>
                </CardContent>
              </Card>
            )}
          </TabsContent>
