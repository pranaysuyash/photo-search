...
import { 
  Search, Grid3x3, Map, Users, Folder, Star, Calendar, Heart,
  Settings, HardDrive, ChevronLeft, ChevronRight, Upload, Moon, Sun,
  Filter, Plus, X, Check, Download, Share2, Trash2, Info, ZoomIn,
  Image, Film, Clock, MapPin, Tag, ArrowLeft, MoreHorizontal,
  FolderOpen, Cloud, Sparkles, Edit, RotateCw, Crop, Maximize,
  Type, Globe, Navigation, Camera, Save, BookOpen, Brain, Copy,
  RefreshCw, AlertCircle, CheckCircle, Loader2, Eye, Database,
  Activity, Zap, FolderPlus, UserPlus, Tags, MessageSquare, Cpu
} from 'lucide-react';
import { PhotoVaultAPI, initializeAPI, getAPI } from '../services/PhotoVaultAPI';
...
export const ComprehensiveUI: React.FC<ComprehensiveUIProps> = ({ 
  ...
  const { dir, provider, hfToken, openaiKey } = store;
  ...
  const [smartCollections, setSmartCollections] = useState<any[]>([]);
  const [selectedCollection, setSelectedCollection] = useState<string | null>(null);

  // People state
  const [faceClusters, setFaceClusters] = useState<any[]>([]);
  const [namedPeople, setNamedPeople] = useState<Map<string, string>>(new Map());
  ...
  const handleSearch = async () => {
    ...
    try {
      ...
      showNotification('success', `Found ${results.results?.length || 0} results`);
    } catch (err: any) {
  ...
  // Face naming
  const nameFaceCluster = async (clusterId: string, name: string) => {
    if (!api.current) return;
    
    try {
      await api.current.nameFaceCluster(clusterId, name);
      setNamedPeople(prev => new Map(prev).set(clusterId, name));
  ...
  // Image editing
  const editImage = async (path: string, operations: any) => {
    if (!api.current) return;
    
    try {
      const result = await api.current.editImage(path, operations);
  ...
  const findDuplicates = async () => {
    ...
    try {
      ...
      setLookalikes(dupes.lookalikes || []);
      setActiveView('duplicates');
      showNotification('info', `Found ${dupes.lookalikes?.length || 0} potential duplicates`);
    } catch (err) {
  ...
  const autoTag = async () => {
    ...
    setLoading(true);
    try {
      const result = await api.current.autoTag();
      ...
      <div className="w-64 bg-gray-900 text-white p-4 overflow-y-auto">
        ...
        <div className="border-t border-gray-800 pt-4 mb-6">
          ...
          <div className="space-y-1">
            <button onClick={() => buildIndex('search')} className="w-full text-left p-1 text-sm hover:bg-gray-800 rounded">
              <Database className="inline w-3 h-3 mr-1" /> Search Index
            </button>
            <button onClick={() => buildIndex('fast')} className="w-full text-left p-1 text-sm hover:bg-gray-800 rounded">
              <Zap className="inline w-3 h-3 mr-1" /> Fast Index
            </button>
            <button onClick={() => buildIndex('faces')} className="w-full text-left p-1 text-sm hover:bg-gray-800 rounded">
              <Users className="inline w-3 h-3 mr-1" /> Face Detection
            </button>
            <button onClick={() => buildIndex('ocr')} className="w-full text-left p-1 text-sm hover:bg-gray-800 rounded">
              <Type className="inline w-3 h-3 mr-1" /> OCR Text
            </button>
            <button onClick={() => buildIndex('metadata')} className="w-full text-left p-1 text-sm hover:bg-gray-800 rounded">
              <Info className="inline w-3 h-3 mr-1" /> Metadata
            </button>
            <button onClick={() => buildIndex('captions')} className="w-full text-left p-1 text-sm hover:bg-gray-800 rounded">
              <MessageSquare className="inline w-3 h-3 mr-1" /> AI Captions
            </button>
            <button onClick={() => buildIndex('trips')} className="w-full text-left p-1 text-sm hover:bg-gray-800 rounded">
        ...
        <div className="border-t border-gray-800 pt-4">
          ...
          <div className="space-y-1">
            <button onClick={findDuplicates} className="w-full text-left p-1 text-sm hover:bg-gray-800 rounded">
              <Copy className="inline w-3 h-3 mr-1" /> Find Duplicates
            </button>
            <button onClick={autoTag} className="w-full text-left p-1 text-sm hover:bg-gray-800 rounded">
              <Tags className="inline w-3 h-3 mr-1" /> Auto-Tag
            </button>
            <button onClick={runDiagnostics} className="w-full text-left p-1 text-sm hover:bg-gray-800 rounded">
              <Activity className="inline w-3 h-3 mr-1" /> Diagnostics
            </button>
            <button onClick={exportSelected} className="w-full text-left p-1 text-sm hover:bg-gray-800 rounded">
              <Download className="inline w-3 h-3 mr-1" /> Export Selected
            </button>
            <button onClick={deleteSelected} className="w-full text-left p-1 text-sm hover:bg-gray-800 rounded text-red-400">
