...
const PerformanceDashboard: React.FC<PerformanceDashboardProps> = ({
	...
	const { metrics, alerts, generateReport, clearAlerts } =
		usePerformanceMonitor();
				...
				<div className="p-4 space-y-4 max-h-80 overflow-y-auto">
					...
					<div className="space-y-2">
						<div className="flex items-center justify-between">
							...
							<span
								className={`text-sm font-mono ${getMemoryStatusColor(metrics.memoryUsage.usagePercent)}`}
							>
					...
					<div className="space-y-2">
						...
						<div className="text-xs text-gray-600 space-y-1">
							<div className="flex justify-between">
								...
								<span className="font-mono">
									{metrics.library.totalItems.toLocaleString()}
								</span>
							</div>
							<div className="flex justify-between">
								<span>Loaded Items:</span>
								<span className="font-mono">
									{metrics.library.loadedItems.toLocaleString()}
								</span>
							</div>
							<div className="flex justify-between">
								<span>Avg Load Time:</span>
								<span className="font-mono">
									{formatTime(metrics.library.averageLoadTime)}
								</span>
							</div>
						</div>
					</div>
					...
					<div className="space-y-2">
						...
						<div className="text-xs text-gray-600 space-y-1">
							<div className="flex justify-between">
								...
								<span
									className={`font-mono ${metrics.rendering.virtualizationEnabled ? "text-green-600" : "text-gray-600"}`}
								>
									{metrics.rendering.virtualizationEnabled
										? "Enabled"
										: "Disabled"}
								</span>
							</div>
							<div className="flex justify-between">
								...
								<span className="font-mono">
									{metrics.rendering.visibleItems}
								</span>
							</div>
							<div className="flex justify-between">
								<span>Grid Render Time:</span>
								<span className="font-mono">
									{formatTime(metrics.rendering.gridRenderTime)}
								</span>
							</div>
						</div>
					</div>
						...
						<div className="text-xs text-blue-800 space-y-1">
							...
							<ul className="list-disc list-inside space-y-1 text-blue-700">
								{metrics.memoryUsage.usagePercent > 75 && (
									<li>
										High memory usage detected. Consider closing other tabs.
									</li>
								)}
								{metrics.library.totalItems > 10000 &&
									!metrics.rendering.virtualizationEnabled && (
										<li>
											Large library detected. Enable virtualized grid for better
											performance.
										</li>
									)}
