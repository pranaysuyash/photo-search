...
export function PullToRefresh({
  ...
  }, [disabled, pullState.canRefresh, pullState.isRefreshing])

  // Calculate transform and styles
  const pullProgress = Math.min(pullState.pullDistance / threshold, 1)
  const contentTransform = pullState.isPulling ? `translateY(${pullState.pullDistance}px)` : ''
      ...
      >
        <div 
          ...
          style={{
            ...
            transition: pullState.isRefreshing ? 'transform 0.5s linear' : 'transform 0.2s ease-out'
          }}
        >
          <RefreshCw 
            className={`w-6 h-6 ${
