...
const ProUI: React.FC<ProUIProps> = ({
	darkMode = false,
	onDarkModeToggle,
}) => {
	// State Management
	...
	const loadLibrary = useCallback(async () => {
		...
		try {
			...
			const result = await apiLibrary(libraryPath, "local", 120, 0);
			...
			const transformedPhotos: Photo[] = result.paths.map(
				(imgPath: string, index: number) => ({
					id: `photo-${index}`,
					path: imgPath,
					thumbnail: thumbUrl(libraryPath!, "local", imgPath), // Use the thumbUrl function for thumbnails
	...
	}, [libraryPath, loadLibrary]);

	const handleSearch = async (e: React.FormEvent) => {
		e.preventDefault();
		if (!searchQuery.trim() || !libraryPath) return;

		setLoading(true);
		try {
			const results = await apiSearch(libraryPath, searchQuery, "local", 20);
			if (results?.results) {
				const searchPhotos = results.results.map((r: any, index: number) => ({
					id: `search-${index}`,
					path: r.image,
					thumbnail: thumbUrl(libraryPath!, "local", r.image),
	...
	const displayPhotos =
		selectedView === "memories" ? photos.filter((p) => p.favorite) : photos;
	...
	return (
		...
		>
			{/* Sidebar */}
			<div
				className={`sidebar ${sidebarCollapsed ? "w-16" : ""}`}
								...
								{(["small", "medium", "large"] as const).map((size) => (
									<button
										...
										style={{
											padding: "6px 12px",
											background:
												gridSize === size ? "var(--bg-primary)" : "transparent",
											border: "none",
