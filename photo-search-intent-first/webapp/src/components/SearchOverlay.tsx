...
export const SearchOverlay: React.FC<SearchOverlayProps> = ({
  ...
  const q = useMemo(() => searchText ?? ctx.state.query, [searchText, ctx.state.query]);
  const setQ = (t: string) => (setSearchText ? setSearchText(t) : ctx.actions.setQuery(t));
  ...
  return (
    <div className="fixed inset-0 z-50 bg-black/40 flex items-start md:items-center justify-center p-3 md:p-6" role="dialog" aria-modal="true" aria-label="Search Photos">
      <FocusTrap onEscape={onClose}>
        <div className="w-full max-w-3xl rounded-xl shadow-2xl bg-white dark:bg-gray-900 border border-gray-200 dark:border-gray-800">
          <div className="flex items-center justify-between px-4 py-3 md:px-6 md:py-4 border-b border-gray-200 dark:border-gray-800">
            <div className="text-base md:text-lg font-semibold text-gray-800 dark:text-gray-100">Search Photos</div>
            <button type="button" className="px-2 py-1 text-sm rounded-md border border-gray-300 dark:border-gray-700 hover:bg-gray-50 dark:hover:bg-gray-800" onClick={onClose} aria-label="Close search">
              Close
            </button>
          </div>
          ...
          <div className="px-4 md:px-6 py-4 md:py-6">
            ...
            <AppliedTokens
              query={q}
              onRemove={(tok) => {
                const next = (q || "").replace(tok, "").replace(/\s{2,}/g, " ").trim();
                setQ(next);
                doSearch(next);
              }}
              onClear={() => {
                const next = (q || "").replace(/(?:[a-z_]+:\"[^\"]+\"|[a-z_]+:[^\s)]+)/gi, "").replace(/\s{2,}/g, " ").trim();
                setQ(next);
            ...
            <div className="mt-3 flex flex-wrap gap-2" aria-label="Quick time filters">
              {["All", "Today", "This Week", "This Month"].map((label) => (
                <button
                  ...
                  onClick={() => {
                    ...
                    // Compute token
                    let tok = "";
                    if (label === "Today") tok = `mtime:>=${Math.floor(startOfDay.getTime() / 1000)}`;
                    else if (label === "This Week") tok = `mtime:>=${Math.floor(startOfWeek.getTime() / 1000)}`;
                    else if (label === "This Month") tok = `mtime:>=${Math.floor(startOfMonth.getTime() / 1000)}`;

                    // Remove existing mtime tokens to avoid duplication
                    const base = (q || "").replace(/\bmtime:[^\s)]+/gi, "").replace(/\s{2,}/g, " ").trim();
                    const next = label === "All" ? base : (base ? `${base} ${tok}` : tok);
                    setQ(next);
                    doSearch(next);
                    onClose();
                  }}
            ...
            <div className="mt-4 flex items-center gap-2">
              ...
              <div className="ml-auto text-xs text-gray-500">Tip: press / to open search</div>
            </div>
            ...
            {showAdvanced && (
              ...
              >
                ...
                <div className="mt-1">Fields: camera:, place:, tag:, rating:, person:, has_text:, filetype:</div>
                <div className="mt-1">Numeric: iso:, fnumber:, width:, height:, mtime:, brightness:, sharpness:, exposure:, focal:, duration: (supports &gt;=, &lt;=, &gt;, &lt;, =)</div>
                ...
                <div className="mt-3 text-xs text-gray-500">Tip: Start typing to see people, tags, and camera suggestions.</div>
              </div>
            )}
          </div>
        </div>
...
function AppliedTokens({
  ...
  const tokens = (query || "").match(/(?:[a-z_]+:\"[^\"]+\"|[a-z_]+:[^\s)]+)/gi) || [];
  if (tokens.length === 0) return null;
  return (
    <div className="mt-2 flex flex-wrap gap-2 items-center" aria-label="Active filters">
      {tokens.map((t, idx) => (
        <button
          key={`${t}-${idx}`}
          type="button"
