...
import { 
  ...
  HardDrive, Cloud, RefreshCw, TrendingUp,
  Calendar, FolderOpen, AlertTriangle, Info
} from 'lucide-react'
import { backupService, type BackupConfig, type BackupJob, type BackupVersion } from '../services/BackupService'
  ...
  return (
    <div className="fixed inset-0 bg-black/60 backdrop-blur-sm z-50 flex items-center justify-center p-4">
      <div className="bg-white dark:bg-gray-800 rounded-2xl max-w-4xl w-full max-h-[80vh] overflow-hidden shadow-2xl">
        ...
        <div className={`p-4 ${health.isHealthy ? 'bg-green-50 dark:bg-green-900/20' : 'bg-yellow-50 dark:bg-yellow-900/20'}`}>
            ...
            <div className="space-y-6">
              ...
              <div className="grid grid-cols-3 gap-4">
                <div className="bg-gray-50 dark:bg-gray-700 rounded-lg p-4">
                  ...
                  <p className="text-sm text-gray-600 dark:text-gray-400">Photos protected</p>
                </div>
                ...
                <div className="bg-gray-50 dark:bg-gray-700 rounded-lg p-4">
                  <div className="flex items-center justify-between mb-2">
                    ...
                    <span className="text-2xl font-bold text-gray-900 dark:text-white">
                      {status.nextScheduled ? formatRelativeTime(status.nextScheduled) : 'Manual'}
                    </span>
                  </div>
  ...
  return (
    <div className="bg-gray-50 dark:bg-gray-700 rounded-lg p-4">
      ...
      <div className="flex items-center justify-between text-xs text-gray-500">
        <span>{job.processedFiles} of {job.totalFiles} photos</span>
        {job.estimatedTimeRemaining && (
          <span>About {Math.ceil(job.estimatedTimeRemaining / 60)} minutes left</span>
        )}
      </div>
      {job.currentFile && (
    ...
    <div className="space-y-6">
      ...
      <div>
        ...
        <div className="grid grid-cols-2 gap-3">
          {[
            { value: 'realtime', label: 'As photos change', desc: 'Most protection' },
            { value: 'hourly', label: 'Every hour', desc: 'Great for active use' },
            { value: 'daily', label: 'Once a day', desc: 'Good balance' },
            { value: 'weekly', label: 'Once a week', desc: 'Light usage' },
            { value: 'manual', label: "I'll do it myself", desc: 'Full control' }
          ].map(option => (
            <button
              key={option.value}
              onClick={() => onChange({ ...config, frequency: option.value as any })}
        ...
        <div className="space-y-2">
          <label className="flex items-center gap-3 p-3 rounded-lg border border-gray-300 dark:border-gray-600 cursor-pointer hover:bg-gray-50 dark:hover:bg-gray-700">
            ...
            <div className="flex-1">
              ...
              <div className="text-xs text-gray-500">Keep a copy on your computer or external drive</div>
            </div>
          </label>
        </div>
...
function BackupHistory() {
  ...
  if (history.length === 0) {
    return (
      <div className="text-center py-8">
        ...
        <p className="text-sm text-gray-400 mt-1">Your backup history will appear here</p>
      </div>
    )
  }
  ...
  return (
    <div className="space-y-2">
      {history.map(version => (
        <div key={version.id} className="p-4 bg-gray-50 dark:bg-gray-700 rounded-lg">
          <div className="flex items-center justify-between">
            <div>
              ...
              <div className="text-sm text-gray-500">
                {version.fileCount.toLocaleString()} photos • {formatBytes(version.totalSize)}
              </div>
