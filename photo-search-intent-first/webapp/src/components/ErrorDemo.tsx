...
export class OfflineActionQueue {
		...
		const action: OfflineAction = {
			...
			metadata: {
				...
				conflictResolutionStrategy:
					options?.conflictResolutionStrategy || "LAST_WRITE_WINS",
			},
		...
		for (const action of queuedActions) {
			if (action.dependencies && action.dependencies.length > 0) {
				...
				const allDependenciesSynced = action.dependencies.every((depId) => {
					...
					return (
						dep && (dep.status === "SYNCED" || dep.status === "PENDING_SYNC")
					);
				});
	...
	async sync(): Promise<void> {
		...
		try {
			...
			const actionsToSync = Array.from(this.actions.values()).filter(
				(action) =>
			...
			actionsToSync.forEach((action) => {
				...
				const backoffDelay = Math.min(
					2 ** action.syncAttempts * 60000, // Start with 1 minute, double each time
					3600000, // Max 1 hour
				);
	...
	clearPendingSync(before?: Date): void {
		const pendingSyncActions = Array.from(this.actions.values()).filter(
			(action) => action.status === "PENDING_SYNC",
		);
		...
		if (this.persistence) {
			this.persistence
				...
				.catch((error) => {
					console.warn(
						"Failed to persist queue after clearing pending sync actions:",
						error,
					);
				});
		}
	...
	getStatistics(): {
		...
		const statistics = {
			...
			requiresNetwork: actions.filter((a) => a.metadata?.requiresNetwork)
				.length,
			requiresUserInteraction: actions.filter(
				(a) => a.metadata?.requiresUserInteraction,
			).length,
		};
	...
	getLowPriorityActions(): OfflineAction[] {
		return Array.from(this.actions.values()).filter(
			(action) => action.priority === "LOW" || action.priority === "BACKGROUND",
		);
	}
	...
	getOldActions(olderThan: number): OfflineAction[] {
		...
		return Array.from(this.actions.values()).filter(
			(action) => action.metadata && action.metadata.createdAt < cutoff,
		);
	}
	...
	getStaleActions(olderThan: number): OfflineAction[] {
		const cutoff = Date.now() - olderThan;
		return Array.from(this.actions.values()).filter(
			(action) => action.metadata && action.metadata.updatedAt < cutoff,
		);
	}
	...
	getActionsWithRetries(minRetries: number): OfflineAction[] {
		return Array.from(this.actions.values()).filter(
			(action) => action.metadata && action.metadata.retryCount >= minRetries,
		);
	}
	...
	getActionsWithSyncAttempts(minSyncAttempts: number): OfflineAction[] {
		return Array.from(this.actions.values()).filter(
			(action) => action.syncAttempts >= minSyncAttempts,
		);
	}
	...
	getScheduledSyncActions(): OfflineAction[] {
		return Array.from(this.actions.values()).filter(
			(action) => action.nextSyncAttempt && action.nextSyncAttempt > Date.now(),
		);
	}
	...
	getPossiblyStuckActions(threshold: number = 300000): OfflineAction[] {
		// 5 minutes default
		return Array.from(this.actions.values()).filter((action) => {
	...
	getActionsByCorrelationId(correlationId: string): OfflineAction[] {
		return Array.from(this.actions.values()).filter(
			(action) => action.context?.correlationId === correlationId,
		);
	}
	...
	getActionsByUserId(userId: string): OfflineAction[] {
		return Array.from(this.actions.values()).filter(
			(action) => action.context?.userId === userId,
		);
	}
	...
	getActionsBySessionId(sessionId: string): OfflineAction[] {
		return Array.from(this.actions.values()).filter(
			(action) => action.context?.sessionId === sessionId,
		);
	}
	...
	getActionsByDeviceId(deviceId: string): OfflineAction[] {
		return Array.from(this.actions.values()).filter(
			(action) => action.context?.deviceId === deviceId,
		);
	}
	...
	validateQueueIntegrity(): { valid: boolean; errors: string[] } {
			...
			if (action.dependencies) {
				action.dependencies.forEach((depId) => {
					if (!this.actions.has(depId)) {
						errors.push(
							`Orphaned dependency: ${action.id} depends on non-existent action ${depId}`,
						);
					}
				});
			}
		...
		Array.from(this.dependencies.entries()).forEach(([depId, dependentIds]) => {
			if (!this.actions.has(depId)) {
				errors.push(
					`Orphaned dependency entry: ${depId} has dependents but doesn't exist`,
				);
			}
			dependentIds.forEach((dependentId) => {
				if (!this.actions.has(dependentId)) {
					errors.push(
						`Orphaned dependent: ${dependentId} depends on ${depId} but doesn't exist`,
					);
				}
			});
		});
	...
	repairQueueIntegrity(): void {
		...
		Array.from(this.actions.values()).forEach((action) => {
			if (action.dependencies) {
				action.dependencies = action.dependencies.filter((depId) =>
					this.actions.has(depId),
				);
		...
		Array.from(this.groups.entries()).forEach(([groupId, memberIds]) => {
			const validMembers = new Set(
				Array.from(memberIds).filter((memberId) => this.actions.has(memberId)),
			);
		...
		Array.from(this.dependencies.entries()).forEach(([depId, dependentIds]) => {
			if (!this.actions.has(depId)) {
				...
				const validDependents = new Set(
					Array.from(dependentIds).filter((dependentId) =>
						this.actions.has(dependentId),
					),
				);
		...
		if (this.persistence) {
			this.persistence
				...
				.catch((error) => {
					console.warn(
						"Failed to persist queue after integrity repair:",
						error,
					);
				});
		}
