...
import { apiAnalytics, apiAutotag, apiTodo } from "../api";
...
function renderMarkdown(text: string) {
		...
		if (list.length) {
			out.push(
				<ul className="list-disc ml-6 space-y-1" key={`ul-${out.length}`}>
					{list.map((node, i) => (
						<li key={`li-${out.length}-${i}`}>{node}</li>
					))}
				</ul>,
			);
			list = [];
	...
	lines.forEach((ln, idx) => {
		...
		if (ln.trim() === "") {
			flushList();
			out.push(<div className="h-2" key={`sp-${idx}-${Math.random()}`} />);
			return;
		}
		flushList();
		out.push(<p key={`p-${idx}-${ln.slice(0, 20)}`}>{ln}</p>);
	});
	flushList();
	return out;
}
...
export default function TasksView() {
	...
	const handleAutotag = async () => {
		...
		try {
			...
			const result = await apiAutotag(dir, engine);
			if (result.tags) {
				const totalPhotos = Object.keys(result.tags || {}).length;
				if (totalPhotos > 0) {
					setAutotagResult(
						`Generated tags for ${totalPhotos} photo${totalPhotos === 1 ? "" : "s"}`,
					);
				} else {
			...
			setAutotagResult(`Error: ${e instanceof Error ? e.message : String(e)}`);
		} finally {
			...
			<div className="mt-6">
				...
				<div className="bg-gray-50 dark:bg-gray-800 border rounded p-3 space-y-3">
					<p className="text-sm text-gray-600 dark:text-gray-400">
						Generate smart tags for your photos using AI analysis. This will
						analyze photo content and automatically assign relevant tags.
					</p>
					...
					{autotagResult && (
						<div
							className={`p-2 rounded text-sm ${
								autotagResult.startsWith("Error:")
			...
			<div className="mt-6">
				...
				<div className="border rounded divide-y">
					{events.length === 0 ? (
						...
						events.map((e) => (
							<div
								key={`${e.type}-${e.time}-${e.query || e.token || Math.random()}`}
