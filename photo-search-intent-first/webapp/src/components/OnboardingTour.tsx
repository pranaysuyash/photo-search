				...
				{currentTourStep?.target && (
					<div className="absolute inset-0">
						<div
							...
							style={{
								clipPath: `polygon(0 0, 100% 0, 100% 100%, 0 100%, 0 0, ${getHighlightClipPath(
									currentTourStep.target,
								)})`,
							}}
						/>
...
function getHighlightClipPath(selector: string): string {
...
function getPositionClasses(position: string, target?: string): string {
	...
	switch (position) {
		case "top":
			return `left-[${centerX}px] top-[${rect.top - 20}px] -translate-x-1/2`;
		case "bottom":
			return `left-[${centerX}px] top-[${rect.bottom + 20}px] -translate-x-1/2`;
		case "left":
			return `left-[${rect.left - 20}px] top-[${centerY}px] -translate-y-1/2`;
		case "right":
			return `left-[${rect.right + 20}px] top-[${centerY}px] -translate-y-1/2`;
		default:
			return "top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2";
	}
}
