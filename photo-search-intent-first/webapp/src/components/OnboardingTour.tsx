      ...
      >
        ...
        {currentTourStep?.target && (
          <div className="absolute inset-0 pointer-events-none">
            {(() => {
              const rect = getTargetRect(currentTourStep.target!);
              const padding = 8;
              if (!rect) return null;
              return (
                <div
...
function getCardPositionStyle(position: string, target?: string): React.CSSProperties {
  ...
  switch (position) {
    case 'top':
      return { left: centerX, top: rect.top - 20, transform: 'translateX(-50%)' };
    case 'bottom':
      return { left: centerX, top: rect.bottom + 20, transform: 'translateX(-50%)' };
    case 'left':
      return { left: rect.left - 20, top: centerY, transform: 'translateY(-50%)' };
    case 'right':
      return { left: rect.right + 20, top: centerY, transform: 'translateY(-50%)' };
    default:
