...
export default function Workspace({ workspace, setWorkspace }: WorkspaceProps) {
	return (
			...
			<div className="mt-2 text-sm">
						...
						{workspace.map((p, i) => (
							...
							>
								...
								<button
									...
									onClick={async () => {
										try {
											const r = await apiWorkspaceRemove(p);
				...
				<div className="mt-2 flex gap-2">
					...
					<button
						type="button"
						onClick={async () => {
							const el = document.getElementById("ws_add") as HTMLInputElement;
							const val = el.value.trim();
							if (!val) return;
							try {
								const r = await apiWorkspaceAdd(val);
								setWorkspace(r.folders || []);
								el.value = "";
							} catch {}
						}}
