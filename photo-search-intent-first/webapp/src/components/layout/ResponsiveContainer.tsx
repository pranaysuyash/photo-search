...
import { LAYOUT_PATTERNS, RESPONSIVE_CLASSES } from '../../constants/responsiveSpacing';
...
export const ResponsiveContainer = forwardRef<HTMLDivElement, ResponsiveContainerProps>(
	({ children, className, maxWidth = '2xl', padding = 'md', centered = false }, ref) => {
		...
		return <div ref={ref} className={containerClasses}>{children}</div>;
	}
);
...
export const ResponsiveGrid = forwardRef<HTMLDivElement, ResponsiveGridProps>(
	({ children, className, cols = { xs: 1, sm: 2, md: 3, lg: 4, xl: 5, '2xl': 6 }, gap = 'md' }, ref) => {
		...
		return <div ref={ref} className={gridClasses}>{children}</div>;
	}
);
...
export const ResponsiveFlex = forwardRef<HTMLDivElement, ResponsiveFlexProps>(
	({
		...
		return <div ref={ref} className={flexClasses}>{children}</div>;
	}
);
...
export const ResponsiveStack = forwardRef<HTMLDivElement, ResponsiveStackProps>(
	({ children, className, spacing = 'md' }, ref) => {
		...
		return <div ref={ref} className={stackClasses}>{children}</div>;
	}
);
