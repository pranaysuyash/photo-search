...
import { AdvancedRecognitionService, QualityMetrics } from '../services/AdvancedRecognitionService'
...
export function QualityOverlay({ imagePath, show, imageData }: QualityOverlayProps) {
  ...
  useEffect(() => {
    ...
    async function assess() {
      setLoading(true)
      try {
        const quality = await AdvancedRecognitionService.assessQuality(imagePath, imageData)
        setMetrics(quality)
      } catch (error) {
    ...
    <div className="absolute top-2 left-2 bg-white/95 backdrop-blur rounded-lg p-3 shadow-lg max-w-xs">
      ...
      <div className="flex items-center justify-between mb-3 pb-3 border-b">
        <div>
          ...
          <p className={`text-2xl font-bold ${getQualityColor(metrics.overall)}`}>
