...
export default function SearchControls({ onSearch, onShowHelp }: SearchControlsProps) {
  ...
  const searchActions = usePhotoActions()
    ...
    <>
      <div className="grid grid-cols-1 lg:grid-cols-6 gap-3 items-end">
        <div className="lg:col-span-2">
          ...
          <input value={dir} onChange={e=>settingsActions.setDir(e.target.value)} placeholder="/path/to/photos" className="w-full border rounded px-3 py-2" />
        </div>
        <div>
          <label className="block text-sm">Engine</label>
          <select value={engine} onChange={e=>settingsActions.setEngine(e.target.value)} className="w-full border rounded px-3 py-2">
            {engines.map(e=> <option key={e.key} value={e.key}>{e.label}</option>)}
          </select>
          <div className="flex items-center gap-2 text-sm">
            <div>Camera</div>
            <input value={camera} onChange={e=>settingsActions.setCamera(e.target.value)} placeholder="e.g., iPhone" className="border rounded px-2 py-1" />
            <div>ISO</div>
            <input value={isoMin} onChange={e=>settingsActions.setIsoMin(e.target.value)} placeholder="min" className="w-20 border rounded px-2 py-1" />
            <div>to</div>
            <input value={isoMax} onChange={e=>settingsActions.setIsoMax(e.target.value)} placeholder="max" className="w-20 border rounded px-2 py-1" />
            <div>f/</div>
            <input value={fMin} onChange={e=>settingsActions.setFMin(e.target.value)} placeholder="min" className="w-20 border rounded px-2 py-1" />
            <div>to</div>
            <input value={fMax} onChange={e=>settingsActions.setFMax(e.target.value)} placeholder="max" className="w-20 border rounded px-2 py-1" />
          </div>
        </div>
        <div className="lg:col-span-2">
          <label className="block text-sm">Query</label>
          <input value={query} onChange={e=>searchActions.setQuery(e.target.value)} placeholder="friends on beach" className="w-full border rounded px-3 py-2" />
        </div>
        <div>
          <button onClick={onShowHelp} className="w-full mt-2 bg-gray-200 rounded px-3 py-2">Help</button>
        </div>
      </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-3 mt-3">
          {needsHf && (
            <div>
              ...
              <input value={hfToken} onChange={e=>settingsActions.setHfToken(e.target.value)} type="password" className="w-full border rounded px-3 py-2" />
            </div>
          )}
          {needsOAI && (
            <div>
              <label className="block text-sm">OpenAI API key</label>
              <input value={openaiKey} onChange={e=>settingsActions.setOpenaiKey(e.target.value)} type="password" className="w-full border rounded px-3 py-2" />
            </div>
          )}
        </div>
      )}
      ...
      <div className="flex items-center gap-3 mt-3 flex-wrap">
        <button onClick={onSearch} className="bg-blue-600 text-white rounded px-4 py-2">Search</button>
        <div className="text-sm text-gray-700">Top‑K</div>
        <input type="number" min={1} max={200} value={topK} onChange={e=>searchActions.setTopK(parseInt(e.target.value||'24',10))} className="w-20 border rounded px-2 py-1" />
        ...
        <div className="flex items-center gap-2 text-sm">
          <div>Place</div>
          <input value={place} onChange={e=>settingsActions.setPlace(e.target.value)} placeholder="e.g., San Francisco" className="border rounded px-2 py-1" />
        </div>
        ...
        {(() => {
          ...
          return (
            <select value={fastKind} onChange={e=>settingsActions.setFastKind(e.target.value as any)} className="border rounded px-2 py-1 text-sm">
        ...
        <div className="flex items-center gap-2 text-sm">
          <div>View</div>
          <select value={viewMode} onChange={e=>uiActions.setViewMode(e.target.value as any)} className="border rounded px-2 py-1 text-sm">
          ...
          <div className="flex items-center gap-1 text-xs">
            {persons.map(p => (
              <span key={p} className="px-2 py-1 bg-blue-600 text-white rounded flex items-center gap-2">
                {p}
                <button onClick={()=> workspaceActions.setPersons(persons.filter(x=>x!==p))} className="bg-white/20 rounded px-1">×</button>
              </span>
            ))}
            <button onClick={()=>workspaceActions.setPersons([])} className="px-2 py-1 bg-gray-200 rounded">Clear</button>
          </div>
        )}
      </div>
    </>
