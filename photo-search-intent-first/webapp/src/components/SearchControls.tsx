		...
		<>
			<div className="grid grid-cols-1 lg:grid-cols-6 gap-3 items-end">
				<div className="lg:col-span-2">
					<label htmlFor="ps_dir" className="block text-sm">
						Your photo folder
					</label>
					<input
						id="ps_dir"
				...
				<div>
					<label htmlFor="ps_engine" className="block text-sm">
						Engine
					</label>
					<select
						id="ps_engine"
					...
					<div className="flex items-center gap-2 text-sm">
						<label htmlFor="ps_camera" className="text-sm">
							Camera
						</label>
						<input
							id="ps_camera"
						...
						/>
						<label htmlFor="ps_iso_min" className="text-sm">
							ISO
						</label>
						<input
							id="ps_iso_min"
							...
							value={isoMin}
							onChange={(e) =>
								settingsActions.setIsoMin(parseFloat(e.target.value) || 0)
							}
							placeholder="min"
						...
						<input
							...
							value={isoMax}
							onChange={(e) =>
								settingsActions.setIsoMax(parseFloat(e.target.value) || 0)
							}
							placeholder="max"
							className="w-20 border rounded px-2 py-1"
						/>
						<label htmlFor="ps_fmin" className="text-sm">
							f/
						</label>
						<input
							id="ps_fmin"
							...
							value={fMin}
							onChange={(e) =>
								settingsActions.setFMin(parseFloat(e.target.value) || 0)
							}
							placeholder="min"
							className="w-20 border rounded px-2 py-1"
						/>
						...
						<input
							...
							value={fMax}
							onChange={(e) =>
								settingsActions.setFMax(parseFloat(e.target.value) || 0)
							}
							placeholder="max"
							className="w-20 border rounded px-2 py-1"
						/>
				...
				<div className="lg:col-span-2">
					<label htmlFor="ps_query" className="block text-sm">
						Query
					</label>
					<input
						id="ps_query"
				...
				<div>
					<button
						type="button"
						onClick={onShowHelp}
						className="w-full mt-2 bg-gray-200 rounded px-3 py-2"
					>
						Help
					</button>
				</div>
			...
			{(needsHf || needsOAI) && (
				<div className="grid grid-cols-1 md:grid-cols-2 gap-3 mt-3">
					{needsHf && (
						<div>
							<label htmlFor="ps_hf" className="block text-sm">
								HF API token
							</label>
							<input
								id="ps_hf"
								value={hfToken}
								onChange={(e) => settingsActions.setHfToken(e.target.value)}
								type="password"
								className="w-full border rounded px-3 py-2"
							/>
						</div>
					...
					{needsOAI && (
						<div>
							<label htmlFor="ps_oai" className="block text-sm">
								OpenAI API key
							</label>
							<input
								id="ps_oai"
								value={openaiKey}
								onChange={(e) => settingsActions.setOpenaiKey(e.target.value)}
								type="password"
								className="w-full border rounded px-3 py-2"
							/>
						</div>
			...
			<div className="flex items-center gap-3 mt-3 flex-wrap">
				...
				<input
					...
					value={topK}
					onChange={(e) =>
						searchActions.setTopK(parseInt(e.target.value || "24", 10))
					}
					className="w-20 border rounded px-2 py-1"
				/>
				...
				<div className="flex items-center gap-2 text-sm">
					<label htmlFor="ps_place" className="text-sm">
						Place
					</label>
					<input
						id="ps_place"
						...
						return (
							<button
								...
								className={`text-xs px-2 py-1 rounded ${
									chosen ? "bg-blue-600 text-white" : "bg-gray-200"
								}`}
							>
								{t}
							</button>
						...
						<select
							...
							onChange={(e) =>
								settingsActions.setFastKind(
									e.target.value as "" | "annoy" | "faiss" | "hnsw",
								)
							}
							className="border rounded px-2 py-1 text-sm"
						>
							<option value="">Auto</option>
							<option value="faiss" disabled={!fa}>
								FAISS{st && !fa ? " (not built)" : ""}
							</option>
							<option value="hnsw" disabled={!hs}>
								HNSW{st && !hs ? " (not built)" : ""}
							</option>
							<option value="annoy" disabled={!an}>
								Annoy{st && !an ? " (not built)" : ""}
							</option>
						</select>
					);
				})()}
				...
				<div className="flex items-center gap-2 text-sm">
					<label htmlFor="ps_view" className="text-sm">
						View
					</label>
					<select
						id="ps_view"
				...
				{persons.length > 0 && (
					<div className="flex items-center gap-1 text-xs">
						...
						<button
							type="button"
							onClick={() => workspaceActions.setPersons([])}
