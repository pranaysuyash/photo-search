...
import { RefreshCw, Activity, Database, Wifi, Cpu, HardDrive } from 'lucide-react';
...
export function MonitoringDashboard() {
  ...
  const fetchMonitoringData = async () => {
    try {
      ...
      console.error('[MonitoringDashboard] Failed to fetch monitoring data:', error);
    } finally {
      setIsLoading(false);
    }
  };
  ...
  if (!monitoringData) {
    return (
      <div className="text-center p-8">
        <p className="text-gray-600 dark:text-gray-400">Failed to load monitoring data</p>
      ...
      <Card>
        ...
        <CardContent>
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
            <div className="flex items-center space-x-3">
              <Wifi className={`w-5 h-5 ${monitoringData.offline.isOnline ? 'text-green-500' : 'text-red-500'}`} />
              <div>
