...
export function FaceVerificationPanel({
	...
	const handleVerify = (clusterId: string, accept: boolean) => {
		...
		setVerifications((prev) =>
			prev.map((v) =>
				v.clusterId === clusterId ? { ...v, verified: accept } : v,
			),
		);
	};
							...
							<div className="flex items-start justify-between">
								<div>
									<p className="text-sm font-medium">
										Suggested:{" "}
										{knownPeople.get(verifications[selectedFace].clusterId)
											?.name || "Unknown"}
									</p>
									<p className="text-xs text-gray-600 mt-1">
										Confidence:{" "}
										{(verifications[selectedFace].confidence * 100).toFixed(1)}%
									</p>
								</div>
								<div className="flex gap-2">
									<button
										type="button"
										onClick={() =>
											handleVerify(verifications[selectedFace].clusterId, true)
										}
									...
									<button
										type="button"
										onClick={() =>
											handleVerify(verifications[selectedFace].clusterId, false)
										}
										className={`px-3 py-1 text-xs rounded ${
