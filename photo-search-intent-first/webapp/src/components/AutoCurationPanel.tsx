...
import { AutoCurationEngine, AutoCurationOptions, AutoCurationResult, CurationAction, SmartCollectionSuggestion, CurationProgress } from '../services/AutoCurationEngine';
...
export function AutoCurationPanel({
	...
	const startAnalysis = useCallback(async () => {
		...
		try {
			const analysisResult = await curationEngine.analyzePhotos(photoPaths, (progress) => {
	...
	const executeSelectedActions = useCallback(async () => {
		...
		const actions = result.actions.filter(action => selectedActions.has(action.description));
				...
				switch (action.type) {
					case 'create_collection':
						if (onCreateCollection) {
							onCreateCollection(`Auto-Created Collection (${new Date().toLocaleDateString()})`, action.photos);
						}
						break;
	...
	}, [result, selectedActions, onCreateCollection, onDeletePhotos, onRatePhotos, onTagPhotos, toast]);

	const createSelectedCollections = useCallback(async () => {
		if (!result || selectedCollections.size === 0) return;

		const collections = result.collections.filter(collection => selectedCollections.has(collection.name));
	...
	const getActionIcon = (actionType: CurationAction['type']) => {
		switch (actionType) {
			case 'create_collection': return <FolderPlus className="w-4 h-4" />;
			case 'delete_duplicates': return <Trash2 className="w-4 h-4" />;
			case 'rate_photos': return <Star className="w-4 h-4" />;
			case 'tag_photos': return <Target className="w-4 h-4" />;
			case 'move_photos': return <RefreshCw className="w-4 h-4" />;
	...
	const getActionColor = (actionType: CurationAction['type']) => {
		switch (actionType) {
			case 'delete_duplicates': return 'bg-red-100 text-red-800 border-red-200';
			case 'rate_photos': return 'bg-yellow-100 text-yellow-800 border-yellow-200';
			case 'create_collection': return 'bg-green-100 text-green-800 border-green-200';
			case 'tag_photos': return 'bg-blue-100 text-blue-800 border-blue-200';
	...
	const getCollectionColor = (type: SmartCollectionSuggestion['type']) => {
		switch (type) {
			...
			case 'people': return 'bg-blue-100 text-blue-800 border-blue-200';
			...
			case 'cleanup': return 'bg-red-100 text-red-800 border-red-200';
			default: return 'bg-gray-100 text-gray-800 border-gray-200';
		}
	};
					...
					{progress && (
						<div className="space-y-3">
							<div className="flex justify-between text-sm">
								...
								<span>{progress.processed_photos}/{progress.total_photos}</span>
							</div>
							<Progress value={(progress.processed_photos / progress.total_photos) * 100} className="w-full" />
							{progress.estimated_time_remaining > 0 && (
								<p className="text-xs text-gray-500">
									Estimated time remaining: {Math.round(progress.estimated_time_remaining / 1000)} seconds
								</p>
							)}
				...
				{!result ? (
					<div className="text-center py-8">
						...
						<p className="text-gray-500 mb-4">
							Select photos and click "Analyze Photos" to start intelligent photo organization
						</p>
					...
					<Tabs defaultValue="actions" className="space-y-4">
							...
							<div className="flex justify-between items-center">
								...
								<div className="flex items-center gap-2">
									<Button
										variant="outline"
										size="sm"
										onClick={() => setSelectedActions(new Set(result.actions.map(a => a.description)))}
									>
										Select All
									</Button>
									<Button
								...
								{result.actions.map((action, index) => (
									...
									>
										<div className="flex items-start justify-between gap-4">
											...
											<Checkbox checked={selectedActions.has(action.description)} />
										</div>
									</motion.div>
								))}
							</div>
						...
						<TabsContent value="collections" className="space-y-4">
							<div className="flex justify-between items-center">
								<p className="text-gray-600">
									Smart collections created based on patterns and content analysis
								</p>
								<div className="flex items-center gap-2">
									<Button
										variant="outline"
										size="sm"
										onClick={() => setSelectedCollections(new Set(result.collections.map(c => c.name)))}
									>
										Select All
									</Button>
									<Button
							...
							<div className="grid gap-4">
								{result.collections.map((collection, index) => (
									...
									>
										<div className="flex items-start justify-between gap-4">
											<div className="flex items-start gap-3">
												<div className={`p-2 rounded-lg ${getCollectionColor(collection.type)}`}>
												...
												<div className="flex-1">
													...
													<div className="flex flex-wrap gap-1">
														{collection.tags.slice(0, 4).map((tag, tagIndex) => (
						...
						<TabsContent value="summary" className="space-y-6">
							<div className="grid grid-cols-2 md:grid-cols-4 gap-4">
								...
								<div className="text-center p-4 bg-green-50 rounded-lg">
									...
									<div className="text-sm text-gray-600">Collections Suggested</div>
								</div>
							</div>
							...
							<div className="grid gap-4">
								...
								<div className="grid grid-cols-2 gap-4 text-sm">
									...
									<div>
										<span className="text-gray-600">Quality Ratings Assigned:</span>
										<span className="ml-2 font-medium">
									...
									<div>
										...
										<span className="ml-2 font-medium">
											{(result.summary.total_photos_analyzed / (result.summary.processing_time / 1000)).toFixed(2)}
										</span>
									</div>
									<div>
										...
										<span className="ml-2 font-medium">
											{result.analysis.length > 0
												? Math.round(result.analysis.reduce((sum, a) => sum + a.quality.overall, 0) / result.analysis.length)
												: 0}%
										</span>
									</div>
								</div>
