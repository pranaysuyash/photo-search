...
import { Search, FolderOpen, Globe, FolderTree, Settings, X, Save, RotateCcw } from 'lucide-react';
...
export default function MultiFolderSearchControls({
  ...
  const [recentSearches, setRecentSearches] = useState<Array<{ query: string; scope: SearchScope; timestamp: number }>>([]);
  ...
  const folderStats = useMemo(() => {
    ...
    const selectedFolders = searchScope.type === 'custom' ? searchScope.selectedFolders.length :
                           searchScope.type === 'all' ? totalFolders : 0;
        ...
        <div className="space-y-3">
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-2">
            {scopePresets.map((preset) => (
              <Button
                key={preset.name}
                variant={searchScope.type === preset.type ? "default" : "outline"}
                onClick={() => onScopeChange({
                  type: preset.type,
                  selectedFolders: preset.type === 'custom' ? searchScope.selectedFolders : [],
                  searchScope: 'all'
                })}
              ...
              <div className="flex items-center justify-between">
                ...
                <div className="flex gap-2">
                  <Button
                    ...
                    disabled={searchScope.selectedFolders.length === workspace.length}
                  >
                    Select All
                  </Button>
                  <Button
                ...
                <div className="space-y-3">
                  ...
                  <div className="max-h-48 overflow-y-auto space-y-1">
                    {filteredFolders.map((folder) => {
                      const isSelected = searchScope.selectedFolders.includes(folder);
                    ...
                    >
                      <div className="flex-1 text-left">
                        ...
                        <div className="text-xs text-gray-500">
                          {config.query} • {config.scope.selectedFolders.length} folders
                        </div>
                      </div>
                    </Button>
                  ))}
