...
export function DuplicateDetectionPanel({
  ...
  const [deleteStrategy, setDeleteStrategy] = useState<'best' | 'newest' | 'largest'>('best')
  ...
  const handleDelete = async () => {
    ...
    try {
      const groupsToDelete = result.groups.filter(g => selectedGroups.has(g.id))
      const deleteResult = await DuplicateDetectionService.batchDeleteDuplicates(
        groupsToDelete,
        onDelete,
        {
      ...
      alert(`Deleted ${deleteResult.deleted} duplicates, freed ${formatBytes(deleteResult.freed)}`)
  ...
  const getTypeColor = (type: 'exact' | 'near' | 'similar'): string => {
    switch (type) {
      ...
      case 'near': return 'text-orange-600'
  ...
  const getTypeLabel = (type: 'exact' | 'near' | 'similar'): string => {
    switch (type) {
      ...
      case 'near': return 'Near Duplicate'
      case 'similar': return 'Similar Photo'
    }
  }
          ...
          <div>
            ...
            {result && (
              <p className="text-sm text-gray-600 mt-1">
                Found {result.totalDuplicates} duplicates in {result.groups.length} groups
                • Can free up {formatBytes(result.potentialSavings)}
              </p>
            )}
          </div>
          <button
          ...
          <div className="px-6 py-3 border-b bg-gray-50">
            <div className="flex items-center justify-between">
              ...
              <div className="flex gap-2">
                <button
                  onClick={() => setSelectedGroups(new Set(result.groups.map(g => g.id)))}
                ...
                >
                  {deleting ? 'Deleting...' : `Delete Selected (${selectedGroups.size})`}
                </button>
              </div>
            </div>
          </div>
        ...
        <div className="flex-1 overflow-y-auto px-6 py-4">
          {scanning ? (
            <div className="text-center py-12">
              ...
              <p className="mt-3 text-gray-600">Analyzing photos for duplicates...</p>
            </div>
          ) : result ? (
                    ...
                    <div className="flex items-center gap-4">
                      ...
                      <div>
                        <div className="flex items-center gap-2">
                          <span className={`font-medium ${getTypeColor(group.type)}`}>
                        ...
                        {group.photos.map((photo) => (
                          <div
                            ...
                            className={`relative p-2 bg-white rounded border ${
                              photo === group.bestPhoto ? 'border-green-500' : 'border-gray-200'
                            }`}
                          >
