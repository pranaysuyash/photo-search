...
import { RefreshCw, Settings, TrendingUp, TrendingDown, AlertTriangle, CheckCircle } from 'lucide-react';
    ...
    const mockOptimizations: OptimizationResult[] = [
      {
        ...
        improvements: [
          { type: 'memory', description: 'Cleared 20 old cache entries', impact: 15, confidence: 0.8 },
          { type: 'memory', description: 'Updated backend selection weights', impact: 10, confidence: 0.7 }
        ],
      ...
      },
      {
        ...
        improvements: [
          { type: 'latency', description: 'Pre-loaded 3 frequently used models', impact: 20, confidence: 0.9 },
          { type: 'latency', description: 'Optimized backend selection', impact: 15, confidence: 0.8 }
        ],
  ...
  return (
    <div className="p-6 space-y-6 bg-gray-50 min-h-screen">
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-3xl font-bold text-gray-900">ANN Performance Dashboard</h1>
        ...
        <TabsContent value="overview" className="space-y-6">
              ...
              <CardContent>
                ...
                <p className="text-xs text-muted-foreground">
                  {performanceData.length > 1 && (
                    <span className={performanceData[performanceData.length - 1].inferenceTime < performanceData[performanceData.length - 2].inferenceTime ? 'text-green-600' : 'text-red-600'}>
                      {performanceData[performanceData.length - 1].inferenceTime < performanceData[performanceData.length - 2].inferenceTime ? '↓' : '↑'}
                      {Math.abs(Math.round(performanceData[performanceData.length - 1].inferenceTime - performanceData[performanceData.length - 2].inferenceTime))}ms
                    </span>
                  )}
                </p>
              </CardContent>
          ...
          <Card>
            <CardHeader>
              ...
              <CardDescription>Real-time performance metrics over the last 30 minutes</CardDescription>
            </CardHeader>
            <CardContent>
                ...
                <CardContent>
                  <div className="space-y-4">
                    <div className="flex justify-between">
                      ...
                      <span className="text-sm font-medium">{Math.round(backend.load * 100)}%</span>
                    </div>
                    <div className="w-full bg-gray-200 rounded-full h-2">
                      <div
                        className={`h-2 rounded-full ${
                          backend.load < 0.5 ? 'bg-green-500' :
                          backend.load < 0.8 ? 'bg-yellow-500' : 'bg-red-500'
                        }`}
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            <Card>
              <CardHeader>
                ...
                <CardDescription>Performance metrics across different backends</CardDescription>
              </CardHeader>
              <CardContent>
                <ResponsiveContainer width="100%" height={300}>
                  <BarChart data={backendMetrics}>
                    ...
                    <Tooltip />
                    <Legend />
                    <Bar dataKey="inferenceTime" fill="#8884d8" name="Inference Time (ms)" />
                    <Bar dataKey="memoryUsage" fill="#82ca9d" name="Memory Usage (MB)" />
                  </BarChart>
                </ResponsiveContainer>
              </CardContent>
            </Card>
            ...
            <Card>
              <CardHeader>
                ...
                <CardDescription>Model accuracy across different backends</CardDescription>
              </CardHeader>
              <CardContent>
                <ResponsiveContainer width="100%" height={300}>
                  <PieChart>
                    <Pie
                      data={backendMetrics.map(b => ({ ...b, value: b.accuracy * 100 }))}
                      cx="50%"
                      cy="50%"
                      labelLine={false}
        ...
        <TabsContent value="optimization" className="space-y-6">
          <Card>
            <CardHeader>
              ...
              <CardDescription>Automatic performance improvements and their impact</CardDescription>
            </CardHeader>
            <CardContent>
              <div className="space-y-4">
        ...
        <TabsContent value="settings" className="space-y-6">
          <Card>
            <CardHeader>
              ...
              <CardDescription>Configure automatic performance optimization</CardDescription>
            </CardHeader>
            <CardContent>
              <div className="space-y-6">
                <div className="flex items-center justify-between">
                  <div>
                    ...
                    <p className="text-sm text-gray-600">Enable automatic performance tuning</p>
                  </div>
