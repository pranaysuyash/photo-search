...
export default function JobsDrawer({ open, onClose }: { open: boolean; onClose: () => void }) {
  const dir = useDir()
  ...
  useEffect(() => {
    ...
    async function load() {
      if (!dir) return
      setLoading(true)
      try { const r = await apiAnalytics(dir, 200); setEvents(r.events || []) } catch {}
      setLoading(false)
          ...
          {events.map((e, i) => (
            <div key={i} className="p-2 border rounded text-sm flex items-center justify-between">
              <div>
                ...
                {e.type === 'export' && <span>{e.copied} copied, {e.skipped} skipped{e.errors?`, ${e.errors} errors`:''}</span>}
                {e.type === 'backup_run' && <span>{e.copied} backed up, {e.skipped} skipped</span>}
                {e.type === 'backup_restore' && <span>{e.restored} restored</span>}
                {e.type === 'captions_build' && <span>{e.updated} captions</span>}
                {e.type === 'ocr_build' && <span>{e.updated} OCR</span>}
                {e.type === 'fast_build' && <span>{e.kind} index {e.ok?'ok':'failed'}</span>}
                {e.type === 'trips_build' && <span>{e.trips} trips</span>}
                {e.type === 'thumbs_build' && <span>{e.made} thumbs</span>}
                {e.type === 'metadata_build' && <span>{e.updated} metadata</span>}
                {e.type === 'share_created' && <span>link • {e.count} items</span>}
                {e.type === 'share_open' && <span>link opened</span>}
                {e.type === 'search' && <span>“{e.query}”</span>}
              </div>
              <div className="text-gray-500">{new Date(e.time).toLocaleString()}</div>
            </div>
          ))}
