...
const AnimatedPhotoGrid: React.FC<AnimatedPhotoGridProps> = ({
	...
	const [_viewTransition, setViewTransition] = useState(false);
			...
			>
				...
				>
					<div className="flex items-center gap-2">
						...
						>
							<Heart
								className={clsx("w-4 h-4", { "fill-current": photo.favorite })}
							/>
						</motion.button>
				...
				{/* Metadata Overlay */}
				{showMetadata &&
					(photo.date || photo.location || photo.people?.length) && (
						<motion.div
							initial={{ opacity: 0 }}
					...
					>
						...
						<div className={clsx("grid", gridSizeClasses[gridSize])}>
							{datePhotos.map((photo, index) =>
								renderPhoto(photo, groupIndex * 10 + index),
							)}
						</div>
					</motion.div>
				))}
