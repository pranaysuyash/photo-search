...
import { Download, X, FolderOpen, Image, FileArchive, Settings2 } from 'lucide-react';
      ...
      for (const batch of batches) {
        const result = await apiExport(
          ...
          exportConfig.createZip,
          {
            ...
            resizeLong: exportConfig.maxWidth > 0 ? exportConfig.maxWidth : undefined,
          }
        );
      ...
      <div className="bg-white dark:bg-gray-800 rounded-xl shadow-2xl max-w-3xl w-full max-h-[90vh] overflow-hidden">
          ...
          <div className="mb-6">
            ...
            <div className="grid grid-cols-4 gap-2">
              {Object.keys(formatSizes).map(preset => (
                <button
                  key={preset}
                  onClick={() => handlePresetSelect(preset as keyof typeof formatSizes)}
                  className={`px-4 py-2 rounded-lg border capitalize ${
                    exportConfig.maxWidth === formatSizes[preset as keyof typeof formatSizes].width
            ...
            <div className="space-y-4 pl-6">
              ...
              <div>
                ...
                <select
                  ...
                  onChange={(e) => setExportConfig(prev => ({ 
                    ...prev, 
                    folderStructure: e.target.value as typeof exportConfig.folderStructure 
                  }))}
                  className="w-full px-3 py-2 border rounded-lg dark:bg-gray-700 dark:border-gray-600"
                  disabled={exporting}
                >
        ...
        <div className="px-6 py-4 bg-gray-50 dark:bg-gray-900 border-t dark:border-gray-700 flex justify-between">
          <div className="text-sm text-gray-500 dark:text-gray-400">
            ...
            {exporting && `Exporting ${Math.round(progress * selectedPaths.length / 100)} of ${selectedPaths.length}`}
          </div>
          <div className="flex gap-2">
            <button
              onClick={onClose}
