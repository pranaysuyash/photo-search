const LS_KEYS = {
	...
	firstSearchShown: "first-search-shown",
	lastAdvancedSearchHint: "last-advanced-search-hint",
	lastBulkSelectHint: "last-bulk-select-hint",
	lastKeyboardShortcutsHint: "last-keyboard-shortcuts-hint",
	onboardingProgress: "onboarding-progress",
} as const;

const PRIORITY_ORDER: Record<HintConfig["priority"], number> = {
	high: 3,
	medium: 2,
	low: 1,
};
...
export function HintProvider({ children }: { children: React.ReactNode }) {
	...
	const showHint = React.useCallback(
		(hint: HintConfig) => {
			if (!hint?.id) return;
			try {
				const raw = localStorage.getItem(LS_KEYS.dismissedHints) || "[]";
			...
			setActiveHints((prev) => {
				const without = prev.filter((h) => h.id !== hint.id);
				return [...without, hint];
			});

			if (hint.autoHide) {
				const prevT = timeoutRefs.current[hint.id];
				if (prevT) window.clearTimeout(prevT);
				timeoutRefs.current[hint.id] = window.setTimeout(() => {
					dismissHint(hint.id);
				}, hint.autoHide * 1000);
			}
		},
		[isReducedMotion, dismissHint],
	);
	...
	const visibleHint = React.useMemo(() => {
		if (activeHints.length === 0) return undefined;
		const sorted = [...activeHints].sort(
			...
			switch (action) {
				case "app-loaded":
					...
					setTimeout(() => {
						try {
							const already =
								localStorage.getItem(LS_KEYS.firstSearchShown) === "1";
							...
							if (!already && hasSearch && !hasDialog) {
								...
								localStorage.setItem(LS_KEYS.firstSearchShown, "1");
							}
						} catch {
							/* noop */
						}
				...
				case "search-success": {
					...
					const lastAdvancedHint = localStorage.getItem(
						LS_KEYS.lastAdvancedSearchHint,
					);
					const now = Date.now();
					if (
						!lastAdvancedHint ||
						now - parseInt(lastAdvancedHint, 10) > 24 * 60 * 60 * 1000
					) {
						// 24 hours
						setTimeout(() => {
				...
				case "photo-selected": {
					...
					const recentBulkSelect = localStorage.getItem(
						LS_KEYS.lastBulkSelectHint,
					);
					if (
						!recentBulkSelect ||
						Date.now() - parseInt(recentBulkSelect, 10) >
							7 * 24 * 60 * 60 * 1000
					) {
						// 7 days
						setTimeout(() => {
				...
				case "keyboard-shortcut-used": {
					...
					const lastKeyboardHint = localStorage.getItem(
						LS_KEYS.lastKeyboardShortcutsHint,
					);
					if (
						!lastKeyboardHint ||
						Date.now() - parseInt(lastKeyboardHint, 10) >
							3 * 24 * 60 * 60 * 1000
					) {
						// 3 days
						setTimeout(() => {
				...
				case "first-collection-created":
					...
					setTimeout(() => {
						showHint({
							...
							action:
								"Collections help you organize photos by events or themes",
							position: "center",
							priority: "medium",
							autoHide: 8,
						});
				...
				case "advanced-search-used":
					...
					setTimeout(() => {
						showHint({
							...
							action:
								"Try combining filters like 'person:john AND place:beach'",
							position: "bottom",
							target: '[data-tour="search-bar"]',
							priority: "low",
							autoHide: 10,
				...
				case "feature-discovery":
					// Show hints for undiscovered features based on context
					{
						const contextObj = context as
							...
							| undefined;
						if (
							contextObj?.feature === "map" &&
							contextObj?.view === "results"
						) {
							setTimeout(() => {
								showHint({
...
export function useOnboardingProgress() {
	...
	useEffect(() => {
		const saved = localStorage.getItem(LS_KEYS.onboardingProgress);
		if (saved) {
			try {
	...
	const updateProgress = React.useCallback(
		(key: keyof typeof progress, value: boolean) => {
			const newProgress = { ...progress, [key]: value };
			setProgress(newProgress);
			try {
				localStorage.setItem(
					LS_KEYS.onboardingProgress,
