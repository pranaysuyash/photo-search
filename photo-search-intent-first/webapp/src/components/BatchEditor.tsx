...
export function BatchEditor({
  ...
  const [currentOperation, setCurrentOperation] =
    useState<BatchEditOperation | null>(null);
  ...
  const addAdjustmentOperation = useCallback(
    (adjustments: any) => {
      const operation: BatchEditOperation = {
        type: "adjustment",
  ...
  const addRenameOperation = useCallback(
    (pattern: string) => {
      const operation: BatchEditOperation = {
        type: "rename",
        name: "Batch Rename",
        ...
        for (const operation of batchOperations) {
          switch (operation.type) {
            ...
            case "adjustment": {
              ...
              currentImagePath = await analysisService.applyCreativeEdits(
                currentImagePath,
                edits
              );
              break;
            ...
            case "rename": {
              ...
              const newPath = applyRenamePattern(
                currentImagePath,
                operation.renamePattern!,
                i
              );
            ...
            case "tag":
              ...
              console.log(
                `Adding tags ${operation.tags?.join(
                  ", "
                )} to ${currentImagePath}`
              );
              break;
                ...
                {images.map((image, index) => (
                  <div
                    ...
                    className={`image-item ${
                      selectedImagesForBatch.has(image) ? "selected" : ""
                    }`}
                  ...
                  <CardContent>
                    <div className="rename-controls">
                      ...
                      >
                        ...
                        <SelectContent>
                          <SelectItem value="{name}_edited">
                            Add "_edited" suffix
                          </SelectItem>
                          <SelectItem value="{name}_{date}">
                            Add date suffix
                          </SelectItem>
                          <SelectItem value="batch_{index}">
                            Sequential numbering
                          </SelectItem>
                          <SelectItem value="photo_{index}_{date}">
                            Date + number
                          </SelectItem>
                        </SelectContent>
                      </Select>
              ...
              <div className="processing-status">
                  ...
                  <Progress
                    value={
                      (processingState.currentIndex /
                        processingState.totalImages) *
                      100
                    }
                    className="mb-2"
                  />
                  <p className="text-sm text-muted-foreground">
                    Processing {processingState.currentIndex + 1} of{" "}
                    {processingState.totalImages} images
                  </p>
                ...
                <div className="processing-stats">
                  <div className="stat">
                    ...
                    <span>
                      {processingState.completedImages.length} completed
                    </span>
                  </div>
                  <div className="stat">
