...
import { 
  ...
  Clock, Pause, Play, X, RotateCw, ChevronDown,
  ChevronUp, Trash2, Download, Upload, Search,
  Image, Database, Cpu
} from 'lucide-react'

export interface Job {
  id: string
  type: 'index' | 'search' | 'edit' | 'export' | 'import' | 'backup' | 'analyze' | 'restore'
  title: string
  description?: string
      ...
      {isOpen && (
        <div className="fixed bottom-20 right-4 w-96 max-h-[600px] bg-white dark:bg-gray-800 rounded-lg shadow-2xl z-40 flex flex-col">
          ...
          <div className="flex-1 overflow-y-auto">
            ...
            {activeJobs.length > 0 && (
              <div className="p-3">
                <h4 className="text-xs font-medium text-gray-500 uppercase mb-2">Working on</h4>
            ...
            {completedJobs.length > 0 && (
              <div className="p-3 border-t dark:border-gray-700">
                <h4 className="text-xs font-medium text-gray-500 uppercase mb-2">Recently finished</h4>
                <div className="space-y-2">
...
function JobItem({
  ...
  const progressPercentage = job.total ? (job.progress || 0) / job.total * 100 : 0
          ...
          <div className="flex-1 min-w-0">
              ...
              <div className="mt-2">
                <div className="flex items-center justify-between text-xs text-gray-500 mb-1">
                  <span>
                    ...
                    {job.type === 'backup' ? ' photos saved' : 
                     job.type === 'index' ? ' photos found' :
                     job.type === 'export' ? ' photos exported' : ' items'}
                  </span>
                ...
                {(job.estimatedTimeRemaining || job.speed) && (
                  <div className="flex items-center justify-between text-xs text-gray-400 mt-1">
                    {job.estimatedTimeRemaining && (
                      <span>About {formatTimeRemaining(job.estimatedTimeRemaining)} left</span>
                    )}
                    {job.speed && (
                      <span>{job.speed}</span>
                    )}
                ...
                {job.currentItem && (
                  <div className="text-xs text-gray-400 mt-1 truncate">
                    {job.type === 'backup' ? 'Saving: ' : 
                     job.type === 'index' ? 'Looking at: ' : 'Processing: '}
                    {job.currentItem}
                  </div>
                )}
              </div>
            ...
            {job.startTime && (
              <div className="text-xs text-gray-500 mt-1">
                {job.status === 'completed' ? 'Finished in ' : 
                 job.status === 'failed' ? 'Failed after ' : 
                 job.status === 'running' ? 'Running for ' : ''}
                {formatDuration(job.startTime, job.endTime)}
                {/* Success/Error counts for completed jobs */}
                {job.status === 'completed' && (
                  <span className="ml-2">
                    {job.successCount && <span className="text-green-500">✓ {job.successCount}</span>}
                    {job.warningCount && <span className="text-yellow-500 ml-1">⚠ {job.warningCount}</span>}
                    {job.errorCount && <span className="text-red-500 ml-1">✗ {job.errorCount}</span>}
                  </span>
                )}
              </div>
            )}
