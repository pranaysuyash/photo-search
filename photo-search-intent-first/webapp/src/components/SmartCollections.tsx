...
export default function SmartCollections({
  dir, engine, topK, smart, setSmart, setResults, setSearchId, setNote,
  query, favOnly, tagFilter, useCaps, useOcr, hasText, camera, isoMin, isoMax, fMin, fMax, place, persons
  ...
  const handleSaveCurrentAsSmart = async () => {
    ...
    try {
      ...
      const rules: any = {
        ...
        useCaptions: useCaps, useOcr, hasText,
    ...
    <div className="bg-white border rounded p-3">
      <div className="flex items-center justify-between">
        ...
        <div className="flex gap-2">
          <button onClick={handleRefreshSmart} className="bg-gray-200 rounded px-3 py-1 text-sm">Refresh</button>
          <button onClick={handleSaveCurrentAsSmart} className="bg-blue-600 text-white rounded px-3 py-1 text-sm">Save current as Smart</button>
        </div>
      </div>
          ...
          {Object.keys(smart).map((name)=> (
            <div key={name} className="border rounded p-2">
              <div className="flex items-center justify-between">
                ...
                <div className="flex gap-2">
                  <button onClick={()=>handleOpenSmart(name)} className="px-2 py-1 bg-blue-600 text-white rounded">Open</button>
                  <button onClick={()=>handleDeleteSmart(name)} className="px-2 py-1 bg-red-600 text-white rounded">Delete</button>
                </div>
              </div>
