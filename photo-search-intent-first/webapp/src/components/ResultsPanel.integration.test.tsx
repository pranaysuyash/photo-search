...
describe("ModularApp search integration", () => {
	it("performs a search and renders returned results", async () => {
		...
		await waitFor(() =>
			expect(vi.mocked(apiSearch)).toHaveBeenCalledWith(
				expect.any(String),
				"sunset",
				expect.any(String),
				expect.any(Number),
		...
		expect(
			await screen.findByRole("button", { name: "1.jpg" }),
		).toBeInTheDocument();
		expect(
			await screen.findByRole("button", { name: "2.jpg" }),
		).toBeInTheDocument();
		expect(
			await screen.findByRole("button", { name: "3.jpg" }),
		).toBeInTheDocument();
	});
});
