...
import { Play, Check, ArrowUpDown, Grid3X3, BarChart3, Calendar, Type, Star, Camera } from 'lucide-react'
...
export default function LibraryBrowser({
  dir, engine, library, onLoadLibrary, selected = new Set(), 
  onToggleSelect, onOpen, tagsMap = {}
}: LibraryBrowserProps) {
  ...
  const stats = useMemo(() => {
    ...
    library.forEach(path => {
      ...
      const ratingTag = tags.find(t => t.startsWith('rating:'))
      if (ratingTag) {
        const rating = parseInt(ratingTag.split(':')[1]) as keyof typeof ratings
  ...
  return (
    <div className="bg-white border rounded p-3">
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-2">
            ...
            <div>
              ...
              <div className="text-gray-600">
                {stats.selected > 0 && `${((stats.selected / stats.total) * 100).toFixed(1)}%`}
              </div>
            </div>
            <div>
              ...
              <div className="flex gap-1">
                {Object.entries(stats.ratings).map(([rating, count]) => (
                  <span key={rating} className="text-gray-600" title={`${rating} stars: ${count} photos`}>
                    {rating}★:{count}
                  </span>
                ))}
              </div>
      ...
      {/* Content */}
      {library.length === 0 ? (
        <div className="text-sm text-gray-600 mt-2">No items yet. Build the index, then click Reload.</div>
      ) : (
        <div className="mt-2 grid grid-cols-3 md:grid-cols-5 lg:grid-cols-8 gap-2">
          {sortedLibrary.map((p, i) => {
