...
export function CollaborativeWorkspace({
	...
	const startCollaboration = async () => {
		try {
			const sessionData = await sharingService.startCollaborationSession(albumId, userId, userName);
			setSession(sessionData);
			setActiveUsers(sessionData.participants.filter(p => p.isViewing));

			// Subscribe to real-time updates
			unsubscribeRef.current = sharingService.subscribeToAlbum(albumId, (data) => {
											...
											>
												...
												<div className="photo-overlay">
													<div className="photo-actions">
														...
														>
															<Heart className={`w-4 h-4 ${isLiked(photo) ? 'fill-current' : ''}`} />
														</Button>
					...
					<Card className="dialog-content">
						...
						<CardContent className="space-y-4">
							...
							<div className="dialog-actions">
								...
								<Button onClick={handleAddCollaborator} disabled={!newCollaboratorEmail.trim()}>
									Send Invitation
								</Button>
							</div>
						</CardContent>
...
function CommentItem({ comment, onReply, onLike, currentUserId }: CommentItemProps) {
		...
		<div className="comment-item">
			...
			<div className="comment-actions">
				...
				<Button variant="ghost" size="sm" onClick={onLike}>
					...
					{comment.likes > 0 && <span className="text-xs ml-1">{comment.likes}</span>}
				</Button>
			</div>
