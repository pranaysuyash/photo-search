...
import { apiListVideos, apiIndexVideos, apiGetVideoMetadata, videoThumbnailUrl } from '../api'
              ...
              {videos.map((video, index) => (
                ...
                >
                  ...
                  <div className="text-sm text-gray-500">
                    {formatFileSize(video.size)} • {new Date(video.mtime * 1000).toLocaleDateString()}
                  </div>
                </div>
              ))}
            </div>
            ...
            <div className="space-y-4">
              ...
              <div className="bg-gray-50 p-4 rounded space-y-2">
                ...
                <div className="text-sm space-y-1">
                  <div><span className="font-medium">Path:</span> {selectedVideo.path}</div>
                  <div><span className="font-medium">Size:</span> {formatFileSize(selectedVideo.size)}</div>
                  <div><span className="font-medium">Modified:</span> {new Date(selectedVideo.mtime * 1000).toLocaleString()}</div>
                </div>
                ...
                {videoMetadata && (
                  <>
                    ...
                    <div className="text-sm space-y-1">
                      <div><span className="font-medium">Dimensions:</span> {videoMetadata.width} × {videoMetadata.height}</div>
                      <div><span className="font-medium">Frame Rate:</span> {videoMetadata.fps?.toFixed(2)} fps</div>
                      <div><span className="font-medium">Duration:</span> {formatDuration(videoMetadata.duration)}</div>
                      <div><span className="font-medium">Frame Count:</span> {videoMetadata.frame_count?.toLocaleString()}</div>
                    </div>
                  </>
                )}
              </div>
