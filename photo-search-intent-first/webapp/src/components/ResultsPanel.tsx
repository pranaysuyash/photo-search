...
export default function ResultsPanel() {
	...
	const toggleSelect = useCallback(
		(p: string) => setSelected((s) => ({ ...s, [p]: !s[p] })),
		[],
	);
	...
	useEffect(() => {
		function onKey(e: KeyboardEvent) {
			...
			if (e.key === "Enter") {
				e.preventDefault();
				if (results[0]) setDetailIdx(0);
			}
			if (e.key === " ") {
				e.preventDefault();
				if (results[0]) toggleSelect(results[0].path);
			}
		}
	...
	return (
		<div className="glass-panel p-3">
				...
				<div className="flex gap-2 text-sm">
					...
					<button
						...
						onClick={() => {
							...
							if (settingsActions.setShowExplain) {
			...
			{busy?.toLowerCase().includes("search") && (
