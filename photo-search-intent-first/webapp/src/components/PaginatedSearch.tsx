...
import { apiSearchPaginated, type SearchResult } from "../api";
...
export function PaginatedSearch({
	...
	const searchWithPagination = useCallback(
		...
		[query, currentDir, provider, searchOptions, results, onResultsChange],
	);
	...
	}, [query, onResultsChange, searchWithPagination]);
			...
			<div className="flex items-center justify-between bg-gray-50 px-4 py-3 rounded">
				<div className="text-sm text-gray-600">
					{loading && pagination.offset === 0 ? (
						...
						<span>
							Showing {Math.min(results.length, pagination.total)} of{" "}
				...
				<div className="text-sm text-gray-600 bg-blue-50 px-4 py-2 rounded">
					...
					{pagination.total > 1000 && (
						<div className="mt-2 text-xs text-amber-600">
							💡 Large result set detected. Consider refining your search for
							better performance.
						</div>
					)}
				</div>
			...
			{results.length > 50 && (
				<div className="bg-yellow-50 border border-yellow-200 px-4 py-3 rounded">
					<div className="text-sm text-yellow-800">
						<strong>Performance Tip:</strong> You're viewing {results.length}{" "}
						results. For better performance with large result sets, consider:
