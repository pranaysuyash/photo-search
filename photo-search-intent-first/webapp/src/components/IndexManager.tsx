			...
			<div className="bg-white border rounded p-3">
				...
				<div className="grid grid-cols-2 md:grid-cols-4 gap-2">
					...
					<button
						...
						onClick={onBuildOCR}
						disabled={!dir || !!busy}
						className="bg-green-600 text-white rounded px-3 py-2 text-sm disabled:opacity-50"
					>
						Build OCR
					</button>
				</div>
				...
				<div className="grid grid-cols-2 gap-2 mt-2">
					...
					<button
						...
						onClick={onAutoTag}
						disabled={!dir || !!busy}
						className="bg-orange-600 text-white rounded px-3 py-2 text-sm disabled:opacity-50"
					>
						Auto Tag
					</button>
				</div>
			...
			<div className="bg-white border rounded p-3">
				...
				{diag ? (
					<div className="mt-2 text-sm">
						<div>
							Folder: <span className="font-mono">{diag.folder}</span>
						</div>
						<div>
							Free disk: {diag.free_gb} GB (OS: {diag.os})
						</div>
						<div className="mt-2">
							<div className="font-semibold mb-1">Indexes</div>
							{diag.engines.length === 0 ? (
								<div className="text-gray-600">No indexes found.</div>
							) : (
