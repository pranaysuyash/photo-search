...
import { Camera, Calendar, MapPin, Image, Settings, Star, Info, ChevronDown, ChevronRight } from 'lucide-react';
...
const METADATA_SECTIONS: MetadataSection[] = [
  ...
  },
  {
    ...
    icon: Settings,
    fields: [
      { key: 'aperture', label: 'Aperture', formatter: (v) => v ? `f/${v}` : undefined },
      { key: 'shutter_speed', label: 'Shutter Speed' },
      { key: 'iso', label: 'ISO', formatter: (v) => v ? `ISO ${v}` : undefined },
      { key: 'focal_length', label: 'Focal Length', formatter: (v) => v ? `${v}mm` : undefined },
  ...
  },
  {
    ...
    fields: [
      ...
      { key: 'megapixels', label: 'Megapixels', formatter: (v) => v ? `${v}MP` : undefined },
      { key: 'aspectRatio', label: 'Aspect Ratio' },
      { key: 'orientation', label: 'Orientation' },
      { key: 'color_space', label: 'Color Space' },
      { key: 'fileSize', label: 'File Size', formatter: (v) => metadataService.formatExifValue('file_size', v) }
    ]
  },
  {
    title: 'Date & Time',
    icon: Calendar,
    fields: [
      { key: 'date_taken', label: 'Date Taken', formatter: (v) => metadataService.formatExifValue('date_taken', v) },
      { key: 'date_modified', label: 'Modified', formatter: (v) => metadataService.formatExifValue('date_modified', v) }
    ]
  },
  {
    title: 'Location',
    icon: MapPin,
    fields: [
      { key: 'latitude', label: 'Latitude', formatter: (v) => v ? `${v.toFixed(6)}°` : undefined },
      { key: 'longitude', label: 'Longitude', formatter: (v) => v ? `${v.toFixed(6)}°` : undefined },
  ...
  },
  {
    ...
    fields: [
      ...
      { key: 'rating', label: 'Rating', formatter: (v) => v ? '★'.repeat(v) : undefined }
    ]
  }
];

export function MetadataPanel({ dir, path, onClose, compact = false }: MetadataPanelProps) {
  ...
  const renderSection = (section: MetadataSection) => {
    ...
    const hasData = section.fields.some(field => {
      const value = field.key === 'fileSize' || field.key === 'megapixels' || field.key === 'aspectRatio' 
        ? metadata?.[field.key]
      ...
      <div key={section.title} className="border-b border-gray-200 last:border-b-0">
        ...
        >
          <div className="flex items-center gap-2">
            <Icon className={`${compact ? 'w-3 h-3' : 'w-4 h-4'} text-gray-500`} />
            <span className={`font-medium ${compact ? 'text-sm' : ''}`}>{section.title}</span>
            {hasData && !isExpanded && compact && (
              <span className="text-xs text-gray-400">({section.fields.filter(f => {
                const value = f.key === 'fileSize' || f.key === 'megapixels' || f.key === 'aspectRatio' 
                  ? metadata?.[f.key]
            ...
            {section.fields.map(field => {
              ...
              return (
                <div key={field.key} className="flex justify-between items-start">
                  <span className="text-gray-600 min-w-0 flex-1">{field.label}:</span>
  ...
  if (loading) {
    return (
      <div className={`bg-white border ${compact ? 'rounded p-2' : 'rounded-lg p-4'}`}>
...
export function QuickMetadata({ 
  ...
  useEffect(() => {
    ...
    metadataService.getMetadata(dir, path).then(meta => {
