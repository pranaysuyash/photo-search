...
import { apiIndex, apiFeedback, apiGetFavorites, apiSetFavorite, apiGetSaved, apiAddSaved, apiDeleteSaved, apiDiagnostics, apiLibrary, apiBuildFast, apiBuildOCR, apiSearchLikePlus, apiWorkspaceList, apiWorkspaceAdd, apiWorkspaceRemove, apiMetadataDetail } from './api'
...
describe('api endpoints (more)', () => {
  it('posts index with batch size and tokens', async () => {
    ...
    expect(JSON.parse(init!.body)).toMatchObject({ dir: '/dir', provider: 'local', batch_size: 32, hf_token: 'hf', openai_key: 'oai' })
    spy.mockRestore()
  })
  ...
  it('feedback posts positives and note', async () => {
    ...
    expect(JSON.parse(init!.body)).toMatchObject({ dir: '/d', search_id: 'sid', query: 'q', positives: ['/a.jpg'], note: 'good' })
    spy.mockRestore()
  })
  ...
  it('favorites get and set', async () => {
    ...
    const [, init] = spy.mock.calls[0]
    expect(JSON.parse(init!.body)).toMatchObject({ dir: '/d', path: '/b', favorite: true })
    spy.mockRestore()
  })
  ...
  it('build endpoints post correct payloads', async () => {
    ...
    expect(JSON.parse(spy.mock.calls[0][1]!.body)).toMatchObject({ kind: 'faiss' })
    spy.mockRestore()
    spy = mockFetch({})
    await apiBuildOCR('/d', 'local', ['eng'], 'hf', 'oai')
    expect(JSON.parse(spy.mock.calls[0][1]!.body)).toMatchObject({ languages: ['eng'] })
    spy.mockRestore()
  })
  ...
  it('search like plus posts text and weight', async () => {
    ...
    expect(JSON.parse(spy.mock.calls[0][1]!.body)).toMatchObject({ text: 'caption', weight: 0.6 })
    spy.mockRestore()
  })
