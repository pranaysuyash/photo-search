...
import { useWorkspaceStore, useHasPersons, useFastIndexStatus } from './workspaceStore'

describe('workspaceStore', () => {
  beforeEach(() => {
    useWorkspaceStore.setState({
      workspace: [], wsToggle: false, persons: [], clusters: [], groups: [], points: [], diag: null,
    })
  })
  ...
  it('exposes fast index status from diagnostics', () => {
    function Fast() {
      ...
      return <div data-fast={fast ? 'yes' : 'no'} data-faiss={fast?.faiss ? '1' : '0'} />
    }
    const { container, rerender } = render(<Fast />)
    expect(container.firstChild).toHaveAttribute('data-fast', 'no')
    useWorkspaceStore.setState({
      diag: { folder: '/x', engines: [{ key: 'local', index_dir: '/idx', count: 10, fast: { annoy: true, faiss: false, hnsw: true } }], free_gb: 10, os: 'macOS' }
    })
    rerender(<Fast />)
