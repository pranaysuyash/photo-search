...
const SimpleStoreContext = createContext<{
  ...
  setPhoto: (photo: Partial<SimplePhoto>) => void
  setUI: (ui: Partial<SimpleUI>) => void
  setWorkspace: (workspace: Partial<SimpleWorkspace>) => void
} | null>(null)

export const SimpleStoreProvider: React.FC<{ children: React.ReactNode }> = ({ children }) => {
  ...
  const setWorkspace = useCallback((workspace: Partial<SimpleWorkspace>) => {
    setState(prev => ({ ...prev, workspace: { ...prev.workspace, ...workspace } }))
  }, [])

  return (
    <SimpleStoreContext.Provider value={{ state, setSettings, setPhoto, setUI, setWorkspace }}>
      {children}
...
export const useSimpleStore = () => {
  ...
  if (!context) throw new Error('useSimpleStore must be used within SimpleStoreProvider')
  return context
}
...
export const useNeedsHf = () => useSimpleStore().state.settings.engine.startsWith('hf')
export const useNeedsOAI = () => useSimpleStore().state.settings.engine === 'openai'
...
export const useFastIndexEnabled = () => useSimpleStore().state.settings.useFast
export const useFastKind = () => useSimpleStore().state.settings.fastKind
export const useOsTrashEnabled = () => useSimpleStore().state.settings.useOsTrash
