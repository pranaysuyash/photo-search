...
export class ExportAPI {
  ...
  static async getExportStatus(id: string): Promise<{ status: string; progress?: number; url?: string }> {
    const r = await fetch(`/export/status/${encodeURIComponent(id)}`);
    if (!r.ok) throw new Error(await r.text());
    return r.json();
  }

  static async listExports(dir: string): Promise<Array<{ id: string; type: string; status: string; created_at: string }>> {
    return get<Array<{ id: string; type: string; status: string; created_at: string }>>(`/exports?dir=${encodeURIComponent(dir)}`);
  }
...
export async function apiExport(dir: string, collection: string, format?: "json" | "csv") {
