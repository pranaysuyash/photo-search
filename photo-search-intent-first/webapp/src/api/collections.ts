import { API_BASE, del, get, post } from "./base";
import type { CollectionsResponse, SavedSearchesResponse } from "./types";
...
export class CollectionsAPI {
	...
	static async getPresets(dir: string): Promise<{ presets: unknown[] }> {
		const r = await fetch(`${API_BASE}/presets?dir=${encodeURIComponent(dir)}`);
		if (!r.ok) throw new Error(await r.text());
		return r.json() as Promise<{ presets: unknown[] }>;
	}
	...
	static async getSmartCollections(
		dir: string,
	): Promise<{ smart_collections: Record<string, unknown> }> {
		const r = await fetch(
	...
	static async deleteSmartCollection(
		dir: string,
		name: string,
	): Promise<{ ok: boolean }> {
		return del<{ ok: boolean }>(
			`/smart_collections/${encodeURIComponent(name)}?dir=${encodeURIComponent(dir)}`,
		);
	}
}
