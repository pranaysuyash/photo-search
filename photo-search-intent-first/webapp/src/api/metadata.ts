...
export class MetadataAPI {
  ...
  static async setFavorite(dir: string, path: string, favorite: boolean): Promise<{ ok: boolean }> {
  ...
  static async setTags(dir: string, paths: string[], tags: string[]): Promise<{ ok: boolean }> {
  ...
  static async getLibrary(dir: string, options?: { offset?: number; limit?: number }): Promise<LibraryResponse> {
  ...
  static async getMap(dir: string): Promise<MapResponse> {
    const r = await fetch(
      `${API_BASE}/map?dir=${encodeURIComponent(dir)}`,
    );
    if (!r.ok) throw new Error(await r.text());
  ...
  static async buildMetadata(params: BuildParams): Promise<{ ok: boolean; message?: string }> {
  ...
  static async buildOCR(params: BuildParams): Promise<{ ok: boolean; message?: string }> {
    const { dir, options = {} } = params;
  ...
  static async buildFast(params: BuildParams): Promise<{ ok: boolean; message?: string }> {
    const { dir, options = {} } = params;
  ...
  static async metadataBatch(params: MetadataBatchParams): Promise<{ ok: boolean }> {
  ...
  static async ocrStatus(dir: string): Promise<{ status: string; progress?: number }> {
    const r = await fetch(
...
export async function apiLibrary(dir: string, options?: { offset?: number; limit?: number }) {
...
export async function apiBuildMetadata(dir: string, options?: { force?: boolean; fast?: boolean }) {
  return MetadataAPI.buildMetadata({ dir, options });
}

export async function apiBuildOCR(dir: string, options?: { force?: boolean; fast?: boolean }) {
  return MetadataAPI.buildOCR({ dir, options });
}

export async function apiBuildFast(dir: string, options?: { force?: boolean; fast?: boolean }) {
  return MetadataAPI.buildFast({ dir, options });
}

export async function apiFacesClusters(dir: string, options?: { threshold?: number }) {
  return MetadataAPI.getFaceClusters({ dir, options });
}

export async function apiMetadataBatch(dir: string, paths: string[], operation: string, value?: any) {
