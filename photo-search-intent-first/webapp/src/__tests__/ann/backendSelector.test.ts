...
describe('BackendSelector', () => {
  ...
  describe('Backend Selection', () => {
    ...
    it('should select multiple backends for load balancing', async () => {
      ...
      const selections = await backendSelector.selectMultipleBackends(task, undefined, 3);
    ...
    it('should throw error when no backends available', async () => {
      ...
      await expect(backendSelector.selectBackend(task)).rejects.toThrow('No available backends for this task');
    });
  });
  ...
  describe('Performance Recording', () => {
    it('should record selection results', () => {
      ...
      expect(() => {
        backendSelector.recordSelectionResult(selection, actualPerformance, true);
      }).not.toThrow();
    });
