...
describe('BackendFactory', () => {
  ...
  describe('Backend Creation', () => {
    it('should create TensorFlow.js backend', async () => {
      const tfMock = vi.spyOn(TensorFlowJSBackend.prototype, 'initialize').mockResolvedValue(true);
    ...
    it('should create PyTorch backend', async () => {
      const ptMock = vi.spyOn(PyTorchBackend.prototype, 'initialize').mockResolvedValue(true);
    ...
    it('should create ONNX backend', async () => {
      const onnxMock = vi.spyOn(ONNXBackend.prototype, 'initialize').mockResolvedValue(true);
    ...
    it('should throw error for unsupported backend type', async () => {
      await expect(factory.createBackend('unsupported' as any))
        .rejects.toThrow('Unsupported backend type: unsupported');
    });

    it('should handle backend initialization failure', async () => {
      const tfMock = vi.spyOn(TensorFlowJSBackend.prototype, 'initialize').mockResolvedValue(false);

      await expect(factory.createBackend('tensorflowjs'))
        .rejects.toThrow('Failed to initialize backend: tensorflowjs');
  ...
  describe('Backend Management', () => {
    ...
    it('should shutdown backend', async () => {
      const tfMock = vi.spyOn(TensorFlowJSBackend.prototype, 'initialize').mockResolvedValue(true);
      const shutdownMock = vi.spyOn(TensorFlowJSBackend.prototype, 'shutdown').mockResolvedValue();
    ...
    it('should shutdown all backends', async () => {
      const tfMock = vi.spyOn(TensorFlowJSBackend.prototype, 'initialize').mockResolvedValue(true);
      const onnxMock = vi.spyOn(ONNXBackend.prototype, 'initialize').mockResolvedValue(true);
      const tfShutdownMock = vi.spyOn(TensorFlowJSBackend.prototype, 'shutdown').mockResolvedValue();
      const onnxShutdownMock = vi.spyOn(ONNXBackend.prototype, 'shutdown').mockResolvedValue();
  ...
  describe('Health Monitoring', () => {
    it('should get all backend health status', async () => {
      const tfMock = vi.spyOn(TensorFlowJSBackend.prototype, 'initialize').mockResolvedValue(true);
      const healthMock = vi.spyOn(TensorFlowJSBackend.prototype, 'getHealth').mockReturnValue({
  ...
  describe('Backend Recommendations', () => {
    it('should provide backend recommendations for different tasks', () => {
      const faceDetectionBackend = factory.getRecommendedBackend('face_detection');
      ...
      expect(['tensorflowjs', 'pytorch', 'onnx']).toContain(faceDetectionBackend || '');
      expect(['tensorflowjs', 'pytorch', 'onnx']).toContain(embeddingBackend || '');
      expect(['tensorflowjs', 'pytorch', 'onnx']).toContain(nlpBackend || '');
      expect(['tensorflowjs', 'pytorch', 'onnx']).toContain(defaultBackend || '');
    });
  });
  ...
  describe('Multiple Backend Creation', () => {
    ...
    it('should handle partial failures in multiple backend creation', async () => {
      const tfMock = vi.spyOn(TensorFlowJSBackend.prototype, 'initialize').mockResolvedValue(true);
      const onnxMock = vi.spyOn(ONNXBackend.prototype, 'initialize').mockRejectedValue(new Error('Failed'));
