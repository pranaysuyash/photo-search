...
describe("TensorFlowJSBackend", () => {
	...
	describe("Inference", () => {
		...
		it("should run batch inference", async () => {
			...
			const mockBatchTensor = {
				data: vi
					.fn()
					.mockResolvedValue([0.1, 0.9, 0.8, 0.2, 0.3, 0.7, 0.6, 0.4]),
				dispose: vi.fn(),
			...
			const { loadLayersModel, tensor, zeros } = await import(
				"@tensorflow/tfjs"
			);
			vi.mocked(loadLayersModel).mockResolvedValue(mockModel);
			vi.mocked(tensor).mockReturnValue(createMockTensor());
		...
		it("should throw error for non-existent model", async () => {
			...
			await expect(
				backend.runInference("non-existent-model", input),
	...
	describe("Performance", () => {
		...
		it("should optimize for specific tasks", async () => {
			await expect(
				backend.optimizeForTask("face_detection"),
			).resolves.not.toThrow();
			await expect(
				backend.optimizeForTask("image_embedding"),
			).resolves.not.toThrow();
			await expect(
				backend.optimizeForTask("object_detection"),
			).resolves.not.toThrow();
		});
	});
	...
	describe("Error Handling", () => {
		...
		it("should handle model loading errors", async () => {
			const { loadLayersModel } = await import("@tensorflow/tfjs");
			vi.mocked(loadLayersModel).mockRejectedValue(
				new Error("Model not found"),
			);

			await expect(
				backend.loadModel("test-model", {
					format: "tfjs" as const,
		...
		it("should handle inference errors", async () => {
			...
			await expect(backend.runInference("test-model", input)).rejects.toThrow(
				"Inference failed",
			);
		});
	});
