...
describe("ModelRegistry", () => {
	...
	describe("Model Registration", () => {
		...
		it("should throw error for invalid metadata", async () => {
			...
			await expect(
				modelRegistry.registerModel(invalidMetadata),
		...
		it("should throw error for duplicate model registration", async () => {
			...
			await expect(modelRegistry.registerModel(modelMetadata)).rejects.toThrow(
				"Model duplicate-model already exists",
			);
		});
	});
	...
	describe("Model Version Management", () => {
		...
		it("should add a new version to existing model", async () => {
			...
			await expect(
				modelRegistry.addModelVersion("versioned-model", newVersion),
			).resolves.not.toThrow();
		...
		it("should throw error for duplicate version", async () => {
			...
			await expect(
				modelRegistry.addModelVersion("versioned-model", version),
		...
		it("should throw error for non-existent model", async () => {
			...
			await expect(
				modelRegistry.addModelVersion("non-existent-model", version),
	...
	describe("Model Loading", () => {
		beforeEach(async () => {
			...
			modelRegistry.selectBackend = vi
				.fn()
				.mockResolvedValue("tensorflow-js-mock");
		});
		...
		it("should throw error for non-existent model", async () => {
			await expect(
				modelRegistry.loadModel("non-existent-model"),
			).rejects.toThrow("Model non-existent-model not found");
		});
		...
		it("should unload a model instance", async () => {
			const instanceId = await modelRegistry.loadModel("loadable-model");
			await expect(
				modelRegistry.unloadModel(instanceId),
			).resolves.not.toThrow();
		...
		it("should throw error for unloading non-existent instance", async () => {
			await expect(
				modelRegistry.unloadModel("non-existent-instance"),
	...
	describe("Model Recommendations", () => {
		...
		it("should recommend models based on task type", () => {
			const recommendations = modelRegistry.getRecommendedModels(
				"image_classification",
			);

			expect(recommendations).toHaveLength(2);
			expect(recommendations[0].performance.accuracy).toBeGreaterThan(
				recommendations[1].performance.accuracy,
			);
		});
		...
		it("should return empty recommendations for unsupported task", () => {
			const recommendations =
				modelRegistry.getRecommendedModels("unsupported_task");
