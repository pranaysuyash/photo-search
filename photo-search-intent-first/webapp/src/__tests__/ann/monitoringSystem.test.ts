...
import {
	...
	MonitoringSystem,
} from "../../services/ann/MonitoringSystem";
...
describe("MonitoringSystem", () => {
	...
	describe("Metrics Collection", () => {
		it("should record metrics with timestamps", () => {
			...
			monitoringSystem.recordMetric("cpu_usage", 75.5, timestamp, {
	...
	describe("Metrics Query", () => {
		beforeEach(() => {
			...
			monitoringSystem.recordMetric("cpu_usage", 50, hourAgo, {
				backend: "tensorflowjs",
			});
			monitoringSystem.recordMetric("cpu_usage", 75, hourAgo + 30 * 60 * 1000, {
				backend: "tensorflowjs",
			});
			monitoringSystem.recordMetric("cpu_usage", 60, now, {
				backend: "pytorch",
			});
		});
	...
	describe("Alert Management", () => {
		...
		it("should trigger alerts when conditions are met", () => {
			...
			monitoringSystem["createAlert"](rule, {
				test_value: 150,
				source: "test",
			});

			const recentAlerts = monitoringSystem.getRecentAlerts(60 * 60 * 1000); // 1 hour
			const testAlert = recentAlerts.find(
				(alert) => alert.ruleId === "test_alert",
			);
	...
	describe("Analytics and Insights", () => {
		it("should generate insights with all required components", () => {
			...
			const insights = monitoringSystem.generateInsights({
				start: dayAgo,
				end: now,
			});
	...
	describe("Subscription Management", () => {
		it("should allow event subscriptions", () => {
			...
			const subscriptionId = monitoringSystem.subscribe(
				"backend_health",
				mockCallback,
			);
