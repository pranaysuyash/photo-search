...
export class AdvancedQueryParser {
	...
	static parse(query: string): ParsedQuery {
		...
		const tokens = AdvancedQueryParser.tokenize(normalized, query);
		const context = AdvancedQueryParser.extractContext(tokens);
		const booleanExpression =
			AdvancedQueryParser.parseBooleanExpression(tokens);
		const expansions = AdvancedQueryParser.generateExpansions(tokens);
		const exclusions = AdvancedQueryParser.extractExclusions(tokens);
		const complexity = AdvancedQueryParser.assessComplexity(
			tokens,
			booleanExpression,
		);
	...
	private static extractContext(tokens: QueryToken[]): QueryContext {
		...
		for (const [timeKey, keywords] of Object.entries(
			AdvancedQueryParser.CONTEXT_KEYWORDS.time,
		)) {
		...
		for (const [seasonKey, keywords] of Object.entries(
			AdvancedQueryParser.CONTEXT_KEYWORDS.season,
		)) {
			if (keywords.some((kw) => tokenTexts.includes(kw))) {
		...
		for (const [locationKey, keywords] of Object.entries(
			AdvancedQueryParser.CONTEXT_KEYWORDS.location,
		)) {
			if (
				keywords.some((kw) => tokenTexts.some((token) => token.includes(kw)))
			) {
				context.locationContext =
					locationKey as QueryContext["locationContext"];
				break;
			}
		}

		// Check for activity context
		for (const [activityKey, keywords] of Object.entries(
			AdvancedQueryParser.CONTEXT_KEYWORDS.activity,
		)) {
			if (
				keywords.some((kw) => tokenTexts.some((token) => token.includes(kw)))
			) {
				context.activityContext =
					activityKey as QueryContext["activityContext"];
				break;
			}
		}

		// Check for quality context
		for (const [qualityKey, keywords] of Object.entries(
			AdvancedQueryParser.CONTEXT_KEYWORDS.quality,
		)) {
			if (
				keywords.some((kw) => tokenTexts.some((token) => token.includes(kw)))
			) {
		...
		for (const [moodKey, keywords] of Object.entries(
			AdvancedQueryParser.CONTEXT_KEYWORDS.mood,
		)) {
			if (
				keywords.some((kw) => tokenTexts.some((token) => token.includes(kw)))
			) {
	 ...
	 */
	private static parseBooleanExpression(
		tokens: QueryToken[],
		...
		return AdvancedQueryParser.parseComplexBoolean(tokens);
	}
	...
	private static parseComplexBoolean(tokens: QueryToken[]): BooleanExpression {
		...
		const rpn = AdvancedQueryParser.shuntingYard(tokens);
		return AdvancedQueryParser.evaluateRPN(rpn);
	}
	...
	private static shuntingYard(tokens: QueryToken[]): QueryToken[] {
		...
		for (const token of tokens) {
			if (
				...
				const opType = AdvancedQueryParser.getOperatorType(token.text);
				while (
					operators.length > 0 &&
					operators[operators.length - 1].type !== "group" &&
					(precedence[
						operators[operators.length - 1].text as keyof typeof precedence
					] > precedence[opType] ||
						(precedence[
							operators[operators.length - 1].text as keyof typeof precedence
						] === precedence[opType] &&
	...
	private static evaluateRPN(rpn: QueryToken[]): BooleanExpression {
		...
		if (
			stack.length === 1 &&
			AdvancedQueryParser.isBooleanExpression(stack[0])
		) {
		...
		return {
			type: "AND",
			operands: stack.filter(
				(t) => t.type === "keyword" || t.type === "include",
			),
			weight: 1.0,
		};
	}
	...
	private static isBooleanOperator(text: string): boolean {
		return Object.values(AdvancedQueryParser.BOOLEAN_OPERATORS)
			.flat()
			.includes(text);
	}

	private static isExcludeOperator(text: string): boolean {
		return AdvancedQueryParser.BOOLEAN_OPERATORS.NOT.includes(text);
	}
	...
	private static getOperatorType(text: string): "AND" | "OR" | "NOT" {
		if (AdvancedQueryParser.BOOLEAN_OPERATORS.NOT.includes(text)) return "NOT";
		if (AdvancedQueryParser.BOOLEAN_OPERATORS.AND.includes(text)) return "AND";
		if (AdvancedQueryParser.BOOLEAN_OPERATORS.OR.includes(text)) return "OR";
		return "AND"; // Default
	}
	...
	static expressionToString(expression: BooleanExpression): string {
		...
		if (expression.operands.length === 1) {
			...
			if (AdvancedQueryParser.isBooleanExpression(operand)) {
				return AdvancedQueryParser.expressionToString(operand);
			} else {
	...
	static getSuggestions(
		...
		if (!AdvancedQueryParser.isBooleanOperator(normalized)) {
			const lastWord = normalized.split(" ").pop() || "";
			if (lastWord && !AdvancedQueryParser.isBooleanOperator(lastWord)) {
