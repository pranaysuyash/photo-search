...
// Top-level sentinel to satisfy hook rules and stabilize deps
      ...
      async (entries) => {
        for (const e of entries) {
          if (e.isIntersecting && selectedView === "library" && !libLoading) {
            ...
            await onLoadMore(libLimit, next);
            setLibOffset(next);
            setLibLoading(false);
          }
        }
...
export default function ModernApp() {
  ...
  const topK = useTopK();
  const tags = useTags() as PhotoState["tags"];
  ...
  const loadLibrary = useCallback(
    ...
    [dir, engine, library, photoActions]
  );

  const loadMetadata = useCallback(async () => {
    try {
      if (!dir) return;
      const r = await (await import("./api")).apiGetMetadata(dir);
  ...
  const setRatingSelected = useCallback(
    async (rating: 1 | 2 | 3 | 4 | 5 | 0) => {
      ...
      try {
        ...
        await Promise.all(
          paths.map(async (p) => {
            const curr = (tags?.tagsMap?.[p] || []).filter((t) => !re.test(t));
    ...
    [dir, selected, tags, loadTags, uiActions]
  );
    ...
    <div className="p-4">
      ...
      <InfiniteSentinel
        ...
        onLoadMore={useCallback(
          (limit: number, next: number) => loadLibrary(limit, next, true),
          [loadLibrary]
        )}
