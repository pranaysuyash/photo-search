...
/*
 * SECURITY NOTE (SSRF Scanner Justification):
 * All network requests go through API_BASE which is validated (loopback or same-origin).
 * Paths passed to apiFetch() MUST start with '/'; user input is only ever appended as
 * query parameters or JSON bodies, never altering host or protocol. This file triggers
 * multiple generic SSRF Semgrep rules due to template literals with `${API_BASE}`.
 * Central validation + strict relative path requirement mitigates SSRF risk.
 * If future requirements introduce cross-origin requests, extend validateApiBase()
 * with an explicit allowlist array and document each additional host.
 * semgrep disable codacy.tools-configs.rules_lgpl_javascript_ssrf_rule-node-ssrf
 */
// nosemgrep: codacy.tools-configs.rules_lgpl_javascript_ssrf_rule-node-ssrf (central validated API_BASE prevents SSRF)
...
function computeApiBase(): string {
  ...
  // Electron: try dynamic base from main process via preload, cached in localStorage
  try {
    const w = window as WindowWithElectron;
    const electronEnv = isElectron();
    if (electronEnv && w.electronAPI?.getApiConfig) {
      // If we've already cached it this session, use it synchronously
      const cached = localStorage.getItem("api_base");
      if (cached) return cached;
      // Kick off async fetch to cache for subsequent calls; fall back to default for first paint
      w.electronAPI
        .getApiConfig()
        .then((cfg) => {
          if (cfg?.base) {
          ...
          if (cfg?.token) {
            try {
              localStorage.setItem("api_token", cfg.token);
            } catch {
              /* ignore */
            }
          }
        ...
        .catch(() => undefined);
      // When Electron injects config we respect that value regardless of env.
      // If preload cannot provide config we fall back below.
      if (!cached && import.meta.env?.DEV) {
        // Dev Electron builds still talk to the bundled API on loopback until preload responds.
        return DEFAULT_LOOPBACK_BASE;
      }
    } else if (import.meta.env?.DEV && !electronEnv) {
      // Browser dev server should default to the FastAPI loopback host.
      return DEFAULT_LOOPBACK_BASE;
    }
  } catch {
    /* ignore */
...
// Centralized, validated API base.
// SECURITY (SSRF Mitigation Rationale):
// 1. All dynamic network requests in this file prefix paths with API_BASE.
// 2. We constrain API_BASE at runtime to ONLY loopback (127.0.0.1/localhost) or the current
//    window.origin host. In packaged Electron builds the backend binds to 127.0.0.1 with a
//    random port and an ephemeral bearer token; on the web we intentionally allow same-origin.
// 3. No user-supplied input is ever concatenated into the host portion; user inputs are encoded
//    strictly as query parameters or JSON bodies.
// 4. This ensures Semgrep SSRF patterns that look for template string interpolation of
//    uncontrolled hosts are safe here. If additional remote bases are required in future, add
//    an explicit allowlist below.
function validateApiBase(raw: string): string {
  try {
    ...
    if (loopback.includes(host)) return raw.replace(/\/$/, "");
    // Allow same-origin host (for hosted web deployments) if it matches current location
    try {
      if (typeof window !== "undefined") {
        const loc = new URL(window.location.origin);
        if (loc.hostname === host) return raw.replace(/\/$/, "");
      }
    } catch {
      /* ignore */
    }
    // Fallback: reject and force loopback default to avoid widening attack surface
    return DEFAULT_LOOPBACK_BASE;
  } catch {
    return DEFAULT_LOOPBACK_BASE;
  }
...
export const API_BASE = validateApiBase(computeApiBase());
...
function authHeaders() {
  try {
    ...
    // ignore: localStorage access may throw in some privacy contexts
  }
...
function shouldInvalidateJsonCache(path: string): boolean {
  return JSON_CACHE_INVALIDATION_PATTERN.test(path);
}

function notifyJsonCacheInvalidation() {
...
export async function apiDemoDir(): Promise<string | null> {
  try {
    const r = await apiFetch(`/demo/dir`);
    if (!r.ok) return null;
...
// semgrep disable next line codacy.tools-configs.rules_lgpl_javascript_ssrf_rule-node-ssrf
// Rationale: Host portion already constrained in validateApiBase(); only relative paths allowed here.
// Unified safe fetch wrapper. We intentionally allow RequestInit (DOM lib) when available.
// Narrow internal init type (we only use method/headers/body in this module)
// Local type definitions to avoid DOM lib dependency
type LocalBodyInit =
  | string
  | Blob
  | ArrayBuffer
...
type EngineBridge = NonNullable<WindowWithElectron["engine"]>;
type EngineResponsePayload = Awaited<ReturnType<EngineBridge["request"]>>;

class EngineHeaders implements Iterable<[string, string]> {
...
class EngineFetchResponse {
  readonly ok: boolean;
  readonly status: number;
  readonly statusText: string;
  readonly headers: EngineHeaders;
  readonly url: string;
  readonly redirected = false;
  readonly type: ResponseType = "basic";
  private readonly payload: EngineResponsePayload;
  private bodyConsumed = false;

  constructor(payload: EngineResponsePayload, requestUrl: string) {
    this.payload = payload;
    this.ok = payload.ok;
    this.status = payload.status;
    this.statusText = payload.statusText ?? "";
    this.headers = new EngineHeaders(payload.headers);
    this.url = requestUrl;
  }

  get bodyUsed() {
    return this.bodyConsumed;
  }
  ...
  async json(): Promise<unknown> {
    this.markUsed();
    if (this.payload.bodyKind === "json") {
      return this.payload.body;
    }
    if (this.payload.bodyKind === "text") {
      try {
        return JSON.parse(String(this.payload.body ?? ""));
      } catch (error) {
        throw new Error(
          `Expected JSON response but failed to parse: ${
  ...
  async text(): Promise<string> {
    this.markUsed();
    if (this.payload.bodyKind === "json") {
      return JSON.stringify(this.payload.body ?? null);
    }
    if (this.payload.bodyKind === "none") {
      return "";
    }
  ...
  async formData(): Promise<FormData> {
    throw new Error("formData() not supported for engine responses");
  }

  clone(): EngineFetchResponse {
    return new EngineFetchResponse(
...
function getEngineBridge(): EngineBridge | null {
  try {
    const w = window as WindowWithElectron;
    return w.engine ?? null;
  } catch {
    return null;
  }
}

function serializeEngineBody(body: LocalBodyInit | undefined) {
  if (body === undefined || body === null) {
    return { payload: { kind: "none" } as const };
  }
  ...
  if (typeof FormData !== "undefined" && body instanceof FormData) {
    ...
    for (const [name, value] of body.entries()) {
      if (typeof value !== "string") {
        throw new Error(
          "Binary FormData entries are not supported in engine mode yet"
        );
      }
      entries.push({ name, value });
    }
    return { payload: { kind: "form-data", entries } as const };
  }

  if (typeof body === "string") {
    return { payload: { kind: "text", value: body } as const };
  }

  if (body instanceof URLSearchParams) {
    return { payload: { kind: "text", value: body.toString() } as const };
  }

  throw new Error("Unsupported body type for engine request");
}
...
async function apiFetch(path: string, init?: ApiFetchInit) {
  ...
  const engine = isElectron() ? getEngineBridge() : null;
  if (engine?.request) {
    const method = init?.method ?? "GET";
    const headers: Record<string, string> = { ...init?.headers };
    let payloadBody: ReturnType<typeof serializeEngineBody>["payload"];
    try {
      const serialized = serializeEngineBody(init?.body);
      payloadBody = serialized.payload;
    } catch (error) {
      return Promise.reject(error);
    }

    const responsePayload = await engine.request({
      path,
      method,
      headers,
      body: payloadBody,
    ...
    if (!responsePayload.ok && responsePayload.status === 0) {
      const message = responsePayload.message || "Engine request failed";
      throw new Error(message);
    }

    const requestUrl = `${API_BASE}${path}`;
    return new EngineFetchResponse(responsePayload, requestUrl);
  }

  return fetch(`${API_BASE}${path}`, init as LocalRequestInit);
}
async function post<T>(path: string, body: unknown): Promise<T> {
  ...
  if (shouldInvalidateJsonCache(path)) {
...
export async function apiIndexStatus(
  ...
  const r = await apiFetch(`/index/status?${qs.toString()}`);
...
/**
 * Post a granular analytics/progress event so Python or other backends
 * can stream job updates that the UI already polls via apiAnalytics().
 *
 * This is best-effort: backend should accept arbitrary JSON with at least
 * { type: string, dir: string }, and will timestamp server-side.
 */
export async function apiPostAnalyticsEvent(
  payload: AnalyticsEventRequest
): Promise<{ ok: boolean }> {
  const r = await apiFetch(`/analytics/event`, {
    method: "POST",
    headers: { "Content-Type": "application/json", ...authHeaders() },
...
export async function apiGetFavorites(dir: string) {
  const r = await apiFetch(`/favorites?dir=${encodeURIComponent(dir)}`);
  if (!r.ok) throw new Error(await r.text());
...
export async function apiGetSaved(dir: string) {
  const r = await apiFetch(`/saved?dir=${encodeURIComponent(dir)}`);
  if (!r.ok) throw new Error(await r.text());
  return r.json() as Promise<{
...
export async function apiGetPresets(dir: string) {
  const r = await apiFetch(`/presets?dir=${encodeURIComponent(dir)}`);
  if (!r.ok) throw new Error(await r.text());
...
export async function apiMap(dir: string) {
  const r = await apiFetch(`/map?dir=${encodeURIComponent(dir)}`);
  if (!r.ok) throw new Error(await r.text());
  return r.json() as Promise<{ points: { lat: number; lon: number }[] }>;
}

// Enhanced map API with clustering support
export async function apiMapClusters(
  dir: string,
  options: {
    zoom?: number;
  ...
  if (options.bounds) {
    params.append(
      "bounds",
      `${options.bounds.ne_lat},${options.bounds.ne_lon},${options.bounds.sw_lat},${options.bounds.sw_lon}`
    );
  }

  const r = await apiFetch(`/map/clusters?${params}`);
  if (!r.ok) throw new Error(await r.text());
  return r.json() as Promise<{
    clusters: MapCluster[];
    points: MapPoint[];
...
// Get detailed photos for a cluster
export async function apiClusterPhotos(
  dir: string,
  clusterId: string,
  options: {
    limit?: number;
  ...
  const r = await apiFetch(`/map/clusters/photos?${params}`);
  if (!r.ok) throw new Error(await r.text());
  return r.json() as Promise<{
    photos: ClusterPhoto[];
    total: number;
...
export async function apiDiagnostics(
  ...
  if (hfToken) qs.set("hf_token", hfToken);
  if (openaiKey) qs.set("openai_key", openaiKey);
  const r = await apiFetch(`/diagnostics?${qs.toString()}`);
  if (!r.ok) throw new Error(await r.text());
  return r.json() as Promise<{
    folder: string;
    engines: {
...
export async function apiLibrary(
  ...
  const r = await apiFetch(`/library?${qs.toString()}`);
  if (!r.ok) throw new Error(await r.text());
  return r.json() as Promise<{
    total: number;
    offset: number;
...
export async function apiLookalikes(dir: string, maxDistance = 5) {
  // nosemgrep: codacy.tools-configs.rules_lgpl_javascript_ssrf_rule-node-ssrf (encoded params; validated base)
  const r = await apiFetch(
    `/lookalikes?dir=${encodeURIComponent(dir)}&max_distance=${maxDistance}`
  );
  if (!r.ok) throw new Error(await r.text());
  return r.json() as Promise<{
...
export async function apiMetadataBatch(dir: string, paths: string[]) {
  // nosemgrep: codacy.tools-configs.rules_lgpl_javascript_ssrf_rule-node-ssrf
  const r = await apiFetch(
    `/metadata/batch?dir=${encodeURIComponent(dir)}&paths=${encodeURIComponent(
      paths.join(",")
    )}`
  );
  if (!r.ok) throw new Error(await r.text());
  return r.json() as Promise<{ ok: boolean; meta: Record<string, unknown> }>;
}

export async function apiMetadataDetail(dir: string, path: string) {
  const qs = new URLSearchParams({ dir, path });
  const r = await apiFetch(`/metadata/detail?${qs.toString()}`);
  if (!r.ok) throw new Error(await r.text());
  const body = (await r.json()) as
    | { meta: Record<string, unknown> }
    | Record<string, unknown>;
  // Normalize to always return { meta }
  if (body && typeof body === "object" && "meta" in body) {
    return body as { meta: Record<string, unknown> };
  }
  return { meta: body as Record<string, unknown> } as {
    meta: Record<string, unknown>;
  };
}

export async function apiThumbBatch(dir: string, paths: string[], size = 256) {
  return post<{ results: Record<string, { data?: string; error?: string }> }>(
    "/thumb/batch",
    {
      dir,
      paths,
...
export async function apiGetTags(dir: string) {
  const r = await apiFetch(`/tags?dir=${encodeURIComponent(dir)}`);
  if (!r.ok) throw new Error(await r.text());
...
export function thumbUrl(
  ...
  // Building relative URL only; consumer uses as img src. nosemgrep
...
export function thumbFaceUrl(
  ...
  // Same as above (face thumbnail) nosemgrep
...
export async function apiGetMetadata(dir: string) {
  const r = await apiFetch(`/metadata?dir=${encodeURIComponent(dir)}`);
  if (!r.ok) throw new Error(await r.text());
...
export async function apiTripsList(dir: string) {
  const r = await apiFetch(`/trips?dir=${encodeURIComponent(dir)}`);
  if (!r.ok) throw new Error(await r.text());
  return r.json() as Promise<{
    trips: {
      id: string;
...
export async function apiUndoDelete(dir: string) {
  const r = await apiFetch(`/undo_delete`, {
    method: "POST",
    headers: { "Content-Type": "application/json", ...authHeaders() },
...
export async function apiListVideos(dir: string) {
  const r = await apiFetch(`/videos?dir=${encodeURIComponent(dir)}`);
  if (!r.ok) throw new Error(await r.text());
  return r.json() as Promise<{
...
export function videoThumbnailUrl(
  ...
  // Video thumbnail URL generation (not a request here) nosemgrep
...
export async function apiSearchPaginated(
  ...
  const r = await apiFetch(`/search/paginated?${qs.toString()}`);
  if (!r.ok) throw new Error(await r.text());
  return r.json() as Promise<{
    search_id: string;
    results: SearchResult[];
...
export async function apiLibraryPaginated(
  ...
  const r = await apiFetch(`/library/paginated?${qs.toString()}`);
  if (!r.ok) throw new Error(await r.text());
  return r.json() as Promise<{
    total: number;
    offset: number;
...
export async function apiDataNuke(
  ...
  void _dir;
  void _all; // suppress unused warnings for stub
