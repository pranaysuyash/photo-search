...
import { BackendHealth, PerformanceMetrics } from './types';
...
export class HealthMonitor {
  ...
  async initialize(): Promise<boolean> {
    try {
      ...
      console.log('[HealthMonitor] Health monitoring system initialized successfully');
      return true;
    } catch (error) {
  ...
  getHealthSummary(): {
    ...
    const healthyBackends = histories.filter(h => h.currentStatus.status === 'healthy').length;
    const degradedBackends = histories.filter(h => h.currentStatus.status === 'degraded').length;
    const unhealthyBackends = histories.filter(h => h.currentStatus.status === 'unhealthy').length;
    const unknownBackends = histories.filter(h => h.currentStatus.status === 'unknown').length;

    const totalUptime = histories.reduce((sum, h) => sum + h.uptime, 0);
    const averageUptime = totalBackends > 0 ? totalUptime / totalBackends : 0;

    const totalResponseTime = histories.reduce((sum, h) => sum + h.averageResponseTime, 0);
    const averageResponseTime = totalBackends > 0 ? totalResponseTime / totalBackends : 0;
    ...
    try {
      ...
      if (this.config.enableMetricsCollection) {
        try {
          ...
          console.warn(`[HealthMonitor] Error getting metrics for ${backendId}:`, error);
        }
      }
      ...
      if (!isHealthy) {
        check.error = health.status === 'unhealthy' ? 'Backend reports unhealthy status' : 'Backend functionality test failed';
      }
  ...
  async attemptRecovery(backendId: string): Promise<boolean> {
    ...
    console.log(`[HealthMonitor] Attempting recovery for backend: ${backendId}`);

    try {
      // Try to reinitialize the backend
      const initialized = await backend.initialize();
      if (!initialized) {
        console.error(`[HealthMonitor] Failed to reinitialize backend ${backendId}`);
        return false;
      }
      ...
      if (recoverySuccessful) {
        console.log(`[HealthMonitor] Recovery successful for backend: ${backendId}`);
        this.emitEvent('backendRecovered', { backendId, check, timestamp: Date.now() });
      } else {
        console.error(`[HealthMonitor] Recovery failed for backend: ${backendId}`);
      }

      return recoverySuccessful;

    } catch (error) {
      console.error(`[HealthMonitor] Error during recovery for ${backendId}:`, error);
      return false;
    }
  }
  ...
  private async performHealthChecks(): Promise<void> {
    const checkPromises = Array.from(this.monitoringBackends.keys()).map(async (backendId) => {
      try {
        const check = await this.performHealthCheck(backendId);
        await this.processHealthCheck(check);
      } catch (error) {
        console.error(`[HealthMonitor] Health check failed for ${backendId}:`, error);
      }
    });
  ...
  private async processHealthCheck(check: HealthCheck): Promise<void> {
    ...
    history.averageResponseTime = recentChecks.reduce((sum, c) => sum + c.responseTime, 0) / recentChecks.length;
    ...
    if (oldStatus !== newStatus) {
      if (newStatus === 'healthy' && oldStatus !== 'healthy') {
        ...
        this.emitEvent('backendHealthy', { backendId: check.backendId, check, timestamp: Date.now() });
        ...
        this.emitEvent('backendUnhealthy', { backendId: check.backendId, check, timestamp: Date.now() });
      } else if (newStatus === 'degraded') {
        this.emitEvent('backendDegraded', { backendId: check.backendId, check, timestamp: Date.now() });
      }
    }
    ...
    this.emitEvent('healthCheckCompleted', { check, history, timestamp: Date.now() });
  }

  private determineHealthStatus(history: BackendHealthHistory): BackendHealth['status'] {
    const recentChecks = history.checks.slice(-10); // Last 10 checks
    const recentFailures = recentChecks.filter(c => c.status === 'failure').length;
    const recentSuccesses = recentChecks.filter(c => c.status === 'success').length;
    ...
    if (recentSuccesses >= this.config.recoveryThreshold && recentFailures > 0) {
      return 'degraded';
    }
  ...
  private checkAlertConditions(check: HealthCheck, history: BackendHealthHistory): void {
    ...
    // CPU usage alert
    if (check.metrics && check.metrics.cpuUsage && check.metrics.cpuUsage > thresholds.cpuUsage) {
      this.emitEvent('healthAlert', {
        backendId: check.backendId,
        type: 'cpu_usage',
        severity: 'warning',
  ...
  private initializeHealthHistory(backendId: string, backend: BaseBackend): void {
    ...
    if (listeners) {
      listeners.forEach(listener => {
        try {
          ...
          console.error(`[HealthMonitor] Error in event listener for ${event}:`, error);
        }
      });
    }
  }
