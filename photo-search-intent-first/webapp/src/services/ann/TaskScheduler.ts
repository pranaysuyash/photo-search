...
export interface SchedulerConfig {
  ...
  loadBalancingStrategy: 'round-robin' | 'least-loaded' | 'performance-based' | 'resource-aware';
...
export class TaskScheduler {
   ...
   */
  async submitTask(task: AITask, options?: {
    priority?: TaskPriority;
    timeout?: number;
    maxRetries?: number;
    metadata?: Record<string, any>;
    ...
    console.log(`[TaskScheduler] Task ${taskId} submitted (${task.type}, priority: ${queuedTask.priority})`);
    return taskId;
  }
   ...
   */
  async submitTasks(tasks: AITask[], options?: {
    priority?: TaskPriority;
    timeout?: number;
    maxRetries?: number;
    metadata?: Record<string, any>;
  ...
  getMetrics(): LoadBalancingMetrics {
    ...
    const completedHistory = Array.from(this.completedTasks.values()).filter(t => t.status === 'completed');
    const averageProcessingTime = completedHistory.length > 0
      ? completedHistory.reduce((sum, task) => sum + (task.endTime! - task.startTime!), 0) / completedHistory.length
      : 0;
  ...
  getQueueStatus(): {
    ...
    const oldestTaskAge = this.taskQueue.length > 0
      ? Date.now() - this.taskQueue[0].createdAt
      : 0;

    const estimatedProcessingTime = this.taskQueue.reduce((sum, task) => sum + task.estimatedDuration, 0);
  ...
  getBackendLoadDistribution(): Array<{
    ...
    return backends.map(backend => ({
      ...
      activeTasks: Array.from(this.activeTasks.values()).filter(task => task.assignedBackend === backend.id).length,
  ...
  checkAutoScaling(): ScalingDecision {
    ...
    if (metrics.resourceUtilization.cpu > this.config.scalingThresholds.cpuThreshold) {
      shouldScale = true;
      scaleDirection = 'up';
      reasons.push(`CPU usage (${metrics.resourceUtilization.cpu.toFixed(1)}%) exceeds threshold (${this.config.scalingThresholds.cpuThreshold}%)`);
      recommendedActions.push('Increase maxConcurrentTasks');
    }

    if (metrics.resourceUtilization.memory > this.config.scalingThresholds.memoryThreshold) {
      shouldScale = true;
      scaleDirection = 'up';
      reasons.push(`Memory usage (${metrics.resourceUtilization.memory.toFixed(1)}%) exceeds threshold (${this.config.scalingThresholds.memoryThreshold}%)`);
      recommendedActions.push('Increase available memory or reduce task concurrency');
    }

    if (this.taskQueue.length > this.config.scalingThresholds.queueLengthThreshold) {
      shouldScale = true;
      scaleDirection = 'up';
      reasons.push(`Queue length (${this.taskQueue.length}) exceeds threshold (${this.config.scalingThresholds.queueLengthThreshold})`);
      recommendedActions.push('Increase maxConcurrentTasks or add more backends');
    }
    ...
    return {
      ...
      currentLoad,
      targetLoad: scaleDirection === 'up' ? Math.min(currentLoad * 0.7, 70) : Math.max(currentLoad * 1.3, 40),
      recommendedActions
    };
  }
  ...
  private async processTaskQueue(): Promise<void> {
    ...
    const backendSelection = await this.selectBackendForTask(taskToExecute.task);
    if (!backendSelection) {
      ...
      this.taskQueue = this.taskQueue.filter(task => task.id !== taskToExecute.id);
      return;
    }

    // Execute task
    this.executeTask(taskToExecute, backendSelection.backend, backendSelection.model);
  }
  ...
  private findFairShareTask(): QueuedTask | null {
    ...
    for (const task of this.taskQueue) {
      ...
      const queueCount = this.taskQueue.filter(t => t.priority === task.priority).length;
   ...
   */
  private async selectBackendForTask(task: AITask): Promise<{ backend: string; model: string } | null> {
   ...
   * Execute a task
   */
  private async executeTask(task: QueuedTask, backendId: string, modelId: string): Promise<void> {
    // Update task status
    ...
    try {
      ...
      console.log(`[TaskScheduler] Task ${task.id} completed successfully (${(task.endTime - task.startTime!).toFixed(2)}ms)`);
    } catch (error) {
      task.status = 'failed';
      ...
      if (task.retryCount < task.maxRetries) {
        ...
        console.log(`[TaskScheduler] Task ${task.id} failed, retrying (${task.retryCount}/${task.maxRetries})`);
      } else {
        console.error(`[TaskScheduler] Task ${task.id} failed permanently:`, error);
      }
    } finally {
      // Update backend load
      this.updateBackendLoad(backendId, -1);
    ...
    for (const backend of backends) {
      ...
      if (health.status === 'unhealthy') {
        ...
        if (currentLoad > 0) {
          ...
          console.warn(`[TaskScheduler] Removed load from unhealthy backend: ${backend.id}`);
        }
      }
    }
  ...
  private applyScalingDecision(decision: ScalingDecision): void {
    console.log(`[TaskScheduler] Auto-scaling decision: ${decision.scaleDirection} - ${decision.reason}`);

    if (decision.scaleDirection === 'up') {
      // Scale up
      this.config.maxConcurrentTasks = Math.min(this.config.maxConcurrentTasks + 2, 50);
    } else {
      // Scale down
      this.config.maxConcurrentTasks = Math.max(this.config.maxConcurrentTasks - 1, 2);
    }

    console.log(`[TaskScheduler] Updated maxConcurrentTasks to ${this.config.maxConcurrentTasks}`);
  }
  ...
  private estimateTaskDuration(task: AITask): number {
    ...
    const complexityFactor = (task.resourceRequirements.memory.optimal / 256) * (task.resourceRequirements.cpu.optimal / 20);
  ...
  private calculateThroughput(): number {
    ...
    const recentTasks = Array.from(this.completedTasks.values())
      .filter(task => task.status === 'completed' && task.endTime! > oneHourAgo);
  ...
  private getResourceUtilization(): {
    ...
    return {
      cpu: ((resources.totalCPU - resources.availableCPU) / resources.totalCPU) * 100,
      memory: ((resources.totalMemory - resources.availableMemory) / resources.totalMemory) * 100,
      gpu: resources.totalGPU ? ((resources.totalGPU - resources.availableGPU!) / resources.totalGPU) * 100 : 0
    };
  }
