...
export class BackendRegistry {
	...
	registerBackend(backendId: string, backend: BaseBackend): void {
		...
		console.log(
			`[BackendRegistry] Backend ${backendId} registered successfully`,
		);
	}
	...
	unregisterBackend(backendId: string): void {
		...
		console.log(
			`[BackendRegistry] Backend ${backendId} unregistered successfully`,
		);
	}
	...
	getBackendsByCapability(
		...
		return backendIds
			...
			.filter(
				(backend): backend is BackendInfo =>
					backend !== undefined &&
	...
	findOptimalBackend(
		...
		const candidates = this.getBackendsByCapability(
			"inference",
			modelType,
		).filter((backend) => {
			// Check if backend supports the specific task type
			return backend.capabilities.some(
				(cap) =>
					cap.type === "inference" &&
	...
	getBackendStatistics(): {
		...
		backends.forEach((backend) => {
			backend.capabilities.forEach((cap) => {
				capabilityDistribution[cap.type] =
					(capabilityDistribution[cap.type] || 0) + 1;
			});
		});
	...
	validateBackendConfiguration(backendInfo: BackendInfo): string[] {
		...
		if (!reqs.memory || reqs.memory.min <= 0)
			errors.push("Memory requirements must be positive");
		if (!reqs.cpu || reqs.cpu.min <= 0)
			errors.push("CPU requirements must be positive");

		// Performance metrics validation
		const perf = backendInfo.performanceProfile;
		if (!perf.inferenceTime || perf.inferenceTime <= 0)
			errors.push("Inference time must be positive");
		if (!perf.memoryUsage || perf.memoryUsage < 0)
			errors.push("Memory usage must be non-negative");
	...
	importConfiguration(config: {
		...
		console.log(
			`[BackendRegistry] Imported configuration for ${config.backends.length} backends`,
		);
	}
		...
		if (listeners) {
			listeners.forEach((listener) => {
				try {
					...
					console.error(
						`[BackendRegistry] Error in event listener for ${event}:`,
						error,
					);
				}
			});
	...
	private calculateResourceScore(
		...
		if (constraints?.maxMemory) {
			const memoryRatio =
				backend.resourceRequirements.memory.optimal / constraints.maxMemory;
