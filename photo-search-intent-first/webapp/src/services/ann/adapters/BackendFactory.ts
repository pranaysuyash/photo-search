...
export class BackendFactory {
  ...
  registerBackend(type: BackendType, config: BackendConfig): void {
    ...
    console.log(`[BackendFactory] Registered backend configuration for: ${type}`);
  }

  /**
   * Create a new backend instance
   */
  async createBackend(type: BackendType, config?: Partial<BackendConfig>): Promise<BaseBackend> {
    const fullConfig = {
  ...
  updateBackendConfig(type: BackendType, config: Partial<BackendConfig>): void {
    ...
    if (existingConfig) {
      ...
      console.log(`[BackendFactory] Updated configuration for backend: ${type}`);
    }
  }
   ...
   */
  async createBackends(configs: Array<{
    type: BackendType;
    ...
    for (const { type, config } of configs) {
      try {
        ...
        console.error(`[BackendFactory] Failed to create backend ${type}:`, error);
    ...
    const shutdownPromises = Array.from(this.backends.entries()).map(
      async ([id, backend]) => {
        try {
          ...
          console.error(`[BackendFactory] Error shutting down backend ${id}:`, error);
        }
      }
    );
  ...
  getRecommendedBackend(taskType: string): BackendType | null {
    ...
    const taskRecommendations = recommendations[taskType] || recommendations.default;
