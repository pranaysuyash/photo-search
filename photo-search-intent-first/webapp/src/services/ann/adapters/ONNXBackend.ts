...
export class ONNXBackend extends BaseBackend {
  ...
  readonly capabilities: BackendCapability[] = [
    {
      ...
      classes: ['image-classification', 'object-detection', 'face-detection', 'embedding', 'audio', 'nlp'],
  ...
  isAvailable(): boolean {
    return this.isInitialized && this.health.status === 'healthy' && this.ort !== null;
  }
  ...
  async runBatchInference(modelId: string, inputs: InferenceInput[]): Promise<InferenceOutput[]> {
    ...
    try {
      ...
      console.error(`[ONNX] Batch inference failed for model ${modelId}:`, error);
      throw error;
    }
  }
  ...
  private async prepareInputTensor(input: InferenceInput, model: Model): Promise<any> {
    ...
    switch (format.type) {
      ...
      case 'array':
        return new this.ort.Tensor(
          'float32',
          new Float32Array(data),
          [1, ...this.calculateArrayShape(data)]
        );
      default:
  ...
  private async processOutput(results: any, model: Model): Promise<{
    data: any;
    dtype: string;
    shape: number[];
  }> {
  ...
  private async optimizeForFaceDetection(): Promise<void> {
    ...
    this.ort.env.wasm.numThreads = Math.min(navigator.hardwareConcurrency || 4, 2);
  }
  ...
  private async optimizeForObjectDetection(): Promise<void> {
    ...
    this.ort.env.wasm.numThreads = Math.min(navigator.hardwareConcurrency || 4, 3);
  }
