...
export class ModelRegistry {
	...
	async registerModel(metadata: ModelMetadata): Promise<void> {
		...
		if (
			currentResources.totalMemory < metadata.systemRequirements.minMemoryMB
		) {
			console.warn(
				`[ModelRegistry] System memory (${currentResources.totalMemory}MB) below requirement (${metadata.systemRequirements.minMemoryMB}MB) for model ${metadata.id}`,
			);
		}
		...
		console.log(
			`[ModelRegistry] Registered model: ${metadata.id} v${metadata.version}`,
		);
	}
	...
	async addModelVersion(modelId: string, version: ModelVersion): Promise<void> {
		...
		if (version.version === model.version) {
			throw new Error(
				`Version ${version.version} already exists as default version`,
			);
		}
		...
		console.log(
			`[ModelRegistry] Added version ${version.version} to model ${modelId}`,
		);
	}
	...
	async loadModel(
		...
		const backendInstances =
			this.loadedModels.get(selectedBackend) || new Map();
	...
	private async processLoadingQueue(): Promise<void> {
		...
		this.currentLoads++;
		try {
			await this.performModelLoad(
				loadRequest.modelId,
				loadRequest.version,
				loadRequest.backendId,
			);
		} catch (error) {
			console.error(
				`[ModelRegistry] Failed to load model ${loadRequest.modelId}:`,
				error,
			);
		} finally {
			this.currentLoads--;
	...
	private async performModelLoad(
		...
		try {
			...
			console.log(
				`[ModelRegistry] Loaded model ${modelId} v${version} on backend ${backendId} (${instance.loadTime.toFixed(2)}ms)`,
			);
		} catch (error) {
	...
	async unloadModel(instanceId: string): Promise<void> {
		...
		try {
			...
			console.error(
				`[ModelRegistry] Error unloading model instance ${instanceId}:`,
				error,
			);
			throw error;
		}
	...
	searchModels(criteria: ModelSearchCriteria): ModelMetadata[] {
		...
		for (const model of this.models.values()) {
			...
			if (
				criteria.taskType &&
				!model.supportedTasks.includes(criteria.taskType)
			) {
				matches = false;
			}
			...
			// Check format
			if (criteria.format) {
				const formats = Array.isArray(criteria.format)
					? criteria.format
					: [criteria.format];
			...
			if (criteria.backend) {
				const backends = Array.isArray(criteria.backend)
					? criteria.backend
					: [criteria.backend];
				const hasCompatibleBackend = backends.some(
					(backend) =>
						model.backendRequirements[
							backend as keyof typeof model.backendRequirements
						],
				);
			...
			if (
				criteria.maxMemoryMB &&
				model.systemRequirements.minMemoryMB > criteria.maxMemoryMB
			) {
				matches = false;
			}
			...
			if (
				...
				model.performance.accuracy < criteria.minAccuracy
			) {
				matches = false;
			}
			...
			if (
				criteria.maxInferenceTime &&
				model.performance.inferenceTime > criteria.maxInferenceTime
			) {
				matches = false;
			}
	...
	getUsageStats(modelId?: string): ModelUsageStats[] {
		if (modelId) {
			const stats = Array.from(this.usageStats.values()).filter(
				(stat) => stat.modelId === modelId,
			);
			return stats;
		}
	...
	recordInference(
		...
		if (success) {
			...
			stats.errorCount++;
			stats.successRate =
				(stats.totalInferences - stats.errorCount) / stats.totalInferences;
		}
	...
	getSystemResourceUsage(): {
		...
		const totalMemoryMB = instances.reduce(
		...
		for (const instance of instances) {
			backendDistribution[instance.backendId] =
				(backendDistribution[instance.backendId] || 0) + 1;
			modelDistribution[instance.modelId] =
				(modelDistribution[instance.modelId] || 0) + 1;
		}
	...
	async optimizeLoadedModels(): Promise<void> {
		...
		for (const instance of instances) {
			if (
				now - instance.lastUsed > unusedThreshold &&
				instance.usageCount < 5
			) {
		...
		for (const stats of frequentModels) {
			...
			if (!instance) {
				try {
					...
					console.log(
						`[ModelRegistry] Pre-loaded frequent model: ${stats.modelId}`,
					);
				} catch (error) {
					console.error(
						`[ModelRegistry] Failed to pre-load model ${stats.modelId}:`,
						error,
					);
				}
			}
	...
	importRegistry(data: {
		...
		if (data.instances) {
			for (const instance of data.instances) {
				...
				if (instance.loaded) {
					const backendInstances =
						this.loadedModels.get(instance.backendId) || new Map();
		...
		const scoredBackends = availableBackends
			...
			.map((backend) => {
				...
				const resourceScore = Math.min(
					100,
					(currentResources.availableMemory / memoryBaseline) * 50,
				);
				score += resourceScore;
	...
	private getRequirementKeyForBackend(
		backendId: string,
	): keyof ModelMetadata["backendRequirements"] | null {
