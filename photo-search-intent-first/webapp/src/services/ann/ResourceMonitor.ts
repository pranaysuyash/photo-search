...
import type { NetworkStatus, ResourceUsage, SystemResources } from "./types";
...
export class ResourceMonitor {
	...
	async initialize(): Promise<boolean> {
		try {
			...
			console.log(
				"[ResourceMonitor] Resource monitoring initialized successfully",
			);
			return true;
		} catch (error) {
	...
	updateResources(resources: Partial<SystemResources>): void {
		...
		// Update resources
		if (resources.totalMemory !== undefined)
			this.currentResources.totalMemory = resources.totalMemory;
		if (resources.availableMemory !== undefined)
			this.currentResources.availableMemory = Math.max(
				0,
				resources.availableMemory,
			);
		if (resources.totalCPU !== undefined)
			this.currentResources.totalCPU = resources.totalCPU;
		if (resources.availableCPU !== undefined)
			this.currentResources.availableCPU = Math.max(
				0,
				Math.min(100, resources.availableCPU),
			);
		if (resources.totalGPU !== undefined)
			this.currentResources.totalGPU = resources.totalGPU;
		if (resources.availableGPU !== undefined)
			this.currentResources.availableGPU = resources.availableGPU;
		if (resources.totalStorage !== undefined)
			this.currentResources.totalStorage = resources.totalStorage;
		if (resources.availableStorage !== undefined)
			this.currentResources.availableStorage = Math.max(
				0,
				resources.availableStorage,
			);
		if (resources.network !== undefined)
			this.currentResources.network = resources.network;
	...
	private async collectResourceMetrics(): Promise<SystemResources> {
		...
		return {
			totalMemory:
				metrics[0].status === "fulfilled"
					? metrics[0].value.total
					: this.currentResources.totalMemory,
			availableMemory:
				metrics[0].status === "fulfilled"
					? metrics[0].value.available
					: this.currentResources.availableMemory,
			totalCPU:
				metrics[1].status === "fulfilled"
					? metrics[1].value.total
					: this.currentResources.totalCPU,
			availableCPU:
				metrics[1].status === "fulfilled"
					? metrics[1].value.available
					: this.currentResources.availableCPU,
			totalGPU:
				metrics[2].status === "fulfilled"
					? metrics[2].value?.total
					: this.currentResources.totalGPU,
			availableGPU:
				metrics[2].status === "fulfilled"
					? metrics[2].value?.available
					: this.currentResources.availableGPU,
			totalStorage:
				metrics[3].status === "fulfilled"
					? metrics[3].value.total
					: this.currentResources.totalStorage,
			availableStorage:
				metrics[3].status === "fulfilled"
					? metrics[3].value.available
					: this.currentResources.availableStorage,
			network:
				metrics[4].status === "fulfilled"
					? metrics[4].value
					: this.currentResources.network,
		};
	}
	...
	private async collectMemoryMetrics(): Promise<{
		...
		if (!this.config.enableMemoryMonitoring) {
			return {
				total: this.currentResources.totalMemory,
				available: this.currentResources.availableMemory,
			};
		}
		...
		try {
			...
			if ("deviceMemory" in navigator) {
				const totalGB = (navigator as unknown).deviceMemory || 4;
				return {
	...
	private async collectCPUMetrics(): Promise<{
		...
		if (!this.config.enableCPUMonitoring) {
			return {
				total: this.currentResources.totalCPU,
				available: this.currentResources.availableCPU,
			};
		}
		...
		try {
			...
			if ("measureMemory" in performance) {
				try {
					const memory = await (performance as unknown).measureMemory();
					// This is a rough estimate based on memory pressure
					estimatedUsage = Math.min(
						100,
						Math.max(0, (memory.bytes / (1024 * 1024 * 100)) * 10),
					);
				} catch (error) {
	...
	private async collectGPUMetrics(): Promise<{
		...
		try {
			...
			if ("gpu" in navigator) {
				const gpu = (navigator as unknown).gpu;
				if (gpu) {
			...
			try {
				if (typeof document !== "undefined" && document.createElement) {
					...
					if (canvas && typeof canvas.getContext === "function") {
						...
						if (gl) {
							...
							try {
								const debugInfo = (gl as unknown).getExtension(
				...
				console.warn(
					"[ResourceMonitor] Canvas not available for GPU detection",
				);
			}
	...
	private async collectStorageMetrics(): Promise<{
		total: number;
		available: number;
	}> {
		try {
			if (
				...
				"estimate" in (navigator as unknown).storage
			) {
				const estimate = await (navigator as unknown).storage.estimate();
				return {
					total: Math.round((estimate.quota || 0) / 1024 / 1024), // MB
					available: Math.round(
						(estimate.quota - (estimate.usage || 0)) / 1024 / 1024,
					), // MB
				};
			}
			...
			try {
				...
				const availableMB = 100; // Default 100MB
				const totalMB = 1000; // Default 1GB
			...
			console.warn(
				"[ResourceMonitor] Error collecting storage metrics:",
				error,
			);
			return {
				total: this.currentResources.totalStorage,
				available: this.currentResources.availableStorage,
			};
		}
	}
	...
	private async collectNetworkMetrics(): Promise<NetworkStatus | undefined> {
		...
		try {
				...
				if (navigation) {
					...
					if (navigation.transferSize > 0) {
						bandwidth = Math.max(
							1,
							Math.round(
								(navigation.transferSize * 8) / (fetchTime / 1000) / 1024,
							),
						);
					}
				}
			...
			console.warn(
				"[ResourceMonitor] Error collecting network metrics:",
				error,
			);
			return this.currentResources.network;
		}
	...
	private calculateResourceUsage(): ResourceUsage {
		...
		return {
			...
			cpu:
				totalCPU > 0
					? Math.round(((totalCPU - availableCPU) / totalCPU) * 100)
					: 0,
			gpu:
				totalGPU && totalGPU > 0
					? Math.round(((totalGPU - (availableGPU || 0)) / totalGPU) * 100)
					: undefined,
			storage:
				totalStorage > 0
					? Math.round(((totalStorage - availableStorage) / totalStorage) * 100)
					: 0,
	...
	private async detectSystemCapabilities(): Promise<void> {
		...
		if (typeof navigator === "undefined" || typeof document === "undefined") {
			console.log(
				"[ResourceMonitor] Running in test environment, skipping capability detection",
			);
			return;
		}
		...
		try {
			...
			try {
				...
				console.warn(
					"[ResourceMonitor] Canvas not available for WebGL detection:",
					error,
				);
			}
			...
			this.emitEvent("capabilitiesDetected", {
				capabilities,
				timestamp: Date.now(),
			});
		} catch (error) {
			console.warn(
				"[ResourceMonitor] Error during capability detection:",
				error,
			);
		}
	}
		...
		if (listeners) {
			listeners.forEach((listener) => {
				try {
					...
					console.error(
						`[ResourceMonitor] Error in event listener for ${event}:`,
						error,
					);
				}
			});
