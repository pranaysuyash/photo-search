...
export class DecisionEngine {
	...
	async makeDecision(
		...
		const fairSelection = this.applyFairnessConstraints(
			selected,
			scoredCandidates,
			fullContext,
		);
	...
	async makeBatchDecisions(
		...
		const loadBalancedDecisions = await this.optimizeBatchDecisions(
			tasks,
			fullContext,
		);

		for (const decision of loadBalancedDecisions) {
			const fairDecision = this.applyFairnessConstraints(
				decision,
				[],
				fullContext,
			);
			...
			this.recordDecision(
				tasks[decisions.indexOf(decision)],
				fairDecision,
				fullContext,
			);
		}
	...
	recordTaskResult(
		...
		const learningEntry = this.learningData.find(
			(data) => data.taskId === taskId && data.backendId === backendId,
		);
	...
	async optimizeWeights(
		performanceGoal: "speed" | "efficiency" | "balance" = "balance",
	): Promise<void> {
		console.log(
			`[DecisionEngine] Optimizing weights for ${performanceGoal}...`,
		);
		...
		for (let i = 0; i < optimizationIterations; i++) {
			...
			const simulatedPerformance =
				this.simulatePerformanceWithWeights(currentWeights);
			const currentScore = this.calculateObjectiveScore(
			...
			const perturbedPerformance =
				this.simulatePerformanceWithWeights(perturbedWeights);
			const perturbedScore = this.calculateObjectiveScore(
				perturbedPerformance,
				performanceGoal,
			);
	...
	getDecisionAnalytics(): {
		...
		const successfulDecisions = this.learningData.filter(
			(d) => d.actualPerformance.accuracy > 0.8,
		).length;
		const averageConfidence =
			this.learningData.reduce((sum, d) => sum + d.decision.confidence, 0) /
			totalDecisions;

		const backendDistribution: Record<string, number> = {};
		this.learningData.forEach((data) => {
			backendDistribution[data.backendId] =
				(backendDistribution[data.backendId] || 0) + 1;
		});
	...
	private async generateCandidates(
		context: DecisionContext,
	): Promise<Array<{ backendId: string; confidence: number }>> {
		const candidates: Array<{ backendId: string; confidence: number }> = [];

		// Use backend selector to get initial candidates
		const selection = await this.backendSelector.selectMultipleBackends(
			context.task,
			undefined,
			5,
		);
	...
	private async scoreCandidates(
		...
		const scoredCandidates = await Promise.all(
			candidates.map(async (candidate) => {
				const score = await this.calculateCandidateScore(
					candidate.backendId,
					context,
				);
				return {
	...
	private async calculateCandidateScore(
		...
		const total = Object.entries(scores).reduce((sum, [key, score]) => {
			return sum + score * (this.weights as unknown)[key];
		}, 0);
	...
	private calculateReliabilityScore(
		...
		historicalPerf?: PerformanceHistory,
	): number {
		let healthScore = 0;
		...
		const uptimeScore = Math.min(
			1,
			backendHealth.uptime / (24 * 60 * 60 * 1000),
		); // 24 hours
	...
	private calculateEfficiencyScore(
		...
		const availableMemoryRatio =
			systemResources.availableMemory / systemResources.totalMemory;
		const availableCPURatio =
			systemResources.availableCPU / systemResources.totalCPU;
	...
	private calculateAdaptabilityScore(
		...
		const inferenceTimes = recentSelections.map(
			(s) => s.actualPerformance.inferenceTime,
		);
		const meanInferenceTime =
			inferenceTimes.reduce((sum, time) => sum + time, 0) /
			inferenceTimes.length;
		const variance =
			inferenceTimes.reduce(
				(sum, time) => sum + (time - meanInferenceTime) ** 2,
				0,
			) / inferenceTimes.length;
	...
	private calculateFairnessScore(
		...
		const totalDecisions = this.learningData.length;
		const backendUsage = this.learningData.filter(
			(d) => d.backendId === backendId,
		).length;
		const expectedUsage =
			totalDecisions / Object.keys(context.backendHealth).length;
	...
	private applyFairnessConstraints(
		...
		if (fairnessMetrics.maxDisparity > this.fairnessConstraints.maxDisparity) {
			...
			const underutilized = candidates.filter(
				(c) =>
					fairnessMetrics.backendUsage[c.backendId] <
					this.fairnessConstraints.minRepresentation,
			);
	...
	private async getBackendHealthStatus(): Promise<
	...
	private calculateCurrentLoad(): Map<string, number> {
		...
		const recentTasks = this.learningData.filter(
	...
	private async optimizeBatchDecisions(
		tasks: AITask[],
		context: DecisionContext,
	): Promise<BackendSelection[]> {
		...
		for (const task of tasks) {
			...
			backendLoads.set(
				decision.backend,
				(backendLoads.get(decision.backend) || 0) + 1,
			);
		}
	...
	private generateReasoning(
		best: { backendId: string; score: number; details: any },
		context: DecisionContext,
	): unknown[] {
		return [
			{
	...
	private async retrainModels(): Promise<void> {
		...
		const successfulDecisions = recentData.filter(
			(d) => d.actualPerformance.accuracy > 0.8,
		);
		const failedDecisions = recentData.filter(
			(d) => d.actualPerformance.accuracy <= 0.8,
		);
	...
	private adjustWeightsBasedOnResults(
		successful: LearningData[],
		failed: LearningData[],
	): void {
		// Analyze patterns in successful vs failed decisions
		const avgSuccessfulPerformance =
			successful.reduce(
				(sum, d) => sum + this.calculateDecisionScore(d.decision),
				0,
			) / successful.length;
		const avgFailedPerformance =
			failed.reduce(
				(sum, d) => sum + this.calculateDecisionScore(d.decision),
				0,
			) / failed.length;
		...
		const adjustmentFactor =
			this.adaptationRate * (avgSuccessfulPerformance - avgFailedPerformance);

		// Apply adjustments (simplified)
		this.weights.performance = Math.max(
			0,
			Math.min(1, this.weights.performance + adjustmentFactor),
		);
		this.weights.reliability = Math.max(
			0,
			Math.min(1, this.weights.reliability + adjustmentFactor * 0.8),
		);
	}
	...
	private perturbWeights(weights: DecisionWeights): DecisionWeights {
		...
		Object.keys(perturbed).forEach((key) => {
			const value = (perturbed as unknown)[key];
			const perturbation = (Math.random() - 0.5) * perturbationAmount * 2;
			(perturbed as unknown)[key] = Math.max(
				0,
				Math.min(1, value + perturbation),
			);
		});
	...
	private getWeightEvolution(): Array<{
		timestamp: number;
	...
	private calculateAccuracyTrend(data: LearningData[]): number {
		...
		const recentAccuracy =
			data
				.slice(-10)
				.reduce((sum, d) => sum + d.actualPerformance.accuracy, 0) / 10;
	...
	private calculateConvergenceRate(data: LearningData[]): number {
		...
		const variance =
			accuracies.reduce((sum, acc) => sum + (acc - 0.85) ** 2, 0) /
			accuracies.length;
	...
	private calculateModelStability(): number {
		...
		const mean =
			weightValues.reduce((sum, val) => sum + val, 0) / weightValues.length;
		const variance =
			weightValues.reduce((sum, val) => sum + (val - mean) ** 2, 0) /
			weightValues.length;
