...
export class BackendSelector {
  ...
  private performanceHistory: Map<string, BackendPerformanceHistory> = new Map();
  ...
  async selectBackend(task: AITask, criteria?: Partial<SelectionCriteria>): Promise<BackendSelection> {
  ...
  async selectMultipleBackends(task: AITask, criteria?: Partial<SelectionCriteria>, count: number = 3): Promise<BackendSelection[]> {
    ...
    for (let i = 1; i < count && i < selection.fallbacks.length + 1; i++) {
      const backendId = i === 1 ? selection.backend : selection.fallbacks[i - 2];
  ...
  recordSelectionResult(selection: BackendSelection, actualPerformance: PerformanceMetrics, success: boolean): void {
    ...
    history.averageInferenceTime = recentSelections.reduce((sum, s) => sum + s.actualPerformance.inferenceTime, 0) / recentSelections.length;
    history.averageAccuracy = successfulSelections.reduce((sum, s) => sum + (s.actualPerformance.accuracy || 0), 0) / successfulSelections.length;
  ...
  getSelectionMetrics(): {
    ...
    const allSelections = Array.from(this.performanceHistory.values()).flatMap(h => h.selections);
    ...
    allSelections.forEach(selection => {
      const backendId = this.performanceHistory.entries().next().value?.[0] || 'unknown';
      backendUsage[backendId] = (backendUsage[backendId] || 0) + 1;
    });

    return {
      totalSelections,
      averageConfidence: allSelections.reduce((sum, s) => sum + s.confidence, 0) / totalSelections,
      successRate: successfulSelections.length / totalSelections,
      backendUsage,
      averageInferenceTime: allSelections.reduce((sum, s) => sum + s.actualPerformance.inferenceTime, 0) / totalSelections
    };
  }
  ...
  private getEligibleBackends(criteria: SelectionCriteria): BackendInfo[] {
    ...
    return allBackends.filter(backend => {
      ...
      if (criteria.constraints.requireGPU && !backend.performanceProfile.gpuUsage) {
        return false;
      }
  ...
  private async scoreBackends(backends: BackendInfo[], criteria: SelectionCriteria): Promise<BackendScore[]> {
    const systemResources = this.resourceMonitor.getCurrentResources();

    return Promise.all(backends.map(async (backend) => {
      const score = await this.calculateBackendScore(backend, criteria, systemResources);
      return score;
    }));
  }
  ...
  private calculateCapabilityScore(backend: BackendInfo, criteria: SelectionCriteria): number {
    ...
    if (requiredCapability.confidence && requiredCapability.confidence > 0.8) score += 0.2;
    if (requiredCapability.performance?.throughput && requiredCapability.performance.throughput > 10) score += 0.1;

    return Math.min(1, score);
  }

  private calculateResourceScore(backend: BackendInfo, systemResources: SystemResources): number {
    const requirements = backend.resourceRequirements;

    // Memory availability
    const memoryScore = Math.min(1, systemResources.availableMemory / requirements.memory.optimal);

    // CPU availability
    const cpuScore = Math.min(1, systemResources.availableCPU / requirements.cpu.optimal);

    // GPU availability if required
    let gpuScore = 1;
    if (requirements.gpu && systemResources.availableGPU) {
      gpuScore = Math.min(1, systemResources.availableGPU / requirements.gpu.optimal);
    }

    return (memoryScore + cpuScore + gpuScore) / 3;
  }

  private calculatePerformanceScore(backend: BackendInfo, criteria: SelectionCriteria): number {
  ...
  private calculateLatencyScore(backend: BackendInfo, criteria: SelectionCriteria): number {
  ...
  private calculateCostScore(backend: BackendInfo, criteria: SelectionCriteria): number {
    // Simple cost estimation based on resource usage
    const resourceCost = backend.resourceRequirements.memory.optimal + backend.resourceRequirements.cpu.optimal;
  ...
  private calculateReliabilityScore(backend: BackendInfo): number {
    ...
    const consistency = 1 - (Math.abs(history.averageInferenceTime - backend.performanceProfile.inferenceTime) / backend.performanceProfile.inferenceTime);

    return (reliability + consistency) / 2;
  }

  private calculateModelAvailabilityScore(backend: BackendInfo, criteria: SelectionCriteria): number {
  ...
  private estimateInferenceTime(backend: BackendInfo, criteria: SelectionCriteria): number {
    ...
    const priorityMultiplier = criteria.priority === 'critical' ? 0.8 :
                               criteria.priority === 'high' ? 0.9 : 1.0;

    return baseTime * complexityMultiplier * priorityMultiplier;
  }

  private estimateMemoryUsage(backend: BackendInfo, criteria: SelectionCriteria): number {
  ...
  private estimateAccuracy(backend: BackendInfo, criteria: SelectionCriteria): number {
    ...
    const healthMultiplier = backend.health.status === 'healthy' ? 1.0 :
                             backend.health.status === 'degraded' ? 0.9 : 0.7;
  ...
  private generateCacheKey(task: AITask, criteria?: Partial<SelectionCriteria>): string {
    const keyData = {
      taskType: task.type,
      modelId: task.modelId,
      priority: task.priority,
  ...
  private updatePerformanceHistory(selection: BackendSelection, criteria: SelectionCriteria): void {
  ...
  private adjustWeightsBasedOnPerformance(selection: BackendSelection, actualPerformance: PerformanceMetrics, success: boolean): void {
    // Simple weight adjustment based on performance
    const expectedTime = selection.estimatedPerformance.inferenceTime;
    const timeDiff = Math.abs(actualPerformance.inferenceTime - expectedTime) / expectedTime;

    if (timeDiff > 0.2) { // 20% deviation
      // Adjust performance weight
      this.weights.performanceScore = Math.max(0.1, Math.min(0.4, this.weights.performanceScore * 0.95));
    }

    // Adjust weights based on success rate
    if (!success) {
      // Reduce confidence in selection criteria
      this.weights.healthStatus = Math.max(0.1, this.weights.healthStatus * 0.9);
    }
  }
}
