...
export class PerformanceProfiler {
  ...
  async initialize(): Promise<boolean> {
    try {
      ...
      console.log('[PerformanceProfiler] Performance profiler initialized successfully');
      return true;
    } catch (error) {
  ...
  async profileBackend(
    ...
    console.log(`[PerformanceProfiler] Profiling backend ${backend.id} for ${taskType}...`);
    ...
    for (let i = 0; i < iterations; i++) {
      try {
        ...
        const result = await this.measureInferencePerformance(backend, taskType, modelId);
        measurements.push(result.metrics);

        // Measure resource usage
        const resourceUsage = await this.measureResourceUsage(backend, taskType);
        resourceMeasurements.push(resourceUsage);

        // Small delay between measurements
        await new Promise(resolve => setTimeout(resolve, 100));
      } catch (error) {
        console.warn(`[PerformanceProfiler] Measurement ${i + 1} failed:`, error);
      }
    }

    // Calculate averages and create profile
    const avgMetrics = this.calculateAverageMetrics(measurements);
    const avgResourceUsage = this.calculateAverageResourceUsage(resourceMeasurements);
    ...
    console.log(`[PerformanceProfiler] Profile created for ${backend.id}:`, profile);
    return profile;
  }
  ...
  async createResourceProfile(backend: BaseBackend): Promise<ResourceProfile> {
    ...
    console.log(`[PerformanceProfiler] Resource profile created for ${backend.id}`);
    return profile;
  }

  getPerformanceProfile(backendId: string, taskType: TaskType, modelId: string): PerformanceProfile | null {
    const profileKey = `${backendId}_${taskType}_${modelId}`;
    const profiles = this.profiles.get(profileKey) || [];
    return profiles.length > 0 ? profiles[profiles.length - 1] : null;
  }

  getBenchmark(backendId: string, taskType: TaskType, modelId: string): PerformanceBenchmark | null {
  ...
  generateOptimizationRecommendations(backendId: string): OptimizationRecommendation[] {
    const recommendations: OptimizationRecommendation[] = [];

    // Analyze performance profiles
    const profiles = Array.from(this.profiles.values()).flat().filter(p => p.backendId === backendId);
    ...
    const avgInferenceTime = profiles.reduce((sum, p) => sum + p.metrics.inferenceTime, 0) / profiles.length;
    if (avgInferenceTime > 500) { // 500ms threshold
    ...
    const avgMemoryUsage = profiles.reduce((sum, p) => sum + p.metrics.memoryUsage, 0) / profiles.length;
    if (avgMemoryUsage > 200) { // 200MB threshold
      recommendations.push({
        type: 'resource',
        priority: 'medium',
  ...
  getPerformanceSummary(backendId?: string): {
    ...
    const allProfiles = backendId
      ? Array.from(this.profiles.values()).flat().filter(p => p.backendId === backendId)
    ...
    const avgMetrics = {
      inferenceTime: allProfiles.reduce((sum, p) => sum + p.metrics.inferenceTime, 0) / allProfiles.length,
      memoryUsage: allProfiles.reduce((sum, p) => sum + p.metrics.memoryUsage, 0) / allProfiles.length,
      throughput: allProfiles.reduce((sum, p) => sum + (p.metrics.throughput || 0), 0) / allProfiles.length,
      accuracy: allProfiles.reduce((sum, p) => sum + (p.metrics.accuracy || 0), 0) / allProfiles.filter(p => p.metrics.accuracy).length
    };

    const backendGroups = allProfiles.reduce((groups, profile) => {
    ...
    const backendScores = Object.entries(backendGroups).map(([id, profiles]) => {
      const avgInferenceTime = profiles.reduce((sum, p) => sum + p.metrics.inferenceTime, 0) / profiles.length;
      const avgMemoryUsage = profiles.reduce((sum, p) => sum + p.metrics.memoryUsage, 0) / profiles.length;
      return {
        backendId: id,
  ...
  private async measureInferencePerformance(
    ...
    try {
      ...
      console.error(`[PerformanceProfiler] Inference measurement failed:`, error);
      throw error;
    }
  }

  private async measureResourceUsage(backend: BaseBackend, taskType: TaskType): Promise<ResourceUsage> {
  ...
  private async measureBaselineResourceUsage(backend: BaseBackend): Promise<ResourceUsage> {
  ...
  private async measureScalingFactors(backend: BaseBackend): Promise<{ memory: number; cpu: number; gpu?: number }> {
  ...
  private async updateBenchmark(backendId: string, taskType: TaskType, modelId: string): Promise<void> {
    const benchmarkKey = `${backendId}_${taskType}_${modelId}`;
    const profiles = this.profiles.get(benchmarkKey) || [];

    if (profiles.length < 2) return;

    const averages = this.calculateAverageMetrics(profiles.map(p => p.metrics));
    const percentiles = this.calculatePercentiles(profiles.map(p => p.metrics.inferenceTime));
    const trends = this.analyzePerformanceTrends(backendId);
  ...
  private calculatePerformanceScores(profile: PerformanceProfile): {
    ...
    const speed = Math.max(0, Math.min(1, 1000 / profile.metrics.inferenceTime)); // Normalize to 1 second
    const efficiency = Math.max(0, Math.min(1, 100 / profile.metrics.memoryUsage)); // Normalize to 100MB
  ...
  private analyzePerformanceTrends(backendId: string): { trend: 'improving' | 'stable' | 'degrading'; slope: number } {
    const profiles = Array.from(this.profiles.values()).flat()
  ...
  private calculateOverhead(baseline: ResourceUsage, scalingFactors: { memory: number; cpu: number }): number {
  ...
  private calculateEfficiency(baseline: ResourceUsage, scalingFactors: { memory: number; cpu: number }): number {
  ...
  private detectUsagePattern(profiles: PerformanceProfile[]): 'consistent' | 'bursty' | 'gradual' {
  ...
  private calculateResourceCorrelation(profiles: PerformanceProfile[]): number {
    ...
    const memoryMean = memoryValues.reduce((sum, v) => sum + v, 0) / memoryValues.length;
    ...
    for (let i = 0; i < profiles.length; i++) {
      ...
      memoryDenominator += Math.pow(memoryDiff, 2);
      cpuDenominator += Math.pow(cpuDiff, 2);
    }
  ...
  private findPeakTimes(profiles: PerformanceProfile[]): number[] {
    ...
    const peaks: number[] = [];
    const threshold = values.reduce((sum, v) => sum + v, 0) / values.length * 1.5; // 1.5x mean
  ...
  private createTestInput(taskType: TaskType): any {
    ...
    switch (taskType) {
      case 'face_detection':
        return { data: new Float32Array(224 * 224 * 3), format: { type: 'image', shape: [224, 224, 3], dtype: 'float32' } };
      case 'image_embedding':
        return { data: new Float32Array(299 * 299 * 3), format: { type: 'image', shape: [299, 299, 3], dtype: 'float32' } };
      case 'text_embedding':
        return { data: ['test text'], format: { type: 'text', dtype: 'string' } };
      default:
        return { data: new Float32Array(100), format: { type: 'tensor', shape: [100], dtype: 'float32' } };
    }
  }
  ...
  private estimateInputSize(taskType: TaskType): number {
    ...
    switch (taskType) {
      case 'face_detection': return 224 * 224 * 3 * 4; // 224x224 RGB image as float32
      case 'image_embedding': return 299 * 299 * 3 * 4; // 299x299 RGB image as float32
      case 'text_embedding': return 100; // Small text input
      default: return 1000; // Default estimate
    }
  }
  ...
  private estimateAccuracy(taskType: TaskType, output: any): number {
    ...
    switch (taskType) {
      case 'face_detection': return 0.85;
      case 'image_embedding': return 0.90;
      case 'text_embedding': return 0.88;
      default: return 0.80;
    }
  }
