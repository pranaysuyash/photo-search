...
export abstract class BaseBackend implements ANNBackend {
	...
	// Inference methods
	public abstract runInference(
		modelId: string,
		input: InferenceInput,
	): Promise<InferenceOutput>;
	public abstract runBatchInference(
		modelId: string,
	...
	public canHandleModel(model: Model): boolean {
		return this.capabilities.some((cap) =>
			cap.modelTypes.some(
				(supported) => supported.framework === model.framework,
			),
		);
	}

	public getResourceEstimate(
		modelId: string,
		inputSize: number,
	): ResourceRequirements {
