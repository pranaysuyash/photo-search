...
export class PerformanceMonitor {
  ...
  private checkMemoryThresholds(): void {
    ...
    if (usagePercent > 90) {
      this.createAlert('critical', `Critical memory usage: ${usagePercent}% (${usageMB}MB)`, {
        memoryUsage: this.metrics.memoryUsage,
      });
    } else if (usagePercent > 75) {
      this.createAlert('warning', `High memory usage: ${usagePercent}% (${usageMB}MB)`, {
        memoryUsage: this.metrics.memoryUsage,
      });
    }
  }

  private createAlert(type: 'warning' | 'critical', message: string, metrics: Partial<PerformanceMetrics>): void {
...
export function usePerformanceMonitor() {
  ...
  return {
    ...
    updateLibraryMetrics: (data: any) => monitor.updateLibraryMetrics(data),
