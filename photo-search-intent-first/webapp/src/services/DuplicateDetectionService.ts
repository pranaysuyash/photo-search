...
export class DuplicateDetectionService {
  ...
  static async detectDuplicates(
    ...
    for (const photo of photos) {
      const fingerprint = await this.generateFingerprint(photo.path, photo.embedding);
    ...
    for (let i = 0; i < fingerprints.length; i++) {
      for (let j = i + 1; j < fingerprints.length; j++) {
        ...
        if (similarity >= this.SIMILAR_THRESHOLD) {
          const groupKey = this.getGroupKey(fingerprints[i], fingerprints[j], similarity);
    ...
    duplicateMap.forEach((photos, key) => {
      ...
      const similarity = this.getAverageSimilarity(photos);
      const type = this.getDuplicateType(similarity);
    ...
    const recommendations = this.generateRecommendations(groups);
  ...
  private static async generateFingerprint(
    ...
    // Check cache first
    if (this.fingerprintCache.has(path)) {
      return this.fingerprintCache.get(path)!;
    }
    
    // Generate hashes
    const hash = await this.calculateFileHash(path);
    const perceptualHash = await this.calculatePerceptualHash(path);
   ...
   */
  private static async calculatePerceptualHash(path: string): Promise<string> {
  ...
  private static calculateSimilarity(
    ...
    const hashSimilarity = this.comparePerceptualHashes(
    ...
    if (photo1.embedding && photo2.embedding) {
      embeddingSimilarity = this.cosineSimilarity(photo1.embedding, photo2.embedding);
    }
    ...
    const dimensionSimilarity = this.compareDimensions(photo1.dimensions, photo2.dimensions);
    const timeSimilarity = this.compareTimestamps(photo1.timestamp, photo2.timestamp);
  ...
  private static getAverageSimilarity(photos: PhotoFingerprint[]): number {
    ...
    for (let i = 0; i < photos.length; i++) {
      for (let j = i + 1; j < photos.length; j++) {
        totalSimilarity += this.calculateSimilarity(photos[i], photos[j]);
        comparisons++;
      }
    }
   ...
   */
  private static getDuplicateType(similarity: number): 'exact' | 'near' | 'similar' {
    if (similarity >= this.EXACT_THRESHOLD) return 'exact';
    if (similarity >= this.NEAR_THRESHOLD) return 'near';
    return 'similar';
  }
  ...
  private static getGroupKey(
    ...
    if (similarity >= this.EXACT_THRESHOLD && photo1.hash === photo2.hash) {
      return `exact-${photo1.hash}`;
    }
    
    // For near duplicates, use perceptual hash prefix
    if (similarity >= this.NEAR_THRESHOLD) {
  ...
  private static generateRecommendations(groups: DuplicateGroup[]): string[] {
    ...
    if (totalSavings > 1073741824) { // 1GB
      recommendations.push(
    ...
    const lowQualityDuplicates = groups.filter(g => 
