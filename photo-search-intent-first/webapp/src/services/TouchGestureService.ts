...
export class TouchGestureService {
  ...
  private onPinchCallback?: (scale: number, centerX: number, centerY: number) => void
  ...
  handleTouchStart(e: TouchEvent) {
    ...
    if (this.doubleTapTimer && now - this.lastTouchEndTime < 300) {
      // Double tap detected
      this.handleDoubleTap(this.touchStartPoints[0].x, this.touchStartPoints[0].y)
  ...
  handleTouchEnd(e: TouchEvent) {
    ...
    if (this.gestureState.isSwiping && touchDuration < this.config.maxSwipeTime!) {
  ...
  private handleSingleTouchMove(touch: TouchPoint) {
    ...
    // Swipe detection
    if (distance > this.config.minSwipeDistance! && !this.gestureState.isPanning) {
  ...
  private handlePinchMove(touches: TouchPoint[]) {
    ...
    if (Math.abs(currentDistance - startDistance) > this.config.minPinchDistance!) {
      ...
      const scale = (currentDistance / startDistance) * this.gestureState.lastScale
  ...
  isGestureActive(): boolean {
    return this.gestureState.isPinching || 
