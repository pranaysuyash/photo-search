...
class SearchHistoryService {
  ...
  private calculateHistoryScore(entry: SearchHistoryEntry, query: string): number {
  ...
  private generateSimilarQueries(input: string, history: SearchHistoryEntry[]): string[] {
    ...
    history.forEach(entry => {
      ...
      if (commonWords.length > 0 && entry.query.toLowerCase() !== input.toLowerCase()) {
