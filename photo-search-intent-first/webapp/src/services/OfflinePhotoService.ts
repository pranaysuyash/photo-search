...
export class OfflinePhotoService {
    ...
    return new Promise<void>((resolve, reject) => {
      ...
      request.onupgradeneeded = (event) => {
        ...
        if (!db.objectStoreNames.contains('photoCache')) {
          const photoStore = db.createObjectStore('photoCache', { keyPath: 'path' })
  ...
  async cachePhoto(path: string, priority: 'high' | 'medium' | 'low' = 'medium') {
    try {
      ...
      console.error(`[OfflinePhotoService] Failed to cache photo: ${path}`, error)
    }
  }

  // Cache multiple photos efficiently
  async cachePhotos(photos: Array<{ path: string; priority?: 'high' | 'medium' | 'low' }>) {
  ...
  async getCachedPhotoUrl(path: string, size: 'thumbnail' | 'full' = 'thumbnail'): Promise<string | null> {
    try {
      ...
      console.error(`[OfflinePhotoService] Failed to get cached photo: ${path}`, error)
    }
    return null
  }
    ...
    try {
      ...
      return new Promise((resolve) => {
        request.onsuccess = () => {
          ...
          resolve(totalSize < maxSizeBytes && entries.length < this.config.maxCacheEntries!)
        }
        request.onerror = () => resolve(false)
      })
    } catch {
    ...
    try {
      ...
      return new Promise<void>((resolve) => {
        request.onsuccess = async () => {
          const entries = request.result as PhotoCacheEntry[]
          const now = Date.now()
          const expirationTime = this.config.cacheExpirationDays! * 24 * 60 * 60 * 1000
  ...
  async queueOfflineAction(action: any) {
    try {
      ...
      console.error('[OfflinePhotoService] Failed to queue offline action:', error)
    }
  }
        ...
        request.onsuccess = async () => {
          ...
          for (const action of actions) {
            try {
              ...
              console.error('[OfflinePhotoService] Failed to sync action:', error)
            }
          }
  ...
  private async processOfflineAction(action: any) {
    ...
    switch (action.type) {
      ...
      case 'rating':
        ...
        break
      default:
        console.warn('[OfflinePhotoService] Unknown offline action type:', action.type)
    }
  }
