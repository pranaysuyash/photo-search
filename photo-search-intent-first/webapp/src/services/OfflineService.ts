...
function isSearchActionPayload(payload: unknown): payload is SearchActionPayload {
  return (
...
function isCollectionActionPayload(payload: unknown): payload is CollectionActionPayload {
  return (
    typeof payload === "object" &&
    payload !== null &&
    "dir" in payload &&
...
function isTagActionPayload(payload: unknown): payload is TagActionPayload {
  return (
    ...
    (typeof (payload as TagActionPayload).operation === "string" &&
      ["add", "remove", "replace"].includes((payload as TagActionPayload).operation))
  );
}

function isDeleteActionPayload(payload: unknown): payload is DeleteActionPayload {
  return (
    typeof payload === "object" &&
    payload !== null &&
    "dir" in payload &&
...
function isSetTagsActionPayload(payload: unknown): payload is SetTagsActionPayload {
  return (
    typeof payload === "object" &&
    payload !== null &&
    "dir" in payload &&
...
function isFavoriteActionPayload(payload: unknown): payload is FavoriteActionPayload {
  return (
    typeof payload === "object" &&
    payload !== null &&
    "dir" in payload &&
...
function isSavedSearchActionPayload(payload: unknown): payload is SavedSearchActionPayload {
  return (
    typeof payload === "object" &&
    payload !== null &&
    "dir" in payload &&
...
function isDeleteSavedSearchActionPayload(payload: unknown): payload is DeleteSavedSearchActionPayload {
  return (
    typeof payload === "object" &&
    payload !== null &&
    "dir" in payload &&
...
function isBatchAddToCollectionActionPayload(payload: unknown): payload is BatchAddToCollectionActionPayload {
  return (
    typeof payload === "object" &&
    payload !== null &&
    "dir" in payload &&
    ...
    typeof (payload as BatchAddToCollectionActionPayload).collectionName === "string"
  );
}
...
function isBuildMetadataActionPayload(payload: unknown): payload is BuildMetadataActionPayload {
  return (
    typeof payload === "object" &&
    payload !== null &&
    "dir" in payload &&
...
function isBuildOCRAActionPayload(payload: unknown): payload is BuildOCRAActionPayload {
  return (
    typeof payload === "object" &&
    payload !== null &&
    "dir" in payload &&
...
function isBuildFacesActionPayload(payload: unknown): payload is BuildFacesActionPayload {
  return (
    typeof payload === "object" &&
    payload !== null &&
    "dir" in payload &&
...
function isSmartCollectionActionPayload(payload: unknown): payload is SmartCollectionActionPayload {
  return (
    typeof payload === "object" &&
    payload !== null &&
    "dir" in payload &&
...
function isDeleteSmartCollectionActionPayload(payload: unknown): payload is DeleteSmartCollectionActionPayload {
  return (
    typeof payload === "object" &&
    payload !== null &&
    "dir" in payload &&
...
function isDeleteCollectionActionPayload(payload: unknown): payload is DeleteCollectionActionPayload {
  return (
    typeof payload === "object" &&
    payload !== null &&
    "dir" in payload &&
...
class OfflineService {
  ...
  public async queueActions(
    actions: Omit<OfflineAction, "id" | "timestamp" | "retries" | "authContext">[]
  ): Promise<string[]> {
    const authContext = this.captureAuthContext();
        ...
        for (const action of queue) {
          try {
            ...
            try {
              ...
              console.error("[Offline Service] Failed to save action to IndexedDB:", updateError);
            }
          }
        }
        return;
  ...
  private async removeActionsFromQueue(ids: string[]): Promise<void> {
    try {
      ...
      console.error("[Offline Service] Failed to remove actions from queue:", error);
    }
  }
  ...
  public async syncQueue() {
      ...
      const batchPromises = batch.map(async (action) => {
        try {
          ...
          if (action.retries < this.MAX_RETRIES) {
            ...
            const dir = (payload as any)?.dir || (payload as any)?.path || "";
    ...
    if (remaining.length > 0) {
      ...
      try {
        ...
        console.error("[Offline Service] Failed to save remaining actions:", error);
      }
  ...
  private async processAction(action: OfflineAction): Promise<void> {
    ...
    try {
      originalToken = typeof window !== "undefined" ? localStorage.getItem("api_token") : null;
    } catch {}
  ...
  public async getQueueStatistics(): Promise<{
    ...
    const stats = {
      ...
      oldestAction: queue.length > 0 ? Math.min(...queue.map(a => a.timestamp)) : undefined,
      storageSize: 0
    };
  ...
  public async getPriorityActions(limit: number = 10): Promise<OfflineAction[]> {
    const queue = await this.getQueue();

    // Prioritize actions: search > metadata changes > deletions > batch operations
    const priorityOrder = ["search", "favorite", "set_tags", "tag", "collection", "delete", "saved_search"];
  ...
  public async retryFailedActions(): Promise<void> {
    const queue = await this.getQueue();
    const failedActions = queue.filter(a => a.retries > 0 && a.retries < this.MAX_RETRIES);

    if (failedActions.length > 0) {
      console.log(`[Offline Service] Retrying ${failedActions.length} failed actions`);
  ...
  public async cancelActions(actionIds: string[]): Promise<void> {
    ...
    console.log(`[Offline Service] Cancelled ${actionIds.length} actions`);
  }
  ...
  public async getActionsByDateRange(start: number, end: number): Promise<OfflineAction[]> {
    const queue = await this.getQueue();
    return queue.filter(action => action.timestamp >= start && action.timestamp <= end);
  }
  ...
  private async measureNetworkQuality(): Promise<number> {
    try {
      ...
      const response = await fetch(`${API_BASE}/api/monitoring?ts=${Date.now()}`, {
        method: "GET",
        cache: "no-store",
        headers: { Accept: "application/json" },
      });
  ...
  public async adaptiveSync(): Promise<void> {
    ...
    if (priorityActions.length > 0) {
      console.log(`[Offline Service] Adaptive sync: processing ${priorityActions.length} actions (network quality: ${networkQuality.toFixed(2)})`);
      const actionIds = priorityActions.map(a => a.id);
      const result = await this.syncActions(actionIds);

      console.log(`[Offline Service] Adaptive sync results: ${result.successful.length} successful, ${result.failed.length} failed`);
    }
  }
