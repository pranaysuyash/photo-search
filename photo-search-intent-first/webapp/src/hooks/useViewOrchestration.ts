...
export function useViewOrchestration() {
  ...
  useQueryParamFilters({
    ...
    settingsActions: {
      ...
      setTimelineBucket: appState.stateActions.settingsActions.setTimelineBucket,
    },
    workspaceActions: {
  ...
  // Navigation actions
  const navigateTo = useCallback((viewId: string, options?: { replace?: boolean }) => {
  ...
  // View validation
  const shouldShowView = useCallback((viewId: string) => {
    const view = availableViews.find(v => v.id === viewId);
    ...
    return true;
  }, [availableViews, contexts.connectivity.isAuthenticated, appState.viewState.library.length]);

  const getViewTitle = useCallback((viewId: string) => {
    const view = availableViews.find(v => v.id === viewId);
  ...
  const shouldShowSidebar = useMemo(() => {
    return ["search", "library", "collections", "people"].includes(appState.currentView);
  }, [appState.currentView]);
  ...
  const getPageTitle = useMemo(() => {
    ...
    if (appState.currentView === "collections" && appState.localState.currentFilter !== "all") {
      return `${appState.localState.currentFilter} Collections - ${viewTitle}`;
    }

    return viewTitle;
  }, [appState.currentView, appState.viewState.query, appState.localState.currentFilter, getViewTitle]);
  ...
  return {
    ...
    // Helpers
    getPageTitle,
    getCurrentViewConfig: () => availableViews.find(v => v.id === appState.currentView),
    getNavigationViews: () => availableViews.filter(v => v.showInNavigation && shouldShowView(v.id)),
  };
}