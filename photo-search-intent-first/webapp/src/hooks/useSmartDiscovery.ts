...
export function useSmartDiscovery(options: UseSmartDiscoveryOptions = {}) {
	...
	const loadRecommendations = useCallback(
		async (algorithm?: DiscoveryAlgorithm, limit?: number) => {
			try {
				setLoading(true);
				setError(null);
	...
	// Load forgotten gems
	const loadForgottenGems = useCallback(
		async (limit?: number) => {
			try {
	...
	// Load insights
	const getInsights =
		useCallback(async (): Promise<DiscoveryInsights | null> => {
			try {
	...
	const recordSearch = useCallback(
		(query: string, resultsCount: number, clickedResults: string[] = []) => {
			if (!trackInteractions) return;

			try {
				discoveryService.recordSearch(
					query,
					resultsCount,
					clickedResults,
					userId,
