...
import type { OfflinePhotoStorage } from "../services/EnhancedOfflineStorage";
import {
...
import { useUIActions } from "../stores/useStores";
import {
  useFavOnly,
  useTagFilter,
  useTopK,
  usePhotoActions as usePhotoActionsFromPhotoStore,
...
export function useOfflineFirstSearch() {
  ...
  const photoActions = usePhotoActionsFromPhotoStore();
...
export function useOfflineFirstLibrary() {
  const dir = useDir();
  const [library, setLibrary] = useState<OfflinePhotoStorage[]>([]);
