...
export function useGlobalShortcuts({
    ...
    const onKey = (e: KeyboardEvent) => {
      ...
      if (selectedView === "results" && resultView === "timeline") {
        const dispatch = (kind: string) =>
          window.dispatchEvent(new CustomEvent("timeline-jump", { detail: { kind } }));
  ...
  }, [anyModalOpen, openModal, toggleModal, searchCommandCenter, showInfoOverlay, setShowInfoOverlay, selectedView, resultView]);
}
