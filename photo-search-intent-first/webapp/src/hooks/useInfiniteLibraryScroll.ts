...
export function useInfiniteLibraryScroll(
	...
	const cleanupMemory = useCallback(() => {
		...
		if (usedMB > maxMemoryMB) {
			...
			console.log(
				`Memory cleanup: removed ${itemsToRemove} items to reduce memory usage`,
			);
		}
	}, [items, maxMemoryMB, checkMemoryUsage]);

	const loadBatch = useCallback(
		async (loadOffset: number, append: boolean = false) => {
			...
			try {
				...
				const total =
					typeof response.total === "number" ? response.total : Infinity;
				const newHasMore =
					newItems.length === batchSize && loadOffset + newItems.length < total;
...
export function usePredictivePreloading(
	...
	return {
		preloadedItems,
		isPreloaded: (item: string) => preloadedItems.has(item),
	};
}
