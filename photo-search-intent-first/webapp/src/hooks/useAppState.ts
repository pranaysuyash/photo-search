...
export function useAppState() {
  ...
  const handleSetResultView = useCallback((view: ResultView) => {
    setResultView(view);
  ...
  const handleSetTimelineBucket = useCallback((bucket: "day" | "week" | "month") => {
  ...
  const viewState: ViewState = {
    // Settings
    dir, engine, hfToken, openaiKey, useFast, fastKind, useCaps, useOcr, useOsTrash, hasText, place, camera, isoMin, isoMax, fMin, fMax,
    // Photo
    results, query, fav, favOnly, topK, tagFilter, allTags, collections, smart, library, libHasMore, tagsMap, needsHf, needsOAI,
    // UI
    busy, note, showWelcome,
    // Workspace
    persons, clusters, points, diag, wsToggle,
  };

  const localState: LocalUIState = {
    searchText, selected, gridSize, resultView, timelineBucket, currentFilter, showFilters, dateFrom, dateTo, ratingMin,
  };
