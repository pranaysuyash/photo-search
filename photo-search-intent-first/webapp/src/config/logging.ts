...
const viteEnv = ((import.meta as unknown as { env?: Record<string, any> })?.env || {}) as Record<string, any>;
// Resolve env dynamically so tests can tweak process.env between calls
// eslint-disable-next-line @typescript-eslint/no-explicit-any
function getEnv(): Record<string, any> { return { ...(typeof process !== "undefined" ? (process as any).env : {}), ...viteEnv }; }
...
export function getLoggingConfig() {
  ...
  return {
    ...
    envGate: (String(env.VITE_LOG_ERRORS_ENV || "prod") === "all" ? "all" : "prod") as LogEnvGate,
...
export type LogService = "image" | "video" | "offline" | "backup" | "offlinePhoto";
...
export function serviceEnabled(service: LogService): boolean {
  ...
  switch (service) {
    ...
    case "offline":
      return bool(env.VITE_LOG_OFFLINE_ERRORS, true) && shouldLogErrorsToServer();
    case "backup":
      return bool(env.VITE_LOG_BACKUP_ERRORS, true) && shouldLogErrorsToServer();
    case "offlinePhoto":
      return bool(env.VITE_LOG_OFFLINE_PHOTO_ERRORS, true) && shouldLogErrorsToServer();
    default:
