...
const fixes = [
	{
		...
		pattern:
			/key=\{`\$\{rec\.id \|\| rec\.path \|\| rec\.name \|\| rec\.key \|\| ""\}-\$\{idx\}`\}/g,
		replacement: "key={`rec-${idx}`}",
	},
	{
		file: "src/components/CommandPalette.tsx",
		pattern:
			/key=\{`\$\{key\.id \|\| key\.path \|\| key\.name \|\| key\.key \|\| ""\}-\$\{idx\}`\}/g,
	...
	},
	{
		...
		pattern:
			/key=\{`\$\{w\.id \|\| w\.path \|\| w\.name \|\| w\.key \|\| ""\}-\$\{i\}`\}/g,
