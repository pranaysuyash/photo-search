...
export async function getIndexStatus(dir: string): Promise<IndexStatus> {
  const res = await fetch(`${API_BASE}/indexing/status?dir=${encodeURIComponent(dir)}`);
...
export async function getFavorites(dir: string): Promise<string[]> {
  const res = await fetch(`${API_BASE}/favorites?dir=${encodeURIComponent(dir)}`);
  return handleResponse(res);
}

export async function toggleFavorite(dir: string, photoPath: string): Promise<void> {
