  ...
  persist(
    (set, get) => ({
      ...
      setThumbnailQuality: (thumbnailQuality) => 
        set({ thumbnailQuality }),
      
      setEnableAnimations: (enableAnimations) => 
        set({ enableAnimations }),
      
      setEnableTelemetry: (enableTelemetry) => 
        set({ enableTelemetry }),
      
      // Search actions
      setSearchResultLimit: (searchResultLimit) =>
        set({ searchResultLimit }),
...
export const useThumbnailQuality = () => useUserPrefsStore(state => state.thumbnailQuality);
export const useEnableAnimations = () => useUserPrefsStore(state => state.enableAnimations);
export const usePrivacyPrefs = () => useUserPrefsStore(state => ({
  faceRecognition: state.enableFaceRecognition,
  locationData: state.enableLocationData,
}));
export const useHasCompletedOnboarding = () => useUserPrefsStore(state => state.hasCompletedOnboarding);
