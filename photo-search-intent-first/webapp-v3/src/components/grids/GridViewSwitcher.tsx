...
interface GridViewSwitcherProps {
  ...
  compact?: boolean;
  className?: string;
  /** Optional external view mode setter for Electron integration */
...
export const GridViewSwitcher: React.FC<GridViewSwitcherProps> = ({
  ...
  /**
   * Electron menu shortcuts integration (Grid/List view)
   */
  useEffect(() => {
    // Only in Electron environment
    const bridge =
      typeof window !== "undefined" ? window.electronAPI : undefined;
    if (!bridge || typeof bridge.onSetViewMode !== "function") {
      return;
    }

    const handleElectronViewMode = (_event: unknown, mode: string) => {
      // Map Electron modes to our view modes
      const modeMap: Record<string, ViewMode> = {
        grid: "masonry",
        list: "filmstrip",
      };

      const viewMode = modeMap[mode];
      if (viewMode) {
        // Use state setter directly to avoid dependency issues
    ...
    bridge.onSetViewMode?.(handleElectronViewMode);

    return () => {
      const remover = bridge.removeListener ?? bridge.off;
      remover?.(
        "set-view-mode",
        handleElectronViewMode as (event: unknown, ...args: unknown[]) => void
      );
    };
  }, [onViewModeChange]);

  useEffect(() => {
    onViewModeChange?.(currentView);
  }, [currentView, onViewModeChange]);
